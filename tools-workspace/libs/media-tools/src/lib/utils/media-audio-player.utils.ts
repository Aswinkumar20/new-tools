...
import type { AudioRepeatMode } from '../types/media-audio-player.types';
...
export function resolveAudioSuggestion(options: {
  ...
  if (!hasTracks) {
    return {
      id: 'ap-voice',
      title: 'Need a fresh recording?',
      reason: 'Capture audio with Voice Recorder, then load the clip here for playback and review.',
      actionLabel: 'Open Voice Recorder',
