...
import { FreemindViewerComponent } from './freemind-viewer';
import { createFmFileRecord, createSampleFmFile } from '../../utils/freemind-viewer.utils';

@Component({ selector: 'lib-navigation', standalone: true, template: '' })
class StubNavigationComponent {}

describe('FreemindViewerComponent', () => {
  let component: FreemindViewerComponent;
  let fixture: ComponentFixture<FreemindViewerComponent>;
  ...
  beforeEach(async () => {
    ...
    await TestBed.configureTestingModule({
      imports: [FreemindViewerComponent],
      providers: [
        provideRouter([]),
      ...
      .overrideComponent(FreemindViewerComponent, {
        remove: { imports: [Navigation] },
        add: { imports: [StubNavigationComponent] }
      })
      .compileComponents();

    fixture = TestBed.createComponent(FreemindViewerComponent);
  ...
  function sampleBytes(): Uint8Array {
    return new TextEncoder().encode(FM_MM_SAMPLE);
  }

  function loadSampleRecord(): void {
    const file = createSampleFmFile();
    const record = createFmFileRecord(file, sampleBytes());
  ...
  it('loads sample via handleFiles and enables export', async () => {
    ...
    expect(component.currentFile?.name).toBe('sample-shop.mm');
  ...
  it('removeFile clears when last file removed and updates index otherwise', async () => {
    await component.loadSample();
    const second = createFmFileRecord(new File([sampleBytes()], 'second.mm', { lastModified: 2 }), sampleBytes());
  ...
  it('shows error suggestion after a hard load failure message', () => {
    component.errorMessage = 'bad.mm: Invalid FreeMind map';
  ...
  it('keeps scroll-owner structure for map wrap, outline, table, and sidebar', async () => {
    ...
    expect(root.querySelector('.fm-map-wrap')).toBeTruthy();
    expect(root.querySelector('.fm-workspace')).toBeTruthy();
    expect(root.querySelector('.fm-sidebar')).toBeTruthy();
    component.setViewMode('table');
    fixture.detectChanges();
    expect(root.querySelector('.fm-table')).toBeTruthy();
  });

  it('collapses sidebar via toggle and keeps toolbar present', () => {
    loadSampleRecord();
    expect(fixture.debugElement.query(By.css('.fm-toolbar'))).toBeTruthy();
    component.toggleSidebar();
    fixture.detectChanges();
    expect(component.sidebarCollapsed).toBe(true);
    expect((fixture.nativeElement as HTMLElement).querySelector('.fm-workspace--sidebar-collapsed')).toBeTruthy();
    expect((fixture.nativeElement as HTMLElement).querySelector('.fm-sidebar')).toBeFalsy();
  });
});
