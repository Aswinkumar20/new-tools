...
export class PrismaSchemaViewerComponent implements AfterViewInit, OnDestroy {
    ...
    try {
      ...
      if (this.currentFile) {
        ...
        if (this.currentFile.softFail) {
          this.toast.warning('Parsed with little or no models — metadata may still be available');
  ...
  onFilterChange(): void {
    if (this.selectedModelId && !this.filteredModels.some((m) => m.id === this.selectedModelId)) {
