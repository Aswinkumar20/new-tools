...
import { PrismaSchemaViewerComponent } from './prisma-schema-viewer';
import { createPrmFileRecord, createSamplePrmFile } from '../../utils/prisma-schema-viewer.utils';

@Component({ selector: 'lib-navigation', standalone: true, template: '' })
class StubNavigationComponent {}

describe('PrismaSchemaViewerComponent', () => {
  let component: PrismaSchemaViewerComponent;
  let fixture: ComponentFixture<PrismaSchemaViewerComponent>;
  ...
  beforeEach(async () => {
    ...
    await TestBed.configureTestingModule({
      imports: [PrismaSchemaViewerComponent],
      providers: [
        provideRouter([]),
      ...
      .overrideComponent(PrismaSchemaViewerComponent, {
        remove: { imports: [Navigation] },
        add: { imports: [StubNavigationComponent] }
      })
      .compileComponents();

    fixture = TestBed.createComponent(PrismaSchemaViewerComponent);
  ...
  function sampleBytes(): Uint8Array {
    return new TextEncoder().encode(PRM_SAMPLE);
  }

  function loadSampleRecord(): void {
    const file = createSamplePrmFile();
    const record = createPrmFileRecord(file, sampleBytes());
  ...
  it('loads sample via handleFiles and enables export', async () => {
    ...
    expect(component.currentFile?.name).toBe('sample-shop.prisma');
    expect(component.canExport).toBe(true);
    expect(component.parsed?.models.length).toBe(4);
    expect(component.parsed?.relations.length).toBe(3);
  ...
  it('filters models and repairs selection when filter removes current item', () => {
    loadSampleRecord();
    const first = component.filteredModels[0];
    expect(first).toBeTruthy();
    component.selectedModelId = first.id;
    component.query = 'model:__none__';
    component.onFilterChange();
    expect(component.filteredModels.length).toBe(0);
    expect(component.selectedModelId).toBe('');
  });

  it('filters relations and repairs selection in table mode', () => {
    loadSampleRecord();
    component.setViewMode('table');
    const first = component.filteredRelations[0];
    component.selectedRelationId = first.id;
    component.query = 'from:__none__';
    component.onFilterChange();
    expect(component.filteredRelations.length).toBe(0);
    expect(component.selectedRelationId).toBe('');
  });

  it('selects model and relation', () => {
    loadSampleRecord();
    const model = component.parsed!.models.find((m) => m.name === 'Customer')!;
    component.selectModel(model.id);
    expect(component.selectedModelId).toBe(model.id);
  ...
  it('removeFile clears when last file removed and updates index otherwise', async () => {
    await component.loadSample();
    const second = createPrmFileRecord(new File([sampleBytes()], 'second.prisma', { lastModified: 2 }), sampleBytes());
  ...
  it('shows error suggestion after a hard load failure message', () => {
    component.errorMessage = 'bad.prisma: Invalid Prisma schema';
  ...
  it('keeps scroll-owner structure for map wrap, cards, table, and sidebar', async () => {
    ...
    expect(root.querySelector('.prm-map-wrap')).toBeTruthy();
    expect(root.querySelector('.prm-workspace')).toBeTruthy();
    expect(root.querySelector('.prm-sidebar')).toBeTruthy();
    component.setViewMode('table');
    fixture.detectChanges();
    expect(root.querySelector('.prm-table')).toBeTruthy();
  });

  it('collapses sidebar via toggle and keeps toolbar present', () => {
    loadSampleRecord();
    expect(fixture.debugElement.query(By.css('.prm-toolbar'))).toBeTruthy();
    component.toggleSidebar();
    fixture.detectChanges();
    expect(component.sidebarCollapsed).toBe(true);
    expect((fixture.nativeElement as HTMLElement).querySelector('.prm-workspace--sidebar-collapsed')).toBeTruthy();
    expect((fixture.nativeElement as HTMLElement).querySelector('.prm-sidebar')).toBeFalsy();
  });
});
