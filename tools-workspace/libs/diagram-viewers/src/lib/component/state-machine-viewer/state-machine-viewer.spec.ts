...
import { StateMachineViewerComponent } from './state-machine-viewer';
import { createSampleSmFile, createSmFileRecord } from '../../utils/state-machine-viewer.utils';

@Component({ selector: 'lib-navigation', standalone: true, template: '' })
class StubNavigationComponent {}

describe('StateMachineViewerComponent', () => {
  let component: StateMachineViewerComponent;
  let fixture: ComponentFixture<StateMachineViewerComponent>;
  ...
  beforeEach(async () => {
    ...
    await TestBed.configureTestingModule({
      imports: [StateMachineViewerComponent],
      providers: [
        provideRouter([]),
      ...
      .overrideComponent(StateMachineViewerComponent, {
        remove: { imports: [Navigation] },
        add: { imports: [StubNavigationComponent] }
      })
      .compileComponents();

    fixture = TestBed.createComponent(StateMachineViewerComponent);
  ...
  function sampleBytes(): Uint8Array {
    return new TextEncoder().encode(SM_SAMPLE);
  }

  function loadSampleRecord(): void {
    const file = createSampleSmFile();
    const record = createSmFileRecord(file, sampleBytes());
  ...
  it('loads sample via handleFiles and enables export', async () => {
    ...
    expect(component.currentFile?.name).toBe('sample-shop.scxml');
    expect(component.canExport).toBe(true);
    expect(component.parsed?.states.length).toBe(4);
    expect(component.parsed?.transitions.length).toBeGreaterThanOrEqual(4);
  ...
  it('filters states and repairs selection when filter removes current item', () => {
    loadSampleRecord();
    const first = component.filteredStates[0];
    expect(first).toBeTruthy();
    component.selectedStateId = first.id;
    component.query = 'state:__none__';
    component.onFilterChange();
    expect(component.filteredStates.length).toBe(0);
    expect(component.selectedStateId).toBe('');
  });

  it('filters transitions and repairs selection in table mode', () => {
    loadSampleRecord();
    component.setViewMode('table');
    const first = component.filteredTransitions[0];
    component.selectedTransitionId = first.id;
    component.query = 'event:__none__';
    component.onFilterChange();
    expect(component.filteredTransitions.length).toBe(0);
    expect(component.selectedTransitionId).toBe('');
  });

  it('selects state and transition', () => {
    loadSampleRecord();
    const state = component.parsed!.states.find((s) => s.id === 'idle')!;
    component.selectState(state.id);
    expect(component.selectedStateId).toBe(state.id);
  ...
  it('switches view modes and toggles sidebar', () => {
    ...
    expect(component.viewMode).toBe('transitions');
  ...
  it('removeFile clears when last file removed and updates index otherwise', async () => {
    await component.loadSample();
    const second = createSmFileRecord(new File([sampleBytes()], 'second.scxml', { lastModified: 2 }), sampleBytes());
  ...
  it('shows error suggestion after a hard load failure message', () => {
    component.errorMessage = 'bad.scxml: Invalid state machine';
  ...
  it('keeps scroll-owner structure for map wrap, cards, table, and sidebar', async () => {
    ...
    expect(root.querySelector('.sm-map-wrap')).toBeTruthy();
    expect(root.querySelector('.sm-workspace')).toBeTruthy();
    expect(root.querySelector('.sm-sidebar')).toBeTruthy();
    component.setViewMode('table');
    fixture.detectChanges();
    expect(root.querySelector('.sm-table')).toBeTruthy();
  });

  it('collapses sidebar via toggle and keeps toolbar present', () => {
    loadSampleRecord();
    expect(fixture.debugElement.query(By.css('.sm-toolbar'))).toBeTruthy();
    component.toggleSidebar();
    fixture.detectChanges();
    expect(component.sidebarCollapsed).toBe(true);
    expect((fixture.nativeElement as HTMLElement).querySelector('.sm-workspace--sidebar-collapsed')).toBeTruthy();
    expect((fixture.nativeElement as HTMLElement).querySelector('.sm-sidebar')).toBeFalsy();
  });
});
