...
export class StateMachineViewerComponent implements AfterViewInit, OnDestroy {
    ...
    try {
      ...
      if (this.currentFile) {
        ...
        if (this.currentFile.softFail) {
          this.toast.warning('Parsed with little or no states — metadata may still be available');
  ...
  onFilterChange(): void {
    if (this.selectedStateId && !this.filteredStates.some((n) => n.id === this.selectedStateId)) {
      this.selectedStateId = this.filteredStates[0]?.id ?? '';
    }
