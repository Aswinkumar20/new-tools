...
import { FreeplaneViewerComponent } from './freeplane-viewer';
import { createFpFileRecord, createSampleFpFile } from '../../utils/freeplane-viewer.utils';

@Component({ selector: 'lib-navigation', standalone: true, template: '' })
class StubNavigationComponent {}

describe('FreeplaneViewerComponent', () => {
  let component: FreeplaneViewerComponent;
  let fixture: ComponentFixture<FreeplaneViewerComponent>;
  ...
  beforeEach(async () => {
    ...
    await TestBed.configureTestingModule({
      imports: [FreeplaneViewerComponent],
      providers: [
        provideRouter([]),
      ...
      .overrideComponent(FreeplaneViewerComponent, {
        remove: { imports: [Navigation] },
        add: { imports: [StubNavigationComponent] }
      })
      .compileComponents();

    fixture = TestBed.createComponent(FreeplaneViewerComponent);
  ...
  function sampleBytes(): Uint8Array {
    return new TextEncoder().encode(FP_MM_SAMPLE);
  }

  function loadSampleRecord(): void {
    const file = createSampleFpFile();
    const record = createFpFileRecord(file, sampleBytes());
  ...
  it('loads sample via handleFiles and enables export', async () => {
    ...
    expect(component.currentFile?.name).toBe('sample-shop-freeplane.mm');
    expect(component.canExport).toBe(true);
    expect(component.parsed?.nodes.length).toBe(4);
    expect(component.parsed?.icons.length).toBe(5);
  ...
  it('selects node, icon, and toggles collapse', () => {
    loadSampleRecord();
    ...
    const icon = component.icons[0];
    component.selectIcon(icon.id);
    expect(component.selectedIconId).toBe(icon.id);
  });
  ...
  it('switches view modes and toggles sidebar', () => {
    loadSampleRecord();
    component.setViewMode('icons');
    expect(component.viewMode).toBe('icons');
  ...
  it('removeFile clears when last file removed and updates index otherwise', async () => {
    await component.loadSample();
    const second = createFpFileRecord(new File([sampleBytes()], 'second.mm', { lastModified: 2 }), sampleBytes());
  ...
  it('shows error suggestion after a hard load failure message', () => {
    component.errorMessage = 'bad.mm: Invalid Freeplane map';
  ...
  it('keeps scroll-owner structure for map wrap, cards, table, and sidebar', async () => {
    ...
    expect(root.querySelector('.fp-map-wrap')).toBeTruthy();
    expect(root.querySelector('.fp-workspace')).toBeTruthy();
    expect(root.querySelector('.fp-sidebar')).toBeTruthy();
    component.setViewMode('table');
    fixture.detectChanges();
    expect(root.querySelector('.fp-table')).toBeTruthy();
  });

  it('collapses sidebar via toggle and keeps toolbar present', () => {
    loadSampleRecord();
    expect(fixture.debugElement.query(By.css('.fp-toolbar'))).toBeTruthy();
    component.toggleSidebar();
    fixture.detectChanges();
    expect(component.sidebarCollapsed).toBe(true);
    expect((fixture.nativeElement as HTMLElement).querySelector('.fp-workspace--sidebar-collapsed')).toBeTruthy();
    expect((fixture.nativeElement as HTMLElement).querySelector('.fp-sidebar')).toBeFalsy();
  });
});
