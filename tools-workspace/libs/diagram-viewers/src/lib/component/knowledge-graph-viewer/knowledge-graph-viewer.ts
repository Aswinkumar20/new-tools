...
export class KnowledgeGraphViewerComponent implements AfterViewInit, OnDestroy {
  ...
  onFilterChange(): void {
    const node = this.filteredEntities[0];
    if (node && !this.filteredEntities.some((n) => n.id === this.selectedEntityId)) this.selectedEntityId = node.id;
    const triple = this.filteredLinks[0];
    if (triple && !this.filteredLinks.some((t) => t.id === this.selectedLinkId)) this.selectedLinkId = triple.id;
    this.renderCanvas();
    this.cdr.markForCheck();
  }
