...
import { KnowledgeGraphViewerComponent } from './knowledge-graph-viewer';
import { createKgFileRecord, createSampleKgFile } from '../../utils/knowledge-graph-viewer.utils';

@Component({ selector: 'lib-navigation', standalone: true, template: '' })
class StubNavigationComponent {}

describe('KnowledgeGraphViewerComponent', () => {
  let component: KnowledgeGraphViewerComponent;
  let fixture: ComponentFixture<KnowledgeGraphViewerComponent>;
  ...
  beforeEach(async () => {
    ...
    await TestBed.configureTestingModule({
      ...
      .overrideComponent(KnowledgeGraphViewerComponent, {
        remove: { imports: [Navigation] },
        add: { imports: [StubNavigationComponent] }
      })
      .compileComponents();

    fixture = TestBed.createComponent(KnowledgeGraphViewerComponent);
  ...
  function sampleBytes(): Uint8Array {
    return new TextEncoder().encode(KG_SAMPLE);
  }

  function loadSampleRecord(): void {
    const file = createSampleKgFile();
    const record = createKgFileRecord(file, sampleBytes());
  ...
  it('loads sample via handleFiles and enables export', async () => {
    ...
    expect(component.currentFile?.name).toBe('sample-shop-kg.json');
    expect(component.canExport).toBe(true);
    expect(component.parsed?.entities.length).toBe(4);
    expect(component.parsed?.links.length).toBe(3);
  ...
  it('filters links and repairs selection when filter removes current item', () => {
    loadSampleRecord();
    const first = component.filteredLinks[0];
    expect(first).toBeTruthy();
    component.viewMode = 'table';
    component.selectedLinkId = first.id;
    component.query = 'from:__none__';
    component.onFilterChange();
    expect(component.filteredLinks.length).toBe(0);
    expect(component.selectedLinkId).toBe('');
  });

  it('selects entity and link', () => {
    loadSampleRecord();
    const web = component.parsed!.entities.find((e) => e.name === 'Web');
    expect(web).toBeTruthy();
    component.selectEntity(web!.id);
    expect(component.selectedEntityId).toBe(web!.id);
    const link = component.parsed!.links[0];
    component.selectLink(link.id);
    expect(component.selectedLinkId).toBe(link.id);
  });
  ...
  it('switches view modes and toggles sidebar', () => {
    ...
    component.setViewMode('links');
    expect(component.viewMode).toBe('links');
  ...
  it('removeFile clears when last file removed and updates index otherwise', async () => {
    await component.loadSample();
    const second = createKgFileRecord(new File([sampleBytes()], 'second.json', { lastModified: 2 }), sampleBytes());
  ...
  it('shows error suggestion after a hard load failure message', () => {
    component.errorMessage = 'bad.json: Invalid knowledge graph';
  ...
  it('keeps scroll-owner structure for map wrap, cards, table, and sidebar', async () => {
    ...
    expect(root.querySelector('.kg-map-wrap')).toBeTruthy();
    expect(root.querySelector('.kg-workspace')).toBeTruthy();
    expect(root.querySelector('.kg-sidebar')).toBeTruthy();
    component.setViewMode('table');
    fixture.detectChanges();
    expect(root.querySelector('.kg-table')).toBeTruthy();
  });

  it('collapses sidebar via toggle and keeps toolbar present', () => {
    loadSampleRecord();
    expect(fixture.debugElement.query(By.css('.kg-toolbar'))).toBeTruthy();
    component.toggleSidebar();
    fixture.detectChanges();
    expect(component.sidebarCollapsed).toBe(true);
    expect((fixture.nativeElement as HTMLElement).querySelector('.kg-workspace--sidebar-collapsed')).toBeTruthy();
    expect((fixture.nativeElement as HTMLElement).querySelector('.kg-sidebar')).toBeFalsy();
  });
});
