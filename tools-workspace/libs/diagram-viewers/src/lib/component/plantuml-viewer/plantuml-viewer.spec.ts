...
import { PlantUmlViewerComponent } from './plantuml-viewer';
import { createPumlFileRecord, createSamplePumlFile } from '../../utils/plantuml-viewer.utils';
...
describe('PlantUmlViewerComponent', () => {
  ...
  beforeEach(async () => {
    ...
    await TestBed.configureTestingModule({
      imports: [PlantUmlViewerComponent],
      providers: [
        provideRouter([]),
      ...
      .overrideComponent(PlantUmlViewerComponent, {
  ...
  function sampleBytes(): Uint8Array {
    return new TextEncoder().encode(PUML_CLASS_SAMPLE);
  }

  function loadSampleRecord(): void {
    const file = createSamplePumlFile();
    const record = createPumlFileRecord(file, sampleBytes());
  ...
  it('loads sample via handleFiles and enables export', async () => {
    ...
    expect(component.currentFile?.name).toBe('sample-shop-classes.puml');
    expect(component.canExport).toBe(true);
    expect(component.parsed?.elements.length).toBe(4);
    expect(component.parsed?.relations.length).toBe(3);
    expect(component.parsed?.kind).toBe('uml');
  ...
  it('filters relations and repairs selection when filter removes current item in table mode', () => {
    loadSampleRecord();
  ...
  it('selects element and relation', () => {
    loadSampleRecord();
    const customer = component.parsed!.elements.find((e) => e.name === 'Customer');
    expect(customer).toBeTruthy();
    component.selectElement(customer!.id);
    expect(component.selectedElementId).toBe(customer!.id);
  ...
  it('switches view modes and toggles sidebar', () => {
    loadSampleRecord();
    component.setViewMode('uml');
    expect(component.viewMode).toBe('uml');
    component.setViewMode('c4');
    expect(component.viewMode).toBe('c4');
  ...
  it('removeFile clears when last file removed and updates index otherwise', async () => {
    await component.loadSample();
    const second = createPumlFileRecord(new File([sampleBytes()], 'second.puml', { lastModified: 2 }), sampleBytes());
  ...
  it('shows error suggestion after a hard load failure message', () => {
    component.errorMessage = 'bad.puml: Not a PlantUML diagram';
  ...
  it('keeps scroll-owner structure for map wrap, cards, table, and sidebar', async () => {
    ...
    expect(root.querySelector('.puml-map-wrap')).toBeTruthy();
    expect(root.querySelector('.puml-workspace')).toBeTruthy();
    expect(root.querySelector('.puml-sidebar')).toBeTruthy();
    component.setViewMode('table');
    fixture.detectChanges();
    expect(root.querySelector('.puml-table')).toBeTruthy();
  });

  it('collapses sidebar via toggle and keeps toolbar present', () => {
    loadSampleRecord();
    expect(fixture.debugElement.query(By.css('.puml-toolbar'))).toBeTruthy();
    component.toggleSidebar();
    fixture.detectChanges();
    expect(component.sidebarCollapsed).toBe(true);
    expect((fixture.nativeElement as HTMLElement).querySelector('.puml-workspace--sidebar-collapsed')).toBeTruthy();
    expect((fixture.nativeElement as HTMLElement).querySelector('.puml-sidebar')).toBeFalsy();
  });
});
