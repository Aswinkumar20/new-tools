...
import { DbmlViewerComponent } from './dbml-viewer';
import { createDbmlFileRecord, createSampleDbmlFile } from '../../utils/dbml-viewer.utils';

@Component({ selector: 'lib-navigation', standalone: true, template: '' })
class StubNavigationComponent {}

describe('DbmlViewerComponent', () => {
  let component: DbmlViewerComponent;
  let fixture: ComponentFixture<DbmlViewerComponent>;
  ...
  beforeEach(async () => {
    ...
    await TestBed.configureTestingModule({
      ...
      .overrideComponent(DbmlViewerComponent, {
        remove: { imports: [Navigation] },
        add: { imports: [StubNavigationComponent] }
      })
      .compileComponents();

    fixture = TestBed.createComponent(DbmlViewerComponent);
  ...
  function sampleBytes(): Uint8Array {
    return new TextEncoder().encode(DBML_SAMPLE);
  }

  function loadSampleRecord(): void {
    const file = createSampleDbmlFile();
    const record = createDbmlFileRecord(file, sampleBytes());
  ...
  it('loads sample via handleFiles and enables export', async () => {
    ...
    expect(component.currentFile?.name).toBe('sample-shop.dbml');
  ...
  it('selects table and ref', () => {
    loadSampleRecord();
    const table = component.parsed!.tables[0];
    component.selectTable(table.id);
    expect(component.selectedTableId).toBe(table.id);
    const ref = component.parsed!.refs[0];
    component.selectRef(ref.id);
    expect(component.selectedRefId).toBe(ref.id);
  });
  ...
  it('switches view modes and toggles sidebar', () => {
    ...
    component.setViewMode('refs');
    expect(component.viewMode).toBe('refs');
  ...
  it('removeFile clears when last file removed and updates index otherwise', async () => {
    await component.loadSample();
    const second = createDbmlFileRecord(new File([sampleBytes()], 'second.dbml', { lastModified: 2 }), sampleBytes());
  ...
  it('shows error suggestion after a hard load failure message', () => {
    component.errorMessage = 'bad.dbml: Invalid DBML';
  ...
  it('keeps scroll-owner structure for map wrap, cards, table, and sidebar', async () => {
    ...
    expect(root.querySelector('.dbml-map-wrap')).toBeTruthy();
    expect(root.querySelector('.dbml-workspace')).toBeTruthy();
    expect(root.querySelector('.dbml-sidebar')).toBeTruthy();
    component.setViewMode('table');
    fixture.detectChanges();
    expect(root.querySelector('.dbml-table')).toBeTruthy();
  });

  it('collapses sidebar via toggle and keeps toolbar present', () => {
    loadSampleRecord();
    expect(fixture.debugElement.query(By.css('.dbml-toolbar'))).toBeTruthy();
    component.toggleSidebar();
    fixture.detectChanges();
    expect(component.sidebarCollapsed).toBe(true);
    expect((fixture.nativeElement as HTMLElement).querySelector('.dbml-workspace--sidebar-collapsed')).toBeTruthy();
    expect((fixture.nativeElement as HTMLElement).querySelector('.dbml-sidebar')).toBeFalsy();
  });
});
