...
import { C4ModelViewerComponent } from './c4-model-viewer';
import { createC4FileRecord, createSampleC4File } from '../../utils/c4-model-viewer.utils';

@Component({ selector: 'lib-navigation', standalone: true, template: '' })
class StubNavigationComponent {}

describe('C4ModelViewerComponent', () => {
  let component: C4ModelViewerComponent;
  let fixture: ComponentFixture<C4ModelViewerComponent>;
  ...
  beforeEach(async () => {
    ...
    await TestBed.configureTestingModule({
      imports: [C4ModelViewerComponent],
      providers: [
        provideRouter([]),
      ...
      .overrideComponent(C4ModelViewerComponent, {
        remove: { imports: [Navigation] },
        add: { imports: [StubNavigationComponent] }
      })
      .compileComponents();

    fixture = TestBed.createComponent(C4ModelViewerComponent);
  ...
  function sampleBytes(): Uint8Array {
    return new TextEncoder().encode(C4_PUML_SAMPLE);
  }

  function loadSampleRecord(): void {
    const file = createSampleC4File();
    const record = createC4FileRecord(file, sampleBytes());
  ...
  it('loads sample via handleFiles and enables export', async () => {
    ...
    expect(component.currentFile?.name).toBe('sample-shop-c4.puml');
  ...
  it('switches view modes and toggles sidebar', () => {
    ...
    component.setViewMode('context');
    expect(component.viewMode).toBe('context');
  ...
  it('removeFile clears when last file removed and updates index otherwise', async () => {
    await component.loadSample();
    const second = createC4FileRecord(new File([sampleBytes()], 'second.puml', { lastModified: 2 }), sampleBytes());
  ...
  it('shows error suggestion after a hard load failure message', () => {
    component.errorMessage = 'bad.puml: Invalid C4 model';
  ...
  it('keeps scroll-owner structure for map wrap, cards, table, and sidebar', async () => {
    ...
    expect(root.querySelector('.c4-map-wrap')).toBeTruthy();
    expect(root.querySelector('.c4-workspace')).toBeTruthy();
    expect(root.querySelector('.c4-sidebar')).toBeTruthy();
    component.setViewMode('table');
    fixture.detectChanges();
    expect(root.querySelector('.c4-table')).toBeTruthy();
  });

  it('collapses sidebar via toggle and keeps toolbar present', () => {
    loadSampleRecord();
    expect(fixture.debugElement.query(By.css('.c4-toolbar'))).toBeTruthy();
    component.toggleSidebar();
    fixture.detectChanges();
    expect(component.sidebarCollapsed).toBe(true);
    expect((fixture.nativeElement as HTMLElement).querySelector('.c4-workspace--sidebar-collapsed')).toBeTruthy();
    expect((fixture.nativeElement as HTMLElement).querySelector('.c4-sidebar')).toBeFalsy();
  });
});
