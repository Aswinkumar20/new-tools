...
import { DependencyGraphViewerComponent } from './dependency-graph-viewer';
import { createDepFileRecord, createSampleDepFile } from '../../utils/dependency-graph-viewer.utils';

@Component({ selector: 'lib-navigation', standalone: true, template: '' })
class StubNavigationComponent {}

describe('DependencyGraphViewerComponent', () => {
  let component: DependencyGraphViewerComponent;
  let fixture: ComponentFixture<DependencyGraphViewerComponent>;
  ...
  beforeEach(async () => {
    ...
    await TestBed.configureTestingModule({
      imports: [DependencyGraphViewerComponent],
      providers: [
        provideRouter([]),
      ...
      .overrideComponent(DependencyGraphViewerComponent, {
        remove: { imports: [Navigation] },
        add: { imports: [StubNavigationComponent] }
      })
      .compileComponents();

    fixture = TestBed.createComponent(DependencyGraphViewerComponent);
  ...
  function sampleBytes(): Uint8Array {
    return new TextEncoder().encode(DEP_SAMPLE);
  }

  function loadSampleRecord(): void {
    const file = createSampleDepFile();
    const record = createDepFileRecord(file, sampleBytes());
  ...
  it('loads sample via handleFiles and enables export', async () => {
    ...
    expect(component.currentFile?.name).toBe('sample-shop-lock.json');
    expect(component.canExport).toBe(true);
    expect(component.parsed?.packages.length).toBeGreaterThan(0);
  ...
  it('filters packages and repairs selection when filter removes current item', () => {
    loadSampleRecord();
    const first = component.filteredPackages[0];
    expect(first).toBeTruthy();
    component.selectedPackageId = first.id;
    component.query = 'pkg:__none__';
    component.onFilterChange();
    expect(component.filteredPackages.length).toBe(0);
    expect(component.selectedPackageId).toBe('');
  });

  it('selects package, cycle, and edge', () => {
    loadSampleRecord();
    const pkg = component.parsed!.packages[0];
    component.selectPackage(pkg.id);
    expect(component.selectedPackageId).toBe(pkg.id);
    const cycle = component.parsed!.cycles[0];
    component.selectCycle(cycle.id);
    expect(component.selectedCycleId).toBe(cycle.id);
    const edge = component.parsed!.edges[0];
    component.selectEdge(edge.id);
    expect(component.selectedEdgeId).toBe(edge.id);
  });

  it('detects cyclic packages', () => {
    loadSampleRecord();
    expect(component.cyclicIds.size).toBeGreaterThan(0);
    const cyclicId = [...component.cyclicIds][0];
    expect(component.isCyclicPackage(cyclicId)).toBe(true);
  });
  ...
  it('switches view modes and toggles sidebar', () => {
    ...
    component.setViewMode('cycles');
    expect(component.viewMode).toBe('cycles');
  ...
  it('removeFile clears when last file removed and updates index otherwise', async () => {
    await component.loadSample();
    const second = createDepFileRecord(new File([sampleBytes()], 'second.json', { lastModified: 2 }), sampleBytes());
  ...
  it('shows error suggestion after a hard load failure message', () => {
    component.errorMessage = 'bad.json: Invalid dependency graph';
  ...
  it('keeps scroll-owner structure for map wrap, cards, table, and sidebar', async () => {
    ...
    expect(root.querySelector('.dep-map-wrap')).toBeTruthy();
    expect(root.querySelector('.dep-workspace')).toBeTruthy();
    expect(root.querySelector('.dep-sidebar')).toBeTruthy();
    component.setViewMode('table');
    fixture.detectChanges();
    expect(root.querySelector('.dep-table')).toBeTruthy();
  });

  it('collapses sidebar via toggle and keeps toolbar present', () => {
    loadSampleRecord();
    expect(fixture.debugElement.query(By.css('.dep-toolbar'))).toBeTruthy();
    component.toggleSidebar();
    fixture.detectChanges();
    expect(component.sidebarCollapsed).toBe(true);
    expect((fixture.nativeElement as HTMLElement).querySelector('.dep-workspace--sidebar-collapsed')).toBeTruthy();
    expect((fixture.nativeElement as HTMLElement).querySelector('.dep-sidebar')).toBeFalsy();
  });
});
