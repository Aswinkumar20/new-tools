...
import { ClassDiagramViewerComponent } from './class-diagram-viewer';
import { createCdgFileRecord, createSampleCdgFile } from '../../utils/class-diagram-viewer.utils';

@Component({ selector: 'lib-navigation', standalone: true, template: '' })
class StubNavigationComponent {}

describe('ClassDiagramViewerComponent', () => {
  let component: ClassDiagramViewerComponent;
  let fixture: ComponentFixture<ClassDiagramViewerComponent>;
  ...
  beforeEach(async () => {
    ...
    await TestBed.configureTestingModule({
      imports: [ClassDiagramViewerComponent],
      providers: [
        provideRouter([]),
      ...
      .overrideComponent(ClassDiagramViewerComponent, {
        remove: { imports: [Navigation] },
        add: { imports: [StubNavigationComponent] }
      })
      .compileComponents();

    fixture = TestBed.createComponent(ClassDiagramViewerComponent);
  ...
  function sampleBytes(): Uint8Array {
    return new TextEncoder().encode(CDG_PUML_SAMPLE);
  }

  function loadSampleRecord(): void {
    const file = createSampleCdgFile();
    const record = createCdgFileRecord(file, sampleBytes());
  ...
  it('loads sample via handleFiles and enables export', async () => {
    ...
    expect(component.currentFile?.name).toBe('sample-catalog-types.puml');
    expect(component.canExport).toBe(true);
    expect(component.parsed?.types.length).toBeGreaterThan(0);
  ...
  it('filters types and repairs selection when filter removes current item', () => {
    loadSampleRecord();
    const first = component.filteredTypes[0];
    expect(first).toBeTruthy();
    component.selectedTypeId = first.id;
  ...
  it('selects type and relation', () => {
    loadSampleRecord();
    const type = component.parsed!.types[0];
    component.selectType(type.id);
    expect(component.selectedTypeId).toBe(type.id);
  ...
  it('switches view modes and toggles sidebar', () => {
    ...
    component.setViewMode('relations');
    expect(component.viewMode).toBe('relations');
  ...
  it('removeFile clears when last file removed and updates index otherwise', async () => {
    await component.loadSample();
    const second = createCdgFileRecord(new File([sampleBytes()], 'second.puml', { lastModified: 2 }), sampleBytes());
  ...
  it('shows error suggestion after a hard load failure message', () => {
    component.errorMessage = 'bad.puml: Invalid class diagram';
  ...
  it('keeps scroll-owner structure for map wrap, cards, table, and sidebar', async () => {
    ...
    expect(root.querySelector('.cdg-map-wrap')).toBeTruthy();
    expect(root.querySelector('.cdg-workspace')).toBeTruthy();
    expect(root.querySelector('.cdg-sidebar')).toBeTruthy();
    component.setViewMode('table');
    fixture.detectChanges();
    expect(root.querySelector('.cdg-table')).toBeTruthy();
  });

  it('collapses sidebar via toggle and keeps toolbar present', () => {
    loadSampleRecord();
    expect(fixture.debugElement.query(By.css('.cdg-toolbar'))).toBeTruthy();
    component.toggleSidebar();
    fixture.detectChanges();
    expect(component.sidebarCollapsed).toBe(true);
    expect((fixture.nativeElement as HTMLElement).querySelector('.cdg-workspace--sidebar-collapsed')).toBeTruthy();
    expect((fixture.nativeElement as HTMLElement).querySelector('.cdg-sidebar')).toBeFalsy();
  });
});
