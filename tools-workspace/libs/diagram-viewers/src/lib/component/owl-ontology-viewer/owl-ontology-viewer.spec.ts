...
import { OwlOntologyViewerComponent } from './owl-ontology-viewer';
import { createOwlFileRecord, createSampleOwlFile } from '../../utils/owl-ontology-viewer.utils';

@Component({ selector: 'lib-navigation', standalone: true, template: '' })
class StubNavigationComponent {}

describe('OwlOntologyViewerComponent', () => {
  let component: OwlOntologyViewerComponent;
  let fixture: ComponentFixture<OwlOntologyViewerComponent>;
  ...
  beforeEach(async () => {
    ...
    await TestBed.configureTestingModule({
      imports: [OwlOntologyViewerComponent],
      providers: [
        provideRouter([]),
      ...
      .overrideComponent(OwlOntologyViewerComponent, {
        remove: { imports: [Navigation] },
        add: { imports: [StubNavigationComponent] }
      })
      .compileComponents();

    fixture = TestBed.createComponent(OwlOntologyViewerComponent);
  ...
  function sampleBytes(): Uint8Array {
    return new TextEncoder().encode(OWL_SAMPLE);
  }

  function loadSampleRecord(): void {
    const file = createSampleOwlFile();
    const record = createOwlFileRecord(file, sampleBytes());
  ...
  it('loads sample via handleFiles and enables export', async () => {
    ...
    expect(component.currentFile?.name).toBe('sample-shop.owl');
    expect(component.canExport).toBe(true);
    expect(component.parsed?.classes.length).toBe(3);
    expect(component.parsed?.properties.length).toBe(2);
    expect(component.parsed?.axioms.length).toBeGreaterThanOrEqual(2);
  ...
  it('filters axioms and repairs selection when filter removes current item in table mode', () => {
    loadSampleRecord();
    const first = component.filteredAxioms[0];
    expect(first).toBeTruthy();
    component.viewMode = 'table';
    component.selectedAxiomId = first.id;
    component.query = 'from:__none__';
    component.onFilterChange();
    expect(component.filteredAxioms.length).toBe(0);
    expect(component.selectedAxiomId).toBe('');
  });

  it('selects class, property, and axiom', () => {
    loadSampleRecord();
    const book = component.parsed!.classes.find((c) => c.name === 'Book');
    expect(book).toBeTruthy();
    component.selectClass(book!.id);
    expect(component.selectedClassId).toBe(book!.id);
    const buys = component.parsed!.properties.find((p) => p.name === 'buys');
    expect(buys).toBeTruthy();
    component.selectProperty(buys!.id);
    expect(component.selectedPropertyId).toBe(buys!.id);
    const axiom = component.parsed!.axioms[0];
    component.selectAxiom(axiom.id);
    expect(component.selectedAxiomId).toBe(axiom.id);
  });

  it('switches view modes and toggles sidebar', () => {
    loadSampleRecord();
    component.setViewMode('classes');
    expect(component.viewMode).toBe('classes');
  ...
  it('removeFile clears when last file removed and updates index otherwise', async () => {
    await component.loadSample();
    const second = createOwlFileRecord(new File([sampleBytes()], 'second.owl', { lastModified: 2 }), sampleBytes());
  ...
  it('shows error suggestion after a hard load failure message', () => {
    component.errorMessage = 'bad.owl: Not an OWL ontology';
  ...
  it('keeps scroll-owner structure for map wrap, cards, table, and sidebar', async () => {
    ...
    expect(root.querySelector('.owl-map-wrap')).toBeTruthy();
    expect(root.querySelector('.owl-workspace')).toBeTruthy();
    expect(root.querySelector('.owl-sidebar')).toBeTruthy();
    component.setViewMode('table');
    fixture.detectChanges();
    expect(root.querySelector('.owl-table')).toBeTruthy();
  });

  it('collapses sidebar via toggle and keeps toolbar present', () => {
    loadSampleRecord();
    expect(fixture.debugElement.query(By.css('.owl-toolbar'))).toBeTruthy();
    component.toggleSidebar();
    fixture.detectChanges();
    expect(component.sidebarCollapsed).toBe(true);
    expect((fixture.nativeElement as HTMLElement).querySelector('.owl-workspace--sidebar-collapsed')).toBeTruthy();
    expect((fixture.nativeElement as HTMLElement).querySelector('.owl-sidebar')).toBeFalsy();
  });
});
