...
import { ErDiagramViewerComponent } from './er-diagram-viewer';
import { createErFileRecord, createSampleErFile } from '../../utils/er-diagram-viewer.utils';

@Component({ selector: 'lib-navigation', standalone: true, template: '' })
class StubNavigationComponent {}

describe('ErDiagramViewerComponent', () => {
  let component: ErDiagramViewerComponent;
  let fixture: ComponentFixture<ErDiagramViewerComponent>;
  ...
  beforeEach(async () => {
    ...
    await TestBed.configureTestingModule({
      imports: [ErDiagramViewerComponent],
      providers: [
        provideRouter([]),
      ...
      .overrideComponent(ErDiagramViewerComponent, {
        remove: { imports: [Navigation] },
        add: { imports: [StubNavigationComponent] }
      })
      .compileComponents();

    fixture = TestBed.createComponent(ErDiagramViewerComponent);
  ...
  function sampleBytes(): Uint8Array {
    return new TextEncoder().encode(ER_PUML_SAMPLE);
  }

  function loadSampleRecord(): void {
    const file = createSampleErFile();
    const record = createErFileRecord(file, sampleBytes());
  ...
  it('loads sample via handleFiles and enables export', async () => {
    ...
    expect(component.currentFile?.name).toBe('sample-shop-er.puml');
    expect(component.canExport).toBe(true);
    expect(component.parsed?.entities.length).toBeGreaterThan(0);
  ...
  it('filters entities and repairs selection when filter removes current item', () => {
    ...
    component.query = 'entity:__none__';
    component.onFilterChange();
    expect(component.filteredEntities.length).toBe(0);
    expect(component.selectedEntityId).toBe('');
  });

  it('selects entity, key, and relation', () => {
    loadSampleRecord();
    const entity = component.parsed!.entities[0];
    component.selectEntity(entity.id);
    expect(component.selectedEntityId).toBe(entity.id);
    const key = component.parsed!.keys[0];
    component.selectKey(key.id);
    expect(component.selectedKeyId).toBe(key.id);
    expect(component.selectedEntityId).toBe(key.entityId);
    const rel = component.parsed!.relations[0];
    component.selectRelation(rel.id);
    expect(component.selectedRelationId).toBe(rel.id);
  });

  it('columnBadge returns PK, FK, UK, or empty', () => {
    expect(component.columnBadge({ pk: true, fk: false, unique: false })).toBe('PK');
    expect(component.columnBadge({ pk: false, fk: true, unique: false })).toBe('FK');
    expect(component.columnBadge({ pk: false, fk: false, unique: true })).toBe('UK');
    expect(component.columnBadge({ pk: false, fk: false, unique: false })).toBe('');
  });
  ...
  it('switches view modes and toggles sidebar', () => {
    ...
    component.setViewMode('keys');
    expect(component.viewMode).toBe('keys');
  ...
  it('removeFile clears when last file removed and updates index otherwise', async () => {
    await component.loadSample();
    const second = createErFileRecord(new File([sampleBytes()], 'second.puml', { lastModified: 2 }), sampleBytes());
  ...
  it('shows error suggestion after a hard load failure message', () => {
    component.errorMessage = 'bad.puml: Invalid ER diagram';
  ...
  it('keeps scroll-owner structure for map wrap, cards, table, and sidebar', async () => {
    ...
    expect(root.querySelector('.erd-map-wrap')).toBeTruthy();
    expect(root.querySelector('.erd-workspace')).toBeTruthy();
    expect(root.querySelector('.erd-sidebar')).toBeTruthy();
    component.setViewMode('table');
    fixture.detectChanges();
    expect(root.querySelector('.erd-table')).toBeTruthy();
  });

  it('collapses sidebar via toggle and keeps toolbar present', () => {
    loadSampleRecord();
    expect(fixture.debugElement.query(By.css('.erd-toolbar'))).toBeTruthy();
    component.toggleSidebar();
    fixture.detectChanges();
    expect(component.sidebarCollapsed).toBe(true);
    expect((fixture.nativeElement as HTMLElement).querySelector('.erd-workspace--sidebar-collapsed')).toBeTruthy();
    expect((fixture.nativeElement as HTMLElement).querySelector('.erd-sidebar')).toBeFalsy();
  });
});
