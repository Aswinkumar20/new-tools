...
import { GexfViewerComponent } from './gexf-viewer';
import { createGxfFileRecord, createSampleGxfFile } from '../../utils/gexf-viewer.utils';

@Component({ selector: 'lib-navigation', standalone: true, template: '' })
class StubNavigationComponent {}

describe('GexfViewerComponent', () => {
  let component: GexfViewerComponent;
  let fixture: ComponentFixture<GexfViewerComponent>;
  ...
  beforeEach(async () => {
    ...
    await TestBed.configureTestingModule({
      ...
      .overrideComponent(GexfViewerComponent, {
        remove: { imports: [Navigation] },
        add: { imports: [StubNavigationComponent] }
      })
      .compileComponents();

    fixture = TestBed.createComponent(GexfViewerComponent);
  ...
  function sampleBytes(): Uint8Array {
    return new TextEncoder().encode(GXF_GEXF_SAMPLE);
  }

  function loadSampleRecord(): void {
    const file = createSampleGxfFile();
    const record = createGxfFileRecord(file, sampleBytes());
  ...
  it('loads sample via handleFiles and enables export', async () => {
    ...
    expect(component.currentFile?.name).toBe('sample-shop.gexf');
  ...
  it('timeline setTime filters nodes at later ticks', () => {
    loadSampleRecord();
    component.viewMode = 'timeline';
    component.setTime(0);
    expect(component.filteredNodes.some((n) => n.label === 'Cart')).toBe(true);
    component.setTime(9);
    expect(component.filteredNodes.some((n) => n.label === 'Cart')).toBe(false);
  });
  ...
  it('removeFile clears when last file removed and updates index otherwise', async () => {
    await component.loadSample();
    const second = createGxfFileRecord(new File([sampleBytes()], 'second.gexf', { lastModified: 2 }), sampleBytes());
  ...
  it('shows error suggestion after a hard load failure message', () => {
    component.errorMessage = 'bad.gexf: Invalid GEXF network';
  ...
  it('keeps scroll-owner structure for map wrap, cards, table, and sidebar', async () => {
    ...
    expect(root.querySelector('.gxf-map-wrap')).toBeTruthy();
    expect(root.querySelector('.gxf-workspace')).toBeTruthy();
    expect(root.querySelector('.gxf-sidebar')).toBeTruthy();
    component.setViewMode('table');
    fixture.detectChanges();
    expect(root.querySelector('.gxf-table')).toBeTruthy();
  });

  it('collapses sidebar via toggle and keeps toolbar present', () => {
    loadSampleRecord();
    expect(fixture.debugElement.query(By.css('.gxf-toolbar'))).toBeTruthy();
    component.toggleSidebar();
    fixture.detectChanges();
    expect(component.sidebarCollapsed).toBe(true);
    expect((fixture.nativeElement as HTMLElement).querySelector('.gxf-workspace--sidebar-collapsed')).toBeTruthy();
    expect((fixture.nativeElement as HTMLElement).querySelector('.gxf-sidebar')).toBeFalsy();
  });
});
