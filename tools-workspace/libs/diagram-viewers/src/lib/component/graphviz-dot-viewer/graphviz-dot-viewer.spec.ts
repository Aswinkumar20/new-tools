...
import { GraphvizDotViewerComponent } from './graphviz-dot-viewer';
import { createGvzFileRecord, createSampleGvzFile } from '../../utils/graphviz-dot-viewer.utils';

@Component({ selector: 'lib-navigation', standalone: true, template: '' })
class StubNavigationComponent {}

describe('GraphvizDotViewerComponent', () => {
  let component: GraphvizDotViewerComponent;
  let fixture: ComponentFixture<GraphvizDotViewerComponent>;
  ...
  beforeEach(async () => {
    ...
    await TestBed.configureTestingModule({
      imports: [GraphvizDotViewerComponent],
      providers: [
        provideRouter([]),
      ...
      .overrideComponent(GraphvizDotViewerComponent, {
        remove: { imports: [Navigation] },
        add: { imports: [StubNavigationComponent] }
      })
      .compileComponents();

    fixture = TestBed.createComponent(GraphvizDotViewerComponent);
  ...
  function sampleBytes(): Uint8Array {
    return new TextEncoder().encode(GVZ_DOT_SAMPLE);
  }

  function loadSampleRecord(): void {
    const file = createSampleGvzFile();
    const record = createGvzFileRecord(file, sampleBytes());
  ...
  it('loads sample via handleFiles and enables export', async () => {
    ...
    expect(component.currentFile?.name).toBe('sample-checkout.dot');
    expect(component.canExport).toBe(true);
    expect(component.parsed?.nodes.length).toBe(5);
    expect(component.parsed?.edges.length).toBe(5);
    expect(component.parsed?.layout).toBe('dot');
  ...
  it('filters edges and repairs selection when filter removes current item', () => {
    loadSampleRecord();
    const first = component.filteredEdges[0];
    expect(first).toBeTruthy();
    component.viewMode = 'table';
    component.selectedEdgeId = first.id;
    component.query = 'from:__none__';
    component.onFilterChange();
    expect(component.filteredEdges.length).toBe(0);
    expect(component.selectedEdgeId).toBe('');
  });

  it('selects node and edge', () => {
    loadSampleRecord();
    const cart = component.parsed!.nodes.find((n) => n.name === 'Cart');
    expect(cart).toBeTruthy();
    component.selectNode(cart!.id);
    expect(component.selectedNodeId).toBe(cart!.id);
    const edge = component.parsed!.edges[0];
    component.selectEdge(edge.id);
    expect(component.selectedEdgeId).toBe(edge.id);
  });

  it('setLayout relayouts and is a no-op when unchanged', () => {
    loadSampleRecord();
    const before = component.parsed!.nodes.map((n) => n.x);
    component.setLayout('circo');
    expect(component.selectedLayout).toBe('circo');
    const after = component.parsed!.nodes.map((n) => n.x);
  ...
  it('switches view modes and toggles sidebar', () => {
    ...
    component.setViewMode('nodes');
    expect(component.viewMode).toBe('nodes');
  ...
  it('setViewMode is a no-op when mode unchanged', () => {
    loadSampleRecord();
    component.viewMode = 'layout';
    component.setViewMode('layout');
    expect(component.viewMode).toBe('layout');
  });
  ...
  it('removeFile clears when last file removed and updates index otherwise', async () => {
    await component.loadSample();
    const second = createGvzFileRecord(new File([sampleBytes()], 'second.dot', { lastModified: 2 }), sampleBytes());
  ...
  it('shows error suggestion after a hard load failure message', () => {
    component.errorMessage = 'bad.dot: Invalid DOT graph';
  ...
  it('keeps scroll-owner structure for map wrap, cards, table, and sidebar', async () => {
    ...
    expect(root.querySelector('.gvz-map-wrap')).toBeTruthy();
    expect(root.querySelector('.gvz-workspace')).toBeTruthy();
    expect(root.querySelector('.gvz-sidebar')).toBeTruthy();
    component.setViewMode('table');
    fixture.detectChanges();
    expect(root.querySelector('.gvz-table')).toBeTruthy();
  });

  it('collapses sidebar via toggle and keeps toolbar present', () => {
    loadSampleRecord();
    expect(fixture.debugElement.query(By.css('.gvz-toolbar'))).toBeTruthy();
    component.toggleSidebar();
    fixture.detectChanges();
    expect(component.sidebarCollapsed).toBe(true);
    expect((fixture.nativeElement as HTMLElement).querySelector('.gvz-workspace--sidebar-collapsed')).toBeTruthy();
    expect((fixture.nativeElement as HTMLElement).querySelector('.gvz-sidebar')).toBeFalsy();
  });
});
