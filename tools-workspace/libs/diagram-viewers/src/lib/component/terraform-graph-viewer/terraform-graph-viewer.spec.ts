...
import { TerraformGraphViewerComponent } from './terraform-graph-viewer';
import { createSampleTfFile, createTfFileRecord } from '../../utils/terraform-graph-viewer.utils';

@Component({ selector: 'lib-navigation', standalone: true, template: '' })
class StubNavigationComponent {}

describe('TerraformGraphViewerComponent', () => {
  let component: TerraformGraphViewerComponent;
  let fixture: ComponentFixture<TerraformGraphViewerComponent>;
  ...
  beforeEach(async () => {
    ...
    await TestBed.configureTestingModule({
      imports: [TerraformGraphViewerComponent],
      providers: [
        provideRouter([]),
      ...
      .overrideComponent(TerraformGraphViewerComponent, {
        remove: { imports: [Navigation] },
        add: { imports: [StubNavigationComponent] }
      })
      .compileComponents();

    fixture = TestBed.createComponent(TerraformGraphViewerComponent);
  ...
  function sampleBytes(): Uint8Array {
    return new TextEncoder().encode(TF_SAMPLE);
  }

  function loadSampleRecord(): void {
    const file = createSampleTfFile();
    const record = createTfFileRecord(file, sampleBytes());
  ...
  it('loads sample via handleFiles and enables export', async () => {
    ...
    expect(component.currentFile?.name).toBe('sample-shop.dot');
    expect(component.canExport).toBe(true);
    expect(component.parsed?.resources.length).toBe(4);
    expect(component.parsed?.edges.length).toBe(3);
  ...
  it('filters resources and repairs selection when filter removes current item', () => {
    loadSampleRecord();
    const first = component.filteredResources[0];
    expect(first).toBeTruthy();
    component.selectedResourceId = first.id;
    component.query = 'resource:__none__';
    component.onFilterChange();
    expect(component.filteredResources.length).toBe(0);
    expect(component.selectedResourceId).toBe('');
  });

  it('filters edges and repairs selection in table mode', () => {
    loadSampleRecord();
  ...
  it('selects resource and edge', () => {
    loadSampleRecord();
    const resource = component.parsed!.resources.find((r) => r.id === 'aws_instance.web')!;
    component.selectResource(resource.id);
    expect(component.selectedResourceId).toBe(resource.id);
  ...
  it('switches view modes and toggles sidebar', () => {
    ...
    component.setViewMode('edges');
    expect(component.viewMode).toBe('edges');
  ...
  it('removeFile clears when last file removed and updates index otherwise', async () => {
    await component.loadSample();
    const second = createTfFileRecord(new File([sampleBytes()], 'second.dot', { lastModified: 2 }), sampleBytes());
  ...
  it('shows error suggestion after a hard load failure message', () => {
    component.errorMessage = 'bad.dot: Invalid Terraform graph';
  ...
  it('keeps scroll-owner structure for map wrap, cards, table, and sidebar', async () => {
    ...
    expect(root.querySelector('.tf-map-wrap')).toBeTruthy();
    expect(root.querySelector('.tf-workspace')).toBeTruthy();
    expect(root.querySelector('.tf-sidebar')).toBeTruthy();
    component.setViewMode('table');
    fixture.detectChanges();
    expect(root.querySelector('.tf-table')).toBeTruthy();
  });

  it('collapses sidebar via toggle and keeps toolbar present', () => {
    loadSampleRecord();
    expect(fixture.debugElement.query(By.css('.tf-toolbar'))).toBeTruthy();
    component.toggleSidebar();
    fixture.detectChanges();
    expect(component.sidebarCollapsed).toBe(true);
    expect((fixture.nativeElement as HTMLElement).querySelector('.tf-workspace--sidebar-collapsed')).toBeTruthy();
    expect((fixture.nativeElement as HTMLElement).querySelector('.tf-sidebar')).toBeFalsy();
  });
});
