...
import { MermaidDiagramViewerComponent } from './mermaid-diagram-viewer';
import { createMmdFileRecord, createSampleMmdFile } from '../../utils/mermaid-diagram-viewer.utils';

@Component({ selector: 'lib-navigation', standalone: true, template: '' })
class StubNavigationComponent {}

describe('MermaidDiagramViewerComponent', () => {
  let component: MermaidDiagramViewerComponent;
  let fixture: ComponentFixture<MermaidDiagramViewerComponent>;
  ...
  beforeEach(async () => {
    ...
    await TestBed.configureTestingModule({
      imports: [MermaidDiagramViewerComponent],
      providers: [
        provideRouter([]),
      ...
      .overrideComponent(MermaidDiagramViewerComponent, {
        remove: { imports: [Navigation] },
        add: { imports: [StubNavigationComponent] }
      })
      .compileComponents();

    fixture = TestBed.createComponent(MermaidDiagramViewerComponent);
  ...
  function sampleBytes(): Uint8Array {
    return new TextEncoder().encode(MMD_FLOWCHART_SAMPLE);
  }

  function loadSampleRecord(): void {
    const file = createSampleMmdFile();
    const record = createMmdFileRecord(file, sampleBytes());
  ...
  it('loads sample via handleFiles and enables export', async () => {
    ...
    expect(component.currentFile?.name).toBe('sample-checkout-flow.mmd');
    expect(component.canExport).toBe(true);
    expect(component.parsed?.nodes.length).toBe(5);
    expect(component.parsed?.edges.length).toBe(5);
    expect(component.parsed?.kind).toBe('flowchart');
  ...
  it('filters edges and repairs selection when filter removes current item in table mode', () => {
    loadSampleRecord();
  ...
  it('selects node and edge', () => {
    loadSampleRecord();
    const cart = component.parsed!.nodes.find((n) => /cart/i.test(n.name));
  ...
  it('removeFile clears when last file removed and updates index otherwise', async () => {
    await component.loadSample();
    const second = createMmdFileRecord(new File([sampleBytes()], 'second.mmd', { lastModified: 2 }), sampleBytes());
  ...
  it('shows error suggestion after a hard load failure message', () => {
    component.errorMessage = 'bad.mmd: Not a Mermaid diagram';
  ...
  it('keeps scroll-owner structure for map wrap, cards, table, and sidebar', async () => {
    ...
    expect(root.querySelector('.mmd-map-wrap')).toBeTruthy();
    expect(root.querySelector('.mmd-workspace')).toBeTruthy();
    expect(root.querySelector('.mmd-sidebar')).toBeTruthy();
    component.setViewMode('table');
    fixture.detectChanges();
    expect(root.querySelector('.mmd-table')).toBeTruthy();
  });

  it('collapses sidebar via toggle and keeps toolbar present', () => {
    loadSampleRecord();
    expect(fixture.debugElement.query(By.css('.mmd-toolbar'))).toBeTruthy();
    component.toggleSidebar();
    fixture.detectChanges();
    expect(component.sidebarCollapsed).toBe(true);
    expect((fixture.nativeElement as HTMLElement).querySelector('.mmd-workspace--sidebar-collapsed')).toBeTruthy();
    expect((fixture.nativeElement as HTMLElement).querySelector('.mmd-sidebar')).toBeFalsy();
  });
});
