...
export class DroolsRuleViewerComponent implements AfterViewInit, OnDestroy {
    ...
    try {
      ...
      if (this.currentFile) {
        ...
        if (this.currentFile.softFail) {
          this.toast.warning('Parsed with little or no rules — metadata may still be available');
  ...
  onFilterChange(): void {
    ...
    if (this.selectedConditionId && !this.filteredConditions.some((c) => c.id === this.selectedConditionId)) {
      this.selectedConditionId = this.filteredConditions[0]?.id ?? '';
    }
    this.renderCanvas();
    this.cdr.markForCheck();
  }
