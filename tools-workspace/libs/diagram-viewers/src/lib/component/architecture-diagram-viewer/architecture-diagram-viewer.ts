...
export class ArchitectureDiagramViewerComponent implements AfterViewInit, OnDestroy {
    ...
    try {
      ...
      if (this.currentFile) {
        ...
        if (this.currentFile.softFail) {
          this.toast.warning('Parsed with little or no boxes — metadata may still be available');
  ...
  onFilterChange(): void {
    if (this.selectedBoxId && !this.filteredBoxes.some((b) => b.id === this.selectedBoxId)) {
      this.selectedBoxId = this.filteredBoxes[0]?.id ?? '';
    }
