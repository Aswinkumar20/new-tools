...
import { ArchitectureDiagramViewerComponent } from './architecture-diagram-viewer';
import { createArchFileRecord, createSampleArchFile } from '../../utils/architecture-diagram-viewer.utils';

@Component({ selector: 'lib-navigation', standalone: true, template: '' })
class StubNavigationComponent {}

describe('ArchitectureDiagramViewerComponent', () => {
  let component: ArchitectureDiagramViewerComponent;
  let fixture: ComponentFixture<ArchitectureDiagramViewerComponent>;
  ...
  beforeEach(async () => {
    ...
    await TestBed.configureTestingModule({
      imports: [ArchitectureDiagramViewerComponent],
      providers: [
        provideRouter([]),
      ...
      .overrideComponent(ArchitectureDiagramViewerComponent, {
        remove: { imports: [Navigation] },
        add: { imports: [StubNavigationComponent] }
      })
      .compileComponents();

    fixture = TestBed.createComponent(ArchitectureDiagramViewerComponent);
  ...
  function sampleBytes(): Uint8Array {
    return new TextEncoder().encode(ARCH_PUML_SAMPLE);
  }

  function loadSampleRecord(): void {
    const file = createSampleArchFile();
    const record = createArchFileRecord(file, sampleBytes());
  ...
  it('loads sample via handleFiles and enables export', async () => {
    ...
    expect(component.currentFile?.name).toBe('sample-shop-arch.puml');
    expect(component.canExport).toBe(true);
    expect(component.parsed?.boxes.length).toBeGreaterThan(0);
  ...
  it('filters boxes and repairs selection when filter removes current item', () => {
    loadSampleRecord();
    const first = component.filteredBoxes[0];
    expect(first).toBeTruthy();
    component.selectedBoxId = first.id;
    component.query = 'kind:__none__';
    component.onFilterChange();
    expect(component.filteredBoxes.length).toBe(0);
    expect(component.selectedBoxId).toBe('');
  });

  it('selects box and connector', () => {
    loadSampleRecord();
    const box = component.parsed!.boxes[0];
    component.selectBox(box.id);
    expect(component.selectedBoxId).toBe(box.id);
    const conn = component.parsed!.connectors[0];
    component.selectConnector(conn.id);
    expect(component.selectedConnectorId).toBe(conn.id);
  });
  ...
  it('switches view modes and toggles sidebar', () => {
    ...
    component.setViewMode('boxes');
    expect(component.viewMode).toBe('boxes');
  ...
  it('removeFile clears when last file removed and updates index otherwise', async () => {
    await component.loadSample();
    const second = createArchFileRecord(
      new File([sampleBytes()], 'second.puml', { lastModified: 2 }),
      sampleBytes()
    );
  ...
  it('shows error suggestion after a hard load failure message', () => {
    component.errorMessage = 'bad.puml: Invalid architecture diagram';
  ...
  it('keeps scroll-owner structure for map wrap, cards, table, and sidebar', async () => {
    ...
    expect(root.querySelector('.arch-map-wrap')).toBeTruthy();
    expect(root.querySelector('.arch-workspace')).toBeTruthy();
    expect(root.querySelector('.arch-sidebar')).toBeTruthy();
    component.setViewMode('table');
    fixture.detectChanges();
    expect(root.querySelector('.arch-table')).toBeTruthy();
  });

  it('collapses sidebar via toggle and keeps toolbar present', () => {
    loadSampleRecord();
    expect(fixture.debugElement.query(By.css('.arch-toolbar'))).toBeTruthy();
    component.toggleSidebar();
    fixture.detectChanges();
    expect(component.sidebarCollapsed).toBe(true);
    expect((fixture.nativeElement as HTMLElement).querySelector('.arch-workspace--sidebar-collapsed')).toBeTruthy();
    expect((fixture.nativeElement as HTMLElement).querySelector('.arch-sidebar')).toBeFalsy();
  });
});
