...
import { SequenceDiagramViewerComponent } from './sequence-diagram-viewer';
import { createSampleSeqFile, createSeqFileRecord } from '../../utils/sequence-diagram-viewer.utils';

@Component({ selector: 'lib-navigation', standalone: true, template: '' })
class StubNavigationComponent {}

describe('SequenceDiagramViewerComponent', () => {
  let component: SequenceDiagramViewerComponent;
  let fixture: ComponentFixture<SequenceDiagramViewerComponent>;
  ...
  beforeEach(async () => {
    ...
    await TestBed.configureTestingModule({
      imports: [SequenceDiagramViewerComponent],
      providers: [
        provideRouter([]),
      ...
      .overrideComponent(SequenceDiagramViewerComponent, {
        remove: { imports: [Navigation] },
        add: { imports: [StubNavigationComponent] }
      })
      .compileComponents();

    fixture = TestBed.createComponent(SequenceDiagramViewerComponent);
  ...
  function sampleBytes(): Uint8Array {
    return new TextEncoder().encode(SEQ_PUML_SAMPLE);
  }

  function loadSampleRecord(): void {
    const file = createSampleSeqFile();
    const record = createSeqFileRecord(file, sampleBytes());
  ...
  it('loads sample via handleFiles and enables export', async () => {
    ...
    expect(component.currentFile?.name).toBe('sample-checkout-seq.puml');
    expect(component.canExport).toBe(true);
    expect(component.parsed?.lifelines.length).toBe(3);
    expect(component.parsed?.messages.length).toBe(4);
  ...
  it('filters lifelines and repairs selection when filter removes current item', () => {
    loadSampleRecord();
    const first = component.filteredLifelines[0];
    expect(first).toBeTruthy();
    component.selectedLifelineId = first.id;
    component.query = 'kind:__none__';
    component.onFilterChange();
    expect(component.filteredLifelines.length).toBe(0);
    expect(component.selectedLifelineId).toBe('');
  });

  it('filters messages and repairs selection in table mode', () => {
    loadSampleRecord();
    component.setViewMode('table');
    const first = component.filteredMessages[0];
    component.selectedMessageId = first.id;
    component.query = 'msg:__none__';
    component.onFilterChange();
    expect(component.filteredMessages.length).toBe(0);
    expect(component.selectedMessageId).toBe('');
  });

  it('selects lifeline and message', () => {
    loadSampleRecord();
    const line = component.parsed!.lifelines.find((l) => l.name === 'User')!;
    component.selectLifeline(line.id);
    expect(component.selectedLifelineId).toBe(line.id);
    const msg = component.parsed!.messages[0];
    component.selectMessage(msg.id);
    expect(component.selectedMessageId).toBe(msg.id);
  });
  ...
  it('switches view modes and toggles sidebar', () => {
    ...
    component.setViewMode('messages');
  ...
  it('removeFile clears when last file removed and updates index otherwise', async () => {
    await component.loadSample();
    const second = createSeqFileRecord(new File([sampleBytes()], 'second.puml', { lastModified: 2 }), sampleBytes());
  ...
  it('shows error suggestion after a hard load failure message', () => {
    component.errorMessage = 'bad.puml: Invalid sequence diagram';
  ...
  it('keeps scroll-owner structure for map wrap, cards, table, and sidebar', async () => {
    ...
    expect(root.querySelector('.seq-map-wrap')).toBeTruthy();
    expect(root.querySelector('.seq-workspace')).toBeTruthy();
    expect(root.querySelector('.seq-sidebar')).toBeTruthy();
    component.setViewMode('table');
    fixture.detectChanges();
    expect(root.querySelector('.seq-table')).toBeTruthy();
  });

  it('collapses sidebar via toggle and keeps toolbar present', () => {
    loadSampleRecord();
    expect(fixture.debugElement.query(By.css('.seq-toolbar'))).toBeTruthy();
    component.toggleSidebar();
    fixture.detectChanges();
    expect(component.sidebarCollapsed).toBe(true);
    expect((fixture.nativeElement as HTMLElement).querySelector('.seq-workspace--sidebar-collapsed')).toBeTruthy();
    expect((fixture.nativeElement as HTMLElement).querySelector('.seq-sidebar')).toBeFalsy();
  });
});
