...
import { UmlViewerComponent } from './uml-viewer';
import { createSampleUmlFile, createUmlFileRecord } from '../../utils/uml-viewer.utils';

@Component({ selector: 'lib-navigation', standalone: true, template: '' })
class StubNavigationComponent {}

describe('UmlViewerComponent', () => {
  let component: UmlViewerComponent;
  let fixture: ComponentFixture<UmlViewerComponent>;
  ...
  beforeEach(async () => {
    ...
    await TestBed.configureTestingModule({
      ...
      .overrideComponent(UmlViewerComponent, {
        remove: { imports: [Navigation] },
        add: { imports: [StubNavigationComponent] }
      })
      .compileComponents();

    fixture = TestBed.createComponent(UmlViewerComponent);
  ...
  function sampleBytes(): Uint8Array {
    return new TextEncoder().encode(UML_CLASS_SAMPLE);
  }

  function loadSampleRecord(): void {
    const file = createSampleUmlFile();
    const record = createUmlFileRecord(file, sampleBytes());
  ...
  it('loads sample via handleFiles and enables export', async () => {
    ...
    expect(component.currentFile?.name).toBe('sample-order-uml.puml');
    expect(component.canExport).toBe(true);
    expect(component.parsed?.nodes.length).toBe(3);
    expect(component.parsed?.links.length).toBe(2);
  ...
  it('filters links and repairs selection in table mode', () => {
    loadSampleRecord();
  ...
  it('selects node and link', () => {
    loadSampleRecord();
    const node = component.parsed!.nodes.find((n) => n.name === 'Customer')!;
  ...
  it('removeFile clears when last file removed and updates index otherwise', async () => {
    await component.loadSample();
    const second = createUmlFileRecord(new File([sampleBytes()], 'second.puml', { lastModified: 2 }), sampleBytes());
  ...
  it('shows error suggestion after a hard load failure message', () => {
    component.errorMessage = 'bad.puml: Invalid UML diagram';
  ...
  it('keeps scroll-owner structure for map wrap, cards, table, and sidebar', async () => {
    ...
    expect(root.querySelector('.uml-map-wrap')).toBeTruthy();
    expect(root.querySelector('.uml-workspace')).toBeTruthy();
    expect(root.querySelector('.uml-sidebar')).toBeTruthy();
    component.setViewMode('table');
    fixture.detectChanges();
    expect(root.querySelector('.uml-table')).toBeTruthy();
  });

  it('collapses sidebar via toggle and keeps toolbar present', () => {
    loadSampleRecord();
    expect(fixture.debugElement.query(By.css('.uml-toolbar'))).toBeTruthy();
    component.toggleSidebar();
    fixture.detectChanges();
    expect(component.sidebarCollapsed).toBe(true);
    expect((fixture.nativeElement as HTMLElement).querySelector('.uml-workspace--sidebar-collapsed')).toBeTruthy();
    expect((fixture.nativeElement as HTMLElement).querySelector('.uml-sidebar')).toBeFalsy();
  });
});
