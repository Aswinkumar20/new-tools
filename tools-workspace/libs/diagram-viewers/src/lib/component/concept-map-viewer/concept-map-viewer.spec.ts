...
import { ConceptMapViewerComponent } from './concept-map-viewer';
import { createCmapFileRecord, createSampleCmapFile } from '../../utils/concept-map-viewer.utils';

@Component({ selector: 'lib-navigation', standalone: true, template: '' })
class StubNavigationComponent {}

describe('ConceptMapViewerComponent', () => {
  let component: ConceptMapViewerComponent;
  let fixture: ComponentFixture<ConceptMapViewerComponent>;
  ...
  beforeEach(async () => {
    ...
    await TestBed.configureTestingModule({
      imports: [ConceptMapViewerComponent],
      providers: [
        provideRouter([]),
      ...
      .overrideComponent(ConceptMapViewerComponent, {
        remove: { imports: [Navigation] },
        add: { imports: [StubNavigationComponent] }
      })
      .compileComponents();

    fixture = TestBed.createComponent(ConceptMapViewerComponent);
  ...
  function sampleBytes(): Uint8Array {
    return new TextEncoder().encode(CMAP_CXL_SAMPLE);
  }

  function loadSampleRecord(): void {
    const file = createSampleCmapFile();
    const record = createCmapFileRecord(file, sampleBytes());
  ...
  it('loads sample via handleFiles and enables export', async () => {
    ...
    expect(component.currentFile?.name).toBe('sample-shop-concept.cxl');
  ...
  it('removeFile clears when last file removed and updates index otherwise', async () => {
    await component.loadSample();
    const second = createCmapFileRecord(new File([sampleBytes()], 'second.cxl', { lastModified: 2 }), sampleBytes());
  ...
  it('shows error suggestion after a hard load failure message', () => {
    component.errorMessage = 'bad.cxl: Invalid concept map';
  ...
  it('keeps scroll-owner structure for map wrap, cards, table, and sidebar', async () => {
    ...
    expect(root.querySelector('.cmap-map-wrap')).toBeTruthy();
    expect(root.querySelector('.cmap-workspace')).toBeTruthy();
    expect(root.querySelector('.cmap-sidebar')).toBeTruthy();
    component.setViewMode('table');
    fixture.detectChanges();
    expect(root.querySelector('.cmap-table')).toBeTruthy();
  });

  it('collapses sidebar via toggle and keeps toolbar present', () => {
    loadSampleRecord();
    expect(fixture.debugElement.query(By.css('.cmap-toolbar'))).toBeTruthy();
    component.toggleSidebar();
    fixture.detectChanges();
    expect(component.sidebarCollapsed).toBe(true);
    expect((fixture.nativeElement as HTMLElement).querySelector('.cmap-workspace--sidebar-collapsed')).toBeTruthy();
    expect((fixture.nativeElement as HTMLElement).querySelector('.cmap-sidebar')).toBeFalsy();
  });
});
