...
import { GraphmlViewerComponent } from './graphml-viewer';
import { createGmlFileRecord, createSampleGmlFile } from '../../utils/graphml-viewer.utils';

@Component({ selector: 'lib-navigation', standalone: true, template: '' })
class StubNavigationComponent {}

describe('GraphmlViewerComponent', () => {
  let component: GraphmlViewerComponent;
  let fixture: ComponentFixture<GraphmlViewerComponent>;
  ...
  beforeEach(async () => {
    ...
    await TestBed.configureTestingModule({
      imports: [GraphmlViewerComponent],
      providers: [
        provideRouter([]),
      ...
      .overrideComponent(GraphmlViewerComponent, {
        remove: { imports: [Navigation] },
        add: { imports: [StubNavigationComponent] }
      })
      .compileComponents();

    fixture = TestBed.createComponent(GraphmlViewerComponent);
  ...
  function sampleBytes(): Uint8Array {
    return new TextEncoder().encode(GML_GRAPHML_SAMPLE);
  }

  function loadSampleRecord(): void {
    const file = createSampleGmlFile();
    const record = createGmlFileRecord(file, sampleBytes());
  ...
  it('loads sample via handleFiles and enables export', async () => {
    ...
    expect(component.currentFile?.name).toBe('sample-shop.graphml');
    expect(component.canExport).toBe(true);
    expect(component.parsed?.nodes.length).toBe(4);
    expect(component.parsed?.edges.length).toBe(4);
    expect(component.parsed?.communities.length).toBe(2);
  ...
  it('selects node, edge, and community', () => {
    loadSampleRecord();
    ...
    const comm = component.communities[0];
    component.selectCommunity(comm.id);
    expect(component.selectedCommunityId).toBe(comm.id);
  });

  it('setLayoutMode relayouts and is a no-op when unchanged', () => {
    loadSampleRecord();
    const before = component.parsed!.nodes.map((n) => n.x);
    component.setLayoutMode('community');
    expect(component.layoutMode).toBe('community');
  ...
  it('switches view modes and toggles sidebar', () => {
    ...
    component.setViewMode('communities');
    expect(component.viewMode).toBe('communities');
  ...
  it('removeFile clears when last file removed and updates index otherwise', async () => {
    await component.loadSample();
    const second = createGmlFileRecord(new File([sampleBytes()], 'second.graphml', { lastModified: 2 }), sampleBytes());
  ...
  it('shows error suggestion after a hard load failure message', () => {
    component.errorMessage = 'bad.graphml: Invalid GraphML network';
  ...
  it('keeps scroll-owner structure for map wrap, cards, table, and sidebar', async () => {
    ...
    expect(root.querySelector('.gml-map-wrap')).toBeTruthy();
    expect(root.querySelector('.gml-workspace')).toBeTruthy();
    expect(root.querySelector('.gml-sidebar')).toBeTruthy();
    component.setViewMode('table');
    fixture.detectChanges();
    expect(root.querySelector('.gml-table')).toBeTruthy();
  });

  it('collapses sidebar via toggle and keeps toolbar present', () => {
    loadSampleRecord();
    expect(fixture.debugElement.query(By.css('.gml-toolbar'))).toBeTruthy();
    component.toggleSidebar();
    fixture.detectChanges();
    expect(component.sidebarCollapsed).toBe(true);
    expect((fixture.nativeElement as HTMLElement).querySelector('.gml-workspace--sidebar-collapsed')).toBeTruthy();
    expect((fixture.nativeElement as HTMLElement).querySelector('.gml-sidebar')).toBeFalsy();
  });
});
