...
import { KubernetesArchitectureViewerComponent } from './kubernetes-architecture-viewer';
import { createK8sFileRecord, createSampleK8sFile } from '../../utils/kubernetes-architecture-viewer.utils';

@Component({ selector: 'lib-navigation', standalone: true, template: '' })
class StubNavigationComponent {}

describe('KubernetesArchitectureViewerComponent', () => {
  let component: KubernetesArchitectureViewerComponent;
  let fixture: ComponentFixture<KubernetesArchitectureViewerComponent>;
  ...
  beforeEach(async () => {
    ...
    await TestBed.configureTestingModule({
      imports: [KubernetesArchitectureViewerComponent],
      providers: [
        provideRouter([]),
      ...
      .overrideComponent(KubernetesArchitectureViewerComponent, {
        remove: { imports: [Navigation] },
        add: { imports: [StubNavigationComponent] }
      })
      .compileComponents();

    fixture = TestBed.createComponent(KubernetesArchitectureViewerComponent);
  ...
  function sampleBytes(): Uint8Array {
    return new TextEncoder().encode(K8S_SAMPLE);
  }

  function loadSampleRecord(): void {
    const file = createSampleK8sFile();
    const record = createK8sFileRecord(file, sampleBytes());
  ...
  it('loads sample via handleFiles and enables export', async () => {
    ...
    expect(component.currentFile?.name).toBe('sample-shop.yaml');
    expect(component.canExport).toBe(true);
    expect(component.parsed?.workloads.length).toBe(2);
    expect(component.parsed?.services.length).toBe(3);
    expect(component.parsed?.links.length).toBeGreaterThanOrEqual(3);
  ...
  it('selects workload, service, and link', () => {
    loadSampleRecord();
    const web = component.parsed!.workloads.find((w) => w.name === 'web');
    expect(web).toBeTruthy();
    component.selectWorkload(web!.id);
    expect(component.selectedWorkloadId).toBe(web!.id);
    const api = component.parsed!.services.find((s) => s.name === 'api');
    expect(api).toBeTruthy();
    component.selectService(api!.id);
    expect(component.selectedServiceId).toBe(api!.id);
  ...
  it('switches view modes and toggles sidebar', () => {
    loadSampleRecord();
    component.setViewMode('workloads');
    expect(component.viewMode).toBe('workloads');
    component.setViewMode('services');
    expect(component.viewMode).toBe('services');
  ...
  it('removeFile clears when last file removed and updates index otherwise', async () => {
    await component.loadSample();
    const second = createK8sFileRecord(new File([sampleBytes()], 'second.yaml', { lastModified: 2 }), sampleBytes());
  ...
  it('shows error suggestion after a hard load failure message', () => {
    component.errorMessage = 'bad.yaml: Invalid Kubernetes manifest';
  ...
  it('keeps scroll-owner structure for map wrap, cards, table, and sidebar', async () => {
    ...
    expect(root.querySelector('.k8s-map-wrap')).toBeTruthy();
    expect(root.querySelector('.k8s-workspace')).toBeTruthy();
    expect(root.querySelector('.k8s-sidebar')).toBeTruthy();
    component.setViewMode('table');
    fixture.detectChanges();
    expect(root.querySelector('.k8s-table')).toBeTruthy();
  });

  it('collapses sidebar via toggle and keeps toolbar present', () => {
    loadSampleRecord();
    expect(fixture.debugElement.query(By.css('.k8s-toolbar'))).toBeTruthy();
    component.toggleSidebar();
    fixture.detectChanges();
    expect(component.sidebarCollapsed).toBe(true);
    expect((fixture.nativeElement as HTMLElement).querySelector('.k8s-workspace--sidebar-collapsed')).toBeTruthy();
    expect((fixture.nativeElement as HTMLElement).querySelector('.k8s-sidebar')).toBeFalsy();
  });
});
