...
import { RdfViewerComponent } from './rdf-viewer';
import { createRdfFileRecord, createSampleRdfFile } from '../../utils/rdf-viewer.utils';
...
describe('RdfViewerComponent', () => {
  ...
  beforeEach(async () => {
    ...
    await TestBed.configureTestingModule({
      ...
      .overrideComponent(RdfViewerComponent, {
  ...
  function sampleBytes(): Uint8Array {
    return new TextEncoder().encode(RDF_SAMPLE);
  }

  function loadSampleRecord(): void {
    const file = createSampleRdfFile();
    const record = createRdfFileRecord(file, sampleBytes());
  ...
  it('loads sample via handleFiles and enables export', async () => {
    ...
    expect(component.currentFile?.name).toBe('sample-shop.ttl');
    expect(component.canExport).toBe(true);
    expect(component.parsed?.triples.length).toBeGreaterThanOrEqual(8);
    expect(component.parsed?.nodes.length).toBeGreaterThanOrEqual(5);
  ...
  it('filters triples and repairs selection in table mode', () => {
    loadSampleRecord();
    component.setViewMode('table');
    const first = component.filteredTriples[0];
    component.selectedTripleId = first.id;
    component.query = 'sub:__none__';
    component.onFilterChange();
    expect(component.filteredTriples.length).toBe(0);
    expect(component.selectedTripleId).toBe('');
  });

  it('selects node and triple', () => {
    loadSampleRecord();
    const node = component.parsed!.nodes[0];
    component.selectNode(node.id);
    expect(component.selectedNodeId).toBe(node.id);
    const triple = component.parsed!.triples[0];
    component.selectTriple(triple.id);
    expect(component.selectedTripleId).toBe(triple.id);
  });
  ...
  it('switches view modes and toggles sidebar', () => {
    ...
    component.setViewMode('triples');
    expect(component.viewMode).toBe('triples');
  ...
  it('removeFile clears when last file removed and updates index otherwise', async () => {
    await component.loadSample();
    const second = createRdfFileRecord(new File([sampleBytes()], 'second.ttl', { lastModified: 2 }), sampleBytes());
  ...
  it('shows error suggestion after a hard load failure message', () => {
    component.errorMessage = 'bad.ttl: Invalid RDF document';
  ...
  it('keeps scroll-owner structure for map wrap, cards, table, and sidebar', async () => {
    ...
    expect(root.querySelector('.rdf-map-wrap')).toBeTruthy();
    expect(root.querySelector('.rdf-workspace')).toBeTruthy();
    expect(root.querySelector('.rdf-sidebar')).toBeTruthy();
    component.setViewMode('table');
    fixture.detectChanges();
    expect(root.querySelector('.rdf-table')).toBeTruthy();
  });

  it('collapses sidebar via toggle and keeps toolbar present', () => {
    loadSampleRecord();
    expect(fixture.debugElement.query(By.css('.rdf-toolbar'))).toBeTruthy();
    component.toggleSidebar();
    fixture.detectChanges();
    expect(component.sidebarCollapsed).toBe(true);
    expect((fixture.nativeElement as HTMLElement).querySelector('.rdf-workspace--sidebar-collapsed')).toBeTruthy();
    expect((fixture.nativeElement as HTMLElement).querySelector('.rdf-sidebar')).toBeFalsy();
  });
});
