...
export class RdfViewerComponent implements AfterViewInit, OnDestroy {
    ...
    try {
      ...
      if (this.currentFile) {
        ...
        if (this.currentFile.softFail) {
          this.toast.warning('Parsed with little or no triples — metadata may still be available');
  ...
  onFilterChange(): void {
    ...
    if (this.selectedTripleId && !this.filteredTriples.some((t) => t.id === this.selectedTripleId)) {
      this.selectedTripleId = this.filteredTriples[0]?.id ?? '';
    }
    this.renderCanvas();
    this.cdr.markForCheck();
  }
