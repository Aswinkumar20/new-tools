import type { VsdConnector, VsdPage, VsdShape } from '../types/visio-viewer.types';

export function vsdShapeColor(index: number): string {
  const colors = ['#93c5fd', '#60a5fa', '#3b82f6', '#bfdbfe', '#2563eb'];
  return colors[index % colors.length];
}

export function renderVsdDiagram(
  ...
  if (!shapes.length) {
    ...
    ctx.fillText('No shapes on this Visio page.', 16, 28);
    return;
  }
  ...
  const pos = new Map(
    shapes.map((s) => [
      s.id,
      {
        x: mapX(s.x + s.width / 2),
        y: mapY(s.y + s.height / 2),
        w: Math.max(36, (s.width / spanX) * usableW),
        h: Math.max(22, (s.height / spanY) * usableH)
      }
    ] as const)
  );
  ...
  shapes.forEach((shape, i) => {
    ...
    if (!p) return;
    ctx.fillStyle = shape.id === selectedId ? '#dbeafe' : vsdShapeColor(i);
...
export function renderVsdPages(canvas: HTMLCanvasElement, pages: VsdPage[], selectedId: string | null): void {
  const ctx = canvas.getContext('2d');
  if (!ctx) return;
  ctx.fillStyle = '#0f172a';
  ctx.fillRect(0, 0, canvas.width, canvas.height);
...
export function renderVsdShapes(canvas: HTMLCanvasElement, shapes: VsdShape[], selectedId: string | null): void {
  const ctx = canvas.getContext('2d');
  if (!ctx) return;
  ctx.fillStyle = '#0f172a';
  ctx.fillRect(0, 0, canvas.width, canvas.height);
...
export function renderVsdConnectors(canvas: HTMLCanvasElement, connectors: VsdConnector[], selectedId: string | null): void {
  const ctx = canvas.getContext('2d');
  if (!ctx) return;
  ctx.fillStyle = '#0f172a';
  ctx.fillRect(0, 0, canvas.width, canvas.height);
