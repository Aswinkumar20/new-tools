import type { SqlsFk, SqlsTable } from '../types/sql-schema-viewer.types';

export function sqlsTableColor(index: number): string {
  const colors = ['#34d399', '#6ee7b7', '#10b981', '#a7f3d0', '#059669'];
  return colors[index % colors.length];
}

export function renderSqlsDiagram(
  ...
  if (!tables.length) {
    ...
    ctx.fillText('No tables in this SQL schema.', 16, 28);
    return;
  }
  ...
  ctx.lineWidth = 1.4;
  for (const fk of fks) {
    const a = pos.get(fk.source);
    const b = pos.get(fk.target);
    if (!a || !b) continue;
    ...
    ctx.lineTo(b.x, b.y);
    ctx.stroke();
    const label = `${fk.sourceColumn} → ${fk.targetColumn}`.trim();
  ...
  tables.forEach((table, i) => {
    ...
    ctx.fillStyle = table.id === selectedId ? '#a7f3d0' : sqlsTableColor(i);
...
export function renderSqlsTables(canvas: HTMLCanvasElement, tables: SqlsTable[], selectedId: string | null): void {
  const ctx = canvas.getContext('2d');
  if (!ctx) return;
  ctx.fillStyle = '#0f172a';
  ctx.fillRect(0, 0, canvas.width, canvas.height);
  ...
  tables.forEach((t, i) => {
    ...
    ctx.fillStyle = sqlsTableColor(i);
...
export function renderSqlsFks(canvas: HTMLCanvasElement, fks: SqlsFk[], selectedId: string | null): void {
  const ctx = canvas.getContext('2d');
  if (!ctx) return;
  ctx.fillStyle = '#0f172a';
  ctx.fillRect(0, 0, canvas.width, canvas.height);
  if (!fks.length) {
    ctx.fillStyle = '#94a3b8';
    ctx.font = '13px sans-serif';
    ctx.fillText('No matching foreign keys in this view.', 16, 28);
    return;
  }
  const rowH = Math.min(36, Math.max(22, (canvas.height - 24) / fks.length));
  fks.forEach((fk, i) => {
    const y = 16 + i * rowH;
    if (fk.id === selectedId) {
      ctx.fillStyle = 'rgba(17, 94, 89, 0.45)';
    ...
    ctx.fillStyle = '#e2e8f0';
    ctx.font = '12px sans-serif';
    ctx.fillText(`${fk.sourceName}.${fk.sourceColumn} → ${fk.targetName}.${fk.targetColumn}`, 32, y + 11);
  });
}
