import type { TfPreview, TfViewTransform } from '../types/tiff-viewer.types';

export function defaultTfView(): TfViewTransform {
  return { scale: 1, offsetX: 40, offsetY: 40 };
}

function previewBBox(items: TfPreview[]): { minX: number; minY: number; maxX: number; maxY: number } {
...
export function fitTfView(items: TfPreview[], width: number, height: number, pad = 36): TfViewTransform {
...
function toScreen(x: number, y: number, view: TfViewTransform): { x: number; y: number } {
  return { x: x * view.scale + view.offsetX, y: y * view.scale + view.offsetY };
}

export function renderTfPreview(canvas: HTMLCanvasElement, items: TfPreview[], selectedId: string | null, view: TfViewTransform): void {
  ...
  if (!items.length) {
    ...
    ctx.fillText('No TIFF preview on this page.', 16, 28);
    return;
  }
  for (const p of items) {
    const selected = p.id === selectedId;
    ctx.lineWidth = selected ? 2.8 : 1.5;
    ctx.strokeStyle = p.colorHex || '#93c5fd';
    ctx.fillStyle = p.colorHex || '#93c5fd';
    if (p.kind === 'circle') {
