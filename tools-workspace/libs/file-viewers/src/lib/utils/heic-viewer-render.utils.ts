import type { HcPreview, HcViewTransform } from '../types/heic-viewer.types';

export function defaultHcView(): HcViewTransform {
  return { scale: 1, offsetX: 40, offsetY: 40 };
}

function previewBBox(items: HcPreview[]): { minX: number; minY: number; maxX: number; maxY: number } {
...
export function fitHcView(items: HcPreview[], width: number, height: number, pad = 36): HcViewTransform {
...
function toScreen(x: number, y: number, view: HcViewTransform): { x: number; y: number } {
  return { x: x * view.scale + view.offsetX, y: y * view.scale + view.offsetY };
}

export function renderHcPreview(canvas: HTMLCanvasElement, items: HcPreview[], selectedId: string | null, view: HcViewTransform): void {
  ...
  if (!items.length) {
    ...
    ctx.fillText('No decoded HEIC preview in this dump.', 16, 28);
    return;
  }
  for (const p of items) {
    const selected = p.id === selectedId;
    ctx.lineWidth = selected ? 2.8 : 1.5;
    ctx.strokeStyle = p.colorHex || '#7dd3fc';
    ctx.fillStyle = p.colorHex || '#7dd3fc';
    if (p.kind === 'circle') {
