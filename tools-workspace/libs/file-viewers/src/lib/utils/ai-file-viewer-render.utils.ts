import type { AiPath, AiViewTransform } from '../types/ai-file-viewer.types';

export function defaultAiView(): AiViewTransform {
  return { scale: 1, offsetX: 40, offsetY: 40 };
}

function pathBBox(paths: AiPath[]): { minX: number; minY: number; maxX: number; maxY: number } {
  if (!paths.length) return { minX: 0, minY: 0, maxX: 12, maxY: 8 };
  let minX = Infinity;
  let minY = Infinity;
  let maxX = -Infinity;
  let maxY = -Infinity;
  for (const p of paths) {
    if (p.kind === 'circle' || p.kind === 'ellipse') {
      const rx = p.kind === 'ellipse' ? Math.max(p.w / 2, p.r, 0.2) : Math.max(p.r, 0.2);
      const ry = p.kind === 'ellipse' ? Math.max(p.h / 2, p.r, 0.2) : rx;
...
export function fitAiView(paths: AiPath[], width: number, height: number, pad = 36): AiViewTransform {
  const box = pathBBox(paths);
...
function toScreen(x: number, y: number, view: AiViewTransform): { x: number; y: number } {
  return { x: x * view.scale + view.offsetX, y: y * view.scale + view.offsetY };
}

export function renderAiPreview(canvas: HTMLCanvasElement, paths: AiPath[], selectedId: string | null, view: AiViewTransform): void {
  ...
  if (!paths.length) {
    ...
    ctx.fillText('No drawable Illustrator paths in this dump.', 16, 28);
    return;
  }
  for (const p of paths) {
    const selected = p.id === selectedId;
    ctx.lineWidth = selected ? 2.8 : 1.5;
    ctx.strokeStyle = p.colorHex || '#b45309';
    ctx.fillStyle = p.colorHex || '#b45309';
    if (p.kind === 'circle' || p.kind === 'ellipse') {
      const c = toScreen(p.x, p.y, view);
      const rx = Math.max(3, (p.kind === 'ellipse' ? Math.max(p.w / 2, p.r) : p.r) * view.scale);
      const ry = Math.max(3, (p.kind === 'ellipse' ? Math.max(p.h / 2, p.r) : p.r) * view.scale);
      ctx.beginPath();
