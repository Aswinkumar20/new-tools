import type { CrAssembly, CrColumn, CrDataset, CrInstance, CrPart, CrPartKind, CrSourceKind } from '../types/creo-viewer.types';
import { CR_JSON_SAMPLE } from '../constants/creo-viewer-sample.data';
import { isGzipMagic, isMostlyText, type CadDumpRec, prettyCadModelName } from './cad-file.utils';

const te = new TextEncoder();
const td = new TextDecoder('utf-8');
const CR_MAGIC = new Uint8Array([0x43, 0x52, 0x30, 0x31]); // CR01
const CR_COLORS = ['#fb7185', '#f59e0b', '#34d399', '#38bdf8', '#c4b5fd', '#fbbf24'];
...
function looksLikeCreo(text: string): boolean {
  const head = text.slice(0, 480);
  return (
    /\bCreo\b/i.test(head) ||
    /^\s*PRT\s+\S+/m.test(text) ||
    /^\s*ASM\s+\S+/m.test(text) ||
...
function isCrMagic(bytes: Uint8Array): boolean {
  return bytes.length >= 4 && bytes[0] === CR_MAGIC[0] && bytes[1] === CR_MAGIC[1] && bytes[2] === CR_MAGIC[2] && bytes[3] === CR_MAGIC[3];
}
...
function partKind(raw: unknown): CrPartKind {
...
function partVolume(kind: CrPartKind, sx: number, sy: number, sz: number, r: number, h: number): number {
...
function makePart(raw: CadDumpRec, index: number): CrPart {
  ...
  return {
    ...
    name,
    kind,
    colorHex: asString(raw.colorHex) || CR_COLORS[index % CR_COLORS.length],
...
function makeAssembly(raw: CadDumpRec, index: number, instanceCount = 0): CrAssembly {
...
function makeInstance(raw: CadDumpRec, index: number): CrInstance {
  const name = asString(raw.name || raw.id, `inst${index + 1}`);
  return {
    id: name,
    index,
    name,
    part: asString(raw.part || raw.prt),
...
function finishDataset(
  ...
  if (!parts.length && !assemblies.length && !instances.length) throw new Error('Creo dump contains no parts or assemblies');
...
function ingestJson(raw: unknown, fileName: string, sourceKind: CrSourceKind = 'json', warnings: string[] = []): CrDataset {
  const root = rec(raw);
  ...
  return finishDataset(
    ...
    sourceKind === 'creo' ? 'ASCII' : 'UTF-8',
    asString(root.version, '11.0'),
...
function parseAsciiCreo(text: string, fileName: string): CrDataset {
  const version = /Creo dump \S+ ([\d.]+)/i.exec(text)?.[1] || /ASM\s+\S+\s+(V[\w.]+)/i.exec(text)?.[1] || '11.0';
  const productName = /ASM\s+([A-Za-z0-9_-]+)/i.exec(text)?.[1] || prettyModelName(fileName, 'Product');
  const name = prettyModelName(fileName, productName);
  const parts: CrPart[] = [];
  const assemblies: CrAssembly[] = [];
  const instances: CrInstance[] = [];
  if (productName) assemblies.push(makeAssembly({ name: productName, description: 'assembly' }, 0));
  const partRe =
    /\bPRT\s+([A-Za-z0-9_-]+)\s+(BOX|CYLINDER|SPHERE|PLANE)\s+([-\d.eE]+)(?:\s+([-\d.eE]+))?(?:\s+([-\d.eE]+))?(?:\s+AT\s+([-\d.eE]+)\s+([-\d.eE]+)\s+([-\d.eE]+))?/gi;
  ...
  const prodRe = /\b(?:ASM|ASSEMBLY)\s+([A-Za-z0-9_-]+)/gi;
  ...
  if (!parts.length && !assemblies.length) throw new Error('Creo dump has no PRT or ASM entries');
  const warnings = ['ASCII Creo dump is a metadata subset — tessellation uses dump parts and instances.'];
  return finishDataset(name, 'creo', name, 'ASCII', version, 'm', parts, assemblies, instances, warnings);
}
...
function parseCsvAsCr(text: string, fileName: string): CrDataset {
  const lines = text.split(/\r?\n/).map((l) => l.trimEnd()).filter((l) => l && !l.startsWith('#'));
  if (lines.length < 2) throw new Error('Creo CSV dump contains no rows');
  ...
  return finishDataset(modelName, 'csv', modelName, 'UTF-8', '11.0', 'm', parts, assemblies, instances, []);
}

function parseMarkdown(text: string, fileName: string, sourceKind: CrSourceKind): CrDataset {
  ...
  if (!parts.length && !assemblies.length && !instances.length) throw new Error('Creo markdown contains no parts or assemblies');
  return finishDataset(name, sourceKind, name, 'UTF-8', '11.0', 'm', parts, assemblies, instances, []);
}

function parseCr01(bytes: Uint8Array, fileName: string): CrDataset {
  if (bytes.length < 8) throw new Error('Creo dump header is truncated');
  const len = u32le(bytes, 4);
  const jsonBytes = bytes.subarray(8, 8 + len);
  if (jsonBytes.length < len) throw new Error('Creo dump JSON payload is truncated');
  let parsed: unknown;
  try {
    parsed = JSON.parse(td.decode(jsonBytes));
  } catch {
    throw new Error('Invalid CR01 JSON');
  }
  return ingestJson(parsed, fileName, 'creo');
}

export function buildSampleCrBytes(): Uint8Array {
  const json = te.encode(CR_JSON_SAMPLE);
  const out: number[] = [...CR_MAGIC];
  writeU32le(json.length, out);
  out.push(...json);
  return new Uint8Array(out);
}

export function buildSampleCrJson(): string {
  return CR_JSON_SAMPLE;
}

export function parseCrText(text: string, fileName = ''): CrDataset {
  const stripped = text.replace(/^\uFEFF/, '');
  if (!stripped.trim()) throw new Error('Creo dump is empty');
  const raw = stripped.replace(/\r?\n+$/, '');
  const ext = (/\.([^.]+)$/.exec(fileName.toLowerCase())?.[1] ?? '').toLowerCase();
  if (ext === 'json' || (looksLikeJson(raw) && !looksLikeCreo(raw))) {
    let parsed: unknown;
    try {
      parsed = JSON.parse(raw);
    } catch {
      throw new Error('Invalid Creo JSON');
    }
    return ingestJson(parsed, fileName);
  }
  if (ext === 'prt' || ext === 'asm' || looksLikeCreo(raw)) return parseAsciiCreo(raw, fileName);
  if (ext === 'csv' || /^[\w."]+,[\w."]+/.test(raw.split(/\r?\n/)[0] || '')) return parseCsvAsCr(raw, fileName);
  if (ext === 'md' || (/^#\s+/m.test(raw) && (raw.includes('|') || /:\s+[A-Za-z]/.test(raw)))) {
    return parseMarkdown(raw, fileName, ext === 'md' ? 'markdown' : 'txt');
  }
  throw new Error('Not a Creo dump');
}

export function parseCrBytes(bytes: Uint8Array, fileName = ''): CrDataset {
  if (!bytes.length) throw new Error('Creo dump is empty');
  if (isGzipMagic(bytes)) throw new Error('Compressed Creo files are not supported — decompress first');
  if (isCrMagic(bytes)) return parseCr01(bytes, fileName);
  if (isOleMagic(bytes)) throw new Error('Binary Creo PRT/ASM is not expanded here — export a dump or JSON');
  const ext = (/\.([^.]+)$/.exec(fileName.toLowerCase())?.[1] ?? '').toLowerCase();
  if ((ext === 'prt' || ext === 'asm') && !isMostlyText(bytes)) {
    throw new Error('Binary Creo PRT/ASM is not expanded here — export a dump or JSON');
  }
  return parseCrText(td.decode(bytes), fileName);
}

export function filterCrParts(parts: CrPart[], query: string): CrPart[] {
...
export function filterCrAssemblies(items: CrAssembly[], query: string): CrAssembly[] {
  const q = query.trim().toLowerCase();
  if (!q) return items;
  const tokens = q.split(/\s+/).filter(Boolean);
...
export function filterCrInstances(items: CrInstance[], query: string): CrInstance[] {
  const q = query.trim().toLowerCase();
  if (!q) return items;
  const tokens = q.split(/\s+/).filter(Boolean);
...
export function filterCrRows(rows: Array<Record<string, string>>, query: string): Array<Record<string, string>> {
  const q = query.trim().toLowerCase();
  if (!q) return rows;
  const tokens = q.split(/\s+/).filter(Boolean);
