import type { CtAssembly, CtColumn, CtDataset, CtInstance, CtPart, CtPartKind, CtSourceKind } from '../types/catia-viewer.types';
import { CT_JSON_SAMPLE } from '../constants/catia-viewer-sample.data';
import { isGzipMagic, isMostlyText, type CadDumpRec, prettyCadModelName } from './cad-file.utils';

const te = new TextEncoder();
const td = new TextDecoder('utf-8');
const CT_MAGIC = new Uint8Array([0x43, 0x54, 0x30, 0x31]); // CT01
const CT_COLORS = ['#f87171', '#fb923c', '#34d399', '#38bdf8', '#c4b5fd', '#fbbf24'];
...
function looksLikeCatia(text: string): boolean {
  const head = text.slice(0, 480);
  return (
    /\bCATPart\b/i.test(head) ||
    /\bCATProduct\b/i.test(head) ||
    /\bCATIA\b/i.test(head) ||
    /^\s*PRODUCT\s+\S+/m.test(text) ||
...
function isCtMagic(bytes: Uint8Array): boolean {
  return bytes.length >= 4 && bytes[0] === CT_MAGIC[0] && bytes[1] === CT_MAGIC[1] && bytes[2] === CT_MAGIC[2] && bytes[3] === CT_MAGIC[3];
}
...
function partKind(raw: unknown): CtPartKind {
...
function partVolume(kind: CtPartKind, sx: number, sy: number, sz: number, r: number, h: number): number {
...
function makePart(raw: CadDumpRec, index: number): CtPart {
  ...
  return {
    ...
    name,
    kind,
    colorHex: asString(raw.colorHex) || CT_COLORS[index % CT_COLORS.length],
...
function makeAssembly(raw: CadDumpRec, index: number, instanceCount = 0): CtAssembly {
...
function makeInstance(raw: CadDumpRec, index: number): CtInstance {
  const name = asString(raw.name || raw.id, `inst${index + 1}`);
  return {
    id: name,
    index,
    name,
    part: asString(raw.part || raw.catpart),
...
function finishDataset(
  ...
  if (!parts.length && !assemblies.length && !instances.length) throw new Error('CATIA dump contains no parts or assemblies');
...
function ingestJson(raw: unknown, fileName: string, sourceKind: CtSourceKind = 'json', warnings: string[] = []): CtDataset {
  const root = rec(raw);
  ...
  return finishDataset(
    ...
    sourceKind === 'catia' ? 'ASCII' : 'UTF-8',
    asString(root.version, 'V5'),
...
function parseAsciiCatia(text: string, fileName: string): CtDataset {
  const version = /CATProduct\s+\S+\s+(V[\w.]+)/i.exec(text)?.[1] || /V5R\d+/i.exec(text)?.[0] || 'V5';
  const productName = /CATProduct\s+([A-Za-z0-9_-]+)/i.exec(text)?.[1] || prettyModelName(fileName, 'Product');
  const name = prettyModelName(fileName, productName);
  const parts: CtPart[] = [];
  const assemblies: CtAssembly[] = [];
  const instances: CtInstance[] = [];
  if (productName) assemblies.push(makeAssembly({ name: productName, description: 'product' }, 0));
  const partRe =
    /\bCATPart\s+([A-Za-z0-9_-]+)\s+(BOX|CYLINDER|SPHERE|PLANE)\s+([-\d.eE]+)(?:\s+([-\d.eE]+))?(?:\s+([-\d.eE]+))?(?:\s+AT\s+([-\d.eE]+)\s+([-\d.eE]+)\s+([-\d.eE]+))?/gi;
  ...
  const prodRe = /\bPRODUCT\s+([A-Za-z0-9_-]+)/gi;
  while ((m = prodRe.exec(text))) {
    const matchName = m?.[1] ?? '';
    if (matchName && !assemblies.some((a) => a.name === matchName)) assemblies.push(makeAssembly({ name: matchName, description: 'product' }, assemblies.length));
  }
  const instRe =
  ...
  if (!parts.length && !assemblies.length) throw new Error('CATIA dump has no CATPart or CATProduct entries');
  const warnings = ['ASCII CATIA dump is a metadata subset — tessellation uses dump parts and instances.'];
  return finishDataset(name, 'catia', name, 'ASCII', version, 'm', parts, assemblies, instances, warnings);
}
...
function parseCsvAsCt(text: string, fileName: string): CtDataset {
  const lines = text.split(/\r?\n/).map((l) => l.trimEnd()).filter((l) => l && !l.startsWith('#'));
  if (lines.length < 2) throw new Error('CATIA CSV dump contains no rows');
  ...
  return finishDataset(modelName, 'csv', modelName, 'UTF-8', 'V5', 'm', parts, assemblies, instances, []);
}

function parseMarkdown(text: string, fileName: string, sourceKind: CtSourceKind): CtDataset {
  ...
  if (!parts.length && !assemblies.length && !instances.length) throw new Error('CATIA markdown contains no parts or assemblies');
  return finishDataset(name, sourceKind, name, 'UTF-8', 'V5', 'm', parts, assemblies, instances, []);
}

function parseCt01(bytes: Uint8Array, fileName: string): CtDataset {
  if (bytes.length < 8) throw new Error('CATIA dump header is truncated');
  const len = u32le(bytes, 4);
  const jsonBytes = bytes.subarray(8, 8 + len);
  if (jsonBytes.length < len) throw new Error('CATIA dump JSON payload is truncated');
  let parsed: unknown;
  try {
    parsed = JSON.parse(td.decode(jsonBytes));
  } catch {
    throw new Error('Invalid CT01 JSON');
  }
  return ingestJson(parsed, fileName, 'catia');
}

export function buildSampleCtBytes(): Uint8Array {
  const json = te.encode(CT_JSON_SAMPLE);
  const out: number[] = [...CT_MAGIC];
  writeU32le(json.length, out);
  out.push(...json);
  return new Uint8Array(out);
}

export function buildSampleCtJson(): string {
  return CT_JSON_SAMPLE;
}

export function parseCtText(text: string, fileName = ''): CtDataset {
  const stripped = text.replace(/^\uFEFF/, '');
  if (!stripped.trim()) throw new Error('CATIA dump is empty');
  const raw = stripped.replace(/\r?\n+$/, '');
  const ext = (/\.([^.]+)$/.exec(fileName.toLowerCase())?.[1] ?? '').toLowerCase();
  if (ext === 'json' || (looksLikeJson(raw) && !looksLikeCatia(raw))) {
    let parsed: unknown;
    try {
      parsed = JSON.parse(raw);
    } catch {
      throw new Error('Invalid CATIA JSON');
    }
    return ingestJson(parsed, fileName);
  }
  if (ext === 'catpart' || ext === 'catproduct' || looksLikeCatia(raw)) return parseAsciiCatia(raw, fileName);
  if (ext === 'csv' || /^[\w."]+,[\w."]+/.test(raw.split(/\r?\n/)[0] || '')) return parseCsvAsCt(raw, fileName);
  if (ext === 'md' || (/^#\s+/m.test(raw) && (raw.includes('|') || /:\s+[A-Za-z]/.test(raw)))) {
    return parseMarkdown(raw, fileName, ext === 'md' ? 'markdown' : 'txt');
  }
  throw new Error('Not a CATIA dump');
}

export function parseCtBytes(bytes: Uint8Array, fileName = ''): CtDataset {
  if (!bytes.length) throw new Error('CATIA dump is empty');
  if (isGzipMagic(bytes)) throw new Error('Compressed CATIA files are not supported — decompress first');
  if (isCtMagic(bytes)) return parseCt01(bytes, fileName);
  if (isOleMagic(bytes)) throw new Error('Binary CATIA CATPart/CATProduct is not expanded here — export a dump or JSON');
  const ext = (/\.([^.]+)$/.exec(fileName.toLowerCase())?.[1] ?? '').toLowerCase();
  if ((ext === 'catpart' || ext === 'catproduct') && !isMostlyText(bytes)) {
    throw new Error('Binary CATIA CATPart/CATProduct is not expanded here — export a dump or JSON');
  }
  return parseCtText(td.decode(bytes), fileName);
}

export function filterCtParts(parts: CtPart[], query: string): CtPart[] {
...
export function filterCtAssemblies(items: CtAssembly[], query: string): CtAssembly[] {
  const q = query.trim().toLowerCase();
  if (!q) return items;
  const tokens = q.split(/\s+/).filter(Boolean);
...
export function filterCtInstances(items: CtInstance[], query: string): CtInstance[] {
  const q = query.trim().toLowerCase();
  if (!q) return items;
  const tokens = q.split(/\s+/).filter(Boolean);
...
export function filterCtRows(rows: Array<Record<string, string>>, query: string): Array<Record<string, string>> {
  const q = query.trim().toLowerCase();
  if (!q) return rows;
  const tokens = q.split(/\s+/).filter(Boolean);
