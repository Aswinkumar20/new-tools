...
import { CreoViewerComponent } from './creo-viewer';
import { buildSampleCrBytes, createSampleCrFile, createCrFileRecord } from '../../utils/creo-viewer.utils';

@Component({ selector: 'lib-navigation', standalone: true, template: '' })
class StubNavigationComponent {}

describe('CreoViewerComponent', () => {
  let component: CreoViewerComponent;
  let fixture: ComponentFixture<CreoViewerComponent>;
  ...
  beforeEach(async () => {
    ...
    await TestBed.configureTestingModule({
      ...
      .overrideComponent(CreoViewerComponent, {
        remove: { imports: [Navigation] },
        add: { imports: [StubNavigationComponent] }
      })
      .compileComponents();

    fixture = TestBed.createComponent(CreoViewerComponent);
  ...
  it('loads sample via handleFiles and enables export', async () => {
    ...
    expect(component.currentFile?.name).toBe('crank-arm.prt');
  ...
  it('removeFile clears when last file removed and updates index otherwise', async () => {
    await component.loadSample();
    const second = createCrFileRecord(
      new File([buildSampleCrBytes()], 'second.prt', { lastModified: 2 }),
      buildSampleCrBytes()
    );
  ...
  it('shows error suggestion after a hard load failure message', () => {
    component.errorMessage = 'bad.prt: Invalid Creo dump';
