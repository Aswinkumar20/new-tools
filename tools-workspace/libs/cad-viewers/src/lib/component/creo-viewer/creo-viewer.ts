...
import {
  ...
  CR_FORMATS_LABEL,
  CR_RELATED_TOOLS,
  CR_SUPPORTED_EXTENSIONS
} from '../../constants/creo-viewer.constants';
import type { CrAssembly, CrColumn, CrExportFormat, CrInstance, CrLoadedFile, CrPart, CrViewMode } from '../../types/creo-viewer.types';
...
import {
  ...
  toCad3dParts,
  toCad3dInstances
} from '../../utils/creo-viewer.utils';

@Component({
  selector: 'lib-creo-viewer',
  standalone: true,
  templateUrl: './creo-viewer.html',
  styleUrls: ['./creo-viewer.scss'],
  imports: [CommonModule, FormsModule, RouterLink, Navigation, TooltipDirective, StatValueTooltipHostDirective],
  changeDetection: ChangeDetectionStrategy.OnPush
})
export class CreoViewerComponent implements AfterViewInit, OnDestroy {
  // ─── Dependencies ───────────────────────────────────────────────────────────
  ...
  // ─── Constants / labels ─────────────────────────────────────────────────────
  readonly acceptAttr = CR_ACCEPT_ATTR;
  readonly relatedTools = CR_RELATED_TOOLS;
  readonly supportedExtensions = CR_SUPPORTED_EXTENSIONS;
  readonly formatsLabel = CR_FORMATS_LABEL;
  readonly formatsHint = CR_FORMATS_HINT;
  readonly viewModes: Array<{ id: CrViewMode; label: string }> = [
  ...
  // ─── Derived state ──────────────────────────────────────────────────────────
  get currentFile(): CrLoadedFile | null {
  ...
  get canExport(): boolean {
    return canExportCr(this.currentFile);
  }
  ...
  get filteredParts(): CrPart[] {
    return this.parsed ? filterCrParts(this.parsed.parts, this.query) : [];
  }

  get filteredAssemblies(): CrAssembly[] {
    return this.parsed ? filterCrAssemblies(this.parsed.assemblies, this.query) : [];
  }

  get filteredInstances(): CrInstance[] {
    return this.parsed ? filterCrInstances(this.parsed.instances, this.query) : [];
  }

  get filteredColumns(): CrColumn[] {
    return this.parsed?.columns ?? [];
  }

  get filteredRows(): Array<Record<string, string>> {
    return this.parsed ? filterCrRows(this.parsed.rows, this.query) : [];
  }

  get visibleParts(): CrPart[] {
    if (!this.hiddenPartIds.size) return this.filteredParts;
    return this.filteredParts.filter((g) => !this.hiddenPartIds.has(g.id));
  }

  get visibleInstances(): CrInstance[] {
  ...
  get selectedPart(): CrPart | null {
    return this.filteredParts.find((s) => s.id === this.selectedPartId) ?? null;
  }

  get selectedAssembly(): CrAssembly | null {
    return this.filteredAssemblies.find((e) => e.id === this.selectedAssemblyId) ?? null;
  }

  get selectedInstance(): CrInstance | null {
    return this.filteredInstances.find((inst) => inst.id === this.selectedInstanceId) ?? null;
  }

  get metadataRows() {
    return this.parsed ? buildCrMetadataRows(this.parsed) : [];
  }

  get partMetadataRows() {
    return this.selectedPart ? buildCrPartMetadata(this.selectedPart) : [];
  }

  get assemblyMetadataRows() {
    return this.selectedAssembly ? buildCrAssemblyMetadata(this.selectedAssembly) : [];
  }
  ...
  get primarySuggestion() {
    const s = resolveCrSuggestion({ hasFiles: this.files.length > 0, hasError: !!this.errorMessage });
  ...
  trackByFileId(_i: number, file: CrLoadedFile): string {
    return file.id;
  }
  ...
  trackByPart(_i: number, part: CrPart): string {
    return part.id;
  }

  trackByAssembly(_i: number, assembly: CrAssembly): string {
    return assembly.id;
  }

  trackByInstance(_i: number, instance: CrInstance): string {
    return instance.id;
  }

  trackByColumn(_i: number, column: CrColumn): string {
    return column.id;
  }
  ...
  formatSize(bytes: number): string {
    return formatCrFileSize(bytes);
  }

  tint(type: string, index: number): string {
    return crTypeColor(type, index);
  }
  ...
  async handleFiles(files: File[]): Promise<void> {
    const { accepted, rejected } = filterValidCrFiles(files);
    ...
    try {
      for (const file of accepted) {
        try {
          const bytes = await readCrFileBytes(file);
          const record = createCrFileRecord(file, bytes);
          ...
          this.errorMessage = `${file.name}: ${error instanceof Error ? error.message : 'Invalid Creo dump'}`;
  ...
  async loadSample(): Promise<void> {
    await this.handleFiles([createSampleCrFile()]);
  }
  ...
  setViewMode(mode: CrViewMode): void {
  ...
  exportAs(format: CrExportFormat, event: Event): void {
    event.stopPropagation();
    this.showExportMenu = false;
    ...
    try {
      ...
      else if (format === 'summary-json') downloadTextFile(exportCrSummaryJson(file), `${file.name}.summary.json`, 'application/json');
      else if (format === 'schema-csv') downloadTextFile(exportCrSchemaCsv(file.parsed), `${file.name}.schema.csv`, 'text/csv');
      else if (format === 'rows-csv') downloadTextFile(exportCrRowsCsv(file.parsed), `${file.name}.rows.csv`, 'text/csv');
      else if (format === 'png') {
  ...
  private selectPartFromInstance(inst: CrInstance): void {
    if (!inst.part) return;
  ...
  private renderCanvas(): void {
    ...
    if (this.viewMode === 'assemblies') {
      renderCrAssemblies(canvas, this.filteredAssemblies, this.selectedAssemblyId || null);
      return;
    }
    if (this.viewMode === 'preview') {
      renderCrInstances(canvas, this.visibleParts, this.visibleInstances, this.selectedInstanceId || null, this.view);
      return;
    }
    renderCrParts(canvas, this.visibleParts, this.selectedPartId || null, this.view);
  }
