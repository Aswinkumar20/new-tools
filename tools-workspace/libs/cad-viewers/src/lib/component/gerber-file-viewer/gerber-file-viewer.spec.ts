...
import { GerberFileViewerComponent } from './gerber-file-viewer';
import { buildSampleGbBytes, createGbFileRecord, createSampleGbFile } from '../../utils/gerber-file-viewer.utils';

@Component({ selector: 'lib-navigation', standalone: true, template: '' })
class StubNavigationComponent {}

describe('GerberFileViewerComponent', () => {
  let component: GerberFileViewerComponent;
  let fixture: ComponentFixture<GerberFileViewerComponent>;
  ...
  beforeEach(async () => {
    ...
    await TestBed.configureTestingModule({
      imports: [GerberFileViewerComponent],
      providers: [
        provideRouter([]),
      ...
      .overrideComponent(GerberFileViewerComponent, {
        remove: { imports: [Navigation] },
        add: { imports: [StubNavigationComponent] }
      })
      .compileComponents();

    fixture = TestBed.createComponent(GerberFileViewerComponent);
  ...
  it('loads sample via handleFiles and enables export', async () => {
    ...
    expect(component.currentFile?.name).toBe('rf-shield.gbr');
  ...
  it('clears selection', () => {
    ...
    expect(component.selectedFeatureId).toBe('');
  ...
  it('filters features and repairs selection when filter removes current item', () => {
    ...
    component.selectedFeatureId = first.id;
  ...
  it('selectRow maps row name to a feature id', () => {
    ...
    expect(component.selectedFeatureId || component.selectedLayerId).toBeTruthy();
  });
  ...
  it('removeFile clears when last file removed and updates index otherwise', async () => {
    await component.loadSample();
    const second = createGbFileRecord(
      new File([buildSampleGbBytes()], 'second.gbr', { lastModified: 2 }),
      buildSampleGbBytes()
    );
  ...
  it('shows error suggestion after a hard load failure message', () => {
    component.errorMessage = 'bad.gbr: Invalid Gerber dump';
