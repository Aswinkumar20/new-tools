...
import { InventorViewerComponent } from './inventor-viewer';
import { buildSampleIvBytes, createIvFileRecord, createSampleIvFile } from '../../utils/inventor-viewer.utils';

@Component({ selector: 'lib-navigation', standalone: true, template: '' })
class StubNavigationComponent {}

describe('InventorViewerComponent', () => {
  let component: InventorViewerComponent;
  let fixture: ComponentFixture<InventorViewerComponent>;
  ...
  beforeEach(async () => {
    ...
    await TestBed.configureTestingModule({
      imports: [InventorViewerComponent],
      providers: [
        provideRouter([]),
      ...
      .overrideComponent(InventorViewerComponent, {
        remove: { imports: [Navigation] },
        add: { imports: [StubNavigationComponent] }
      })
      .compileComponents();

    fixture = TestBed.createComponent(InventorViewerComponent);
  ...
  it('loads sample via handleFiles and enables export', async () => {
    ...
    expect(component.currentFile?.name).toBe('shaft-collar.ipt');
    expect(component.canExport).toBe(true);
    expect(component.parsed?.parts.length).toBeGreaterThan(0);
  ...
  it('clears selection', () => {
    ...
    expect(component.selectedPartId).toBe('');
    expect(component.selectedAssemblyId).toBe('');
    expect(component.selectedInstanceId).toBe('');
  });

  it('filters parts and repairs selection when filter removes current item', () => {
    loadSampleRecord();
    const first = component.filteredParts[0];
    expect(first).toBeTruthy();
    component.selectedPartId = first.id;
    component.query = 'kind:__none__';
    component.onFilterChange();
    expect(component.filteredParts.length).toBe(0);
    expect(component.selectedPartId).toBe('');
  });

  it('isolates selected part and showAllParts restores visibility', () => {
    loadSampleRecord();
    const parts = component.parsed!.parts;
    expect(parts.length).toBeGreaterThan(1);
    component.selectedPartId = parts[0].id;
    component.isolateSelectedPart();
    expect(component.hiddenPartIds.size).toBe(parts.length - 1);
    expect(component.isPartHidden(parts[1].id)).toBe(true);
    component.showAllParts();
    expect(component.hiddenPartIds.size).toBe(0);
  });

  it('toggles part visibility without changing selection', () => {
    loadSampleRecord();
    const id = component.parsed!.parts[0].id;
    const event = { stopPropagation: jest.fn() } as unknown as Event;
    component.togglePartVisible(id, event);
    expect(event.stopPropagation).toHaveBeenCalled();
    expect(component.isPartHidden(id)).toBe(true);
    component.togglePartVisible(id, event);
    expect(component.isPartHidden(id)).toBe(false);
  });
  ...
  it('switches view modes and toggles sidebar', () => {
    ...
    component.setViewMode('assemblies');
    expect(component.viewMode).toBe('assemblies');
    const before = component.sidebarCollapsed;
    component.toggleSidebar();
    expect(component.sidebarCollapsed).toBe(!before);
  });

  it('selectRow maps row name to part/assembly/instance ids', () => {
    loadSampleRecord();
    component.setViewMode('table');
  ...
  it('removeFile clears when last file removed and updates index otherwise', async () => {
    await component.loadSample();
    const second = createIvFileRecord(
      new File([buildSampleIvBytes()], 'second.ipt', { lastModified: 2 }),
      buildSampleIvBytes()
    );
  ...
  it('shows error suggestion after a hard load failure message', () => {
    component.errorMessage = 'bad.ipt: Invalid Inventor dump';
  ...
  it('hides parts in visibleParts and instances when hidden', () => {
    loadSampleRecord();
    const part = component.parsed!.parts[0];
    const beforeParts = component.visibleParts.length;
    const beforeInst = component.visibleInstances.length;
    component.hiddenPartIds = new Set([part.id]);
    expect(component.visibleParts.length).toBeLessThan(beforeParts);
    expect(component.visibleParts.every((p) => p.id !== part.id)).toBe(true);
    expect(component.visibleInstances.length).toBeLessThanOrEqual(beforeInst);
  });

  it('selectInstance links part and assembly when present', () => {
    loadSampleRecord();
    ...
    if (inst.part) {
      expect(component.selectedPartId).toBeTruthy();
    }
  });
});
