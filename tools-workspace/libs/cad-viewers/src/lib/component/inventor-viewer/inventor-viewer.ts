...
import {
  ...
  IV_FORMATS_LABEL,
  IV_RELATED_TOOLS,
  IV_SUPPORTED_EXTENSIONS
} from '../../constants/inventor-viewer.constants';
import type { IvAssembly, IvColumn, IvExportFormat, IvInstance, IvLoadedFile, IvPart, IvViewMode } from '../../types/inventor-viewer.types';
...
import {
  ...
  toCad3dInstances,
  toCad3dParts
} from '../../utils/inventor-viewer.utils';

@Component({
  selector: 'lib-inventor-viewer',
  standalone: true,
  templateUrl: './inventor-viewer.html',
  styleUrls: ['./inventor-viewer.scss'],
  imports: [CommonModule, FormsModule, RouterLink, Navigation, TooltipDirective, StatValueTooltipHostDirective],
  changeDetection: ChangeDetectionStrategy.OnPush
})
export class InventorViewerComponent implements AfterViewInit, OnDestroy {
  ...
  readonly acceptAttr = IV_ACCEPT_ATTR;
  readonly relatedTools = IV_RELATED_TOOLS;
  readonly supportedExtensions = IV_SUPPORTED_EXTENSIONS;
  readonly formatsLabel = IV_FORMATS_LABEL;
  readonly formatsHint = IV_FORMATS_HINT;
  readonly viewModes: Array<{ id: IvViewMode; label: string }> = [
  ...
  get canExport(): boolean {
    return canExportIv(this.currentFile);
  }
  ...
  get filteredParts(): IvPart[] {
    return this.parsed ? filterIvParts(this.parsed.parts, this.query) : [];
  }

  get filteredAssemblies(): IvAssembly[] {
    return this.parsed ? filterIvAssemblies(this.parsed.assemblies, this.query) : [];
  }

  get filteredInstances(): IvInstance[] {
    return this.parsed ? filterIvInstances(this.parsed.instances, this.query) : [];
  }

  get filteredColumns(): IvColumn[] {
    return this.parsed?.columns ?? [];
  }

  get filteredRows(): Array<Record<string, string>> {
    return this.parsed ? filterIvRows(this.parsed.rows, this.query) : [];
  }

  get visibleParts(): IvPart[] {
  ...
  get selectedAssembly(): IvAssembly | null {
    return this.filteredAssemblies.find((e) => e.id === this.selectedAssemblyId) ?? null;
  }

  get selectedInstance(): IvInstance | null {
    return this.filteredInstances.find((inst) => inst.id === this.selectedInstanceId) ?? null;
  }

  get metadataRows() {
    return this.parsed ? buildIvMetadataRows(this.parsed) : [];
  }

  get partMetadataRows() {
    return this.selectedPart ? buildIvPartMetadata(this.selectedPart) : [];
  }

  get assemblyMetadataRows() {
    return this.selectedAssembly ? buildIvAssemblyMetadata(this.selectedAssembly) : [];
  }
  ...
  get primarySuggestion() {
    const s = resolveIvSuggestion({ hasFiles: this.files.length > 0, hasError: !!this.errorMessage });
  ...
  trackByPart(_i: number, part: IvPart): string {
    return part.id;
  }

  trackByAssembly(_i: number, assembly: IvAssembly): string {
    return assembly.id;
  }

  trackByInstance(_i: number, instance: IvInstance): string {
    return instance.id;
  }

  trackByColumn(_i: number, column: IvColumn): string {
    return column.id;
  }
  ...
  async handleFiles(files: File[]): Promise<void> {
    const { accepted, rejected } = filterValidIvFiles(files);
    ...
    try {
      for (const file of accepted) {
        try {
          const bytes = await readIvFileBytes(file);
          const record = createIvFileRecord(file, bytes);
          ...
          this.errorMessage = `${file.name}: ${error instanceof Error ? error.message : 'Invalid Inventor dump'}`;
  ...
  async loadSample(): Promise<void> {
    await this.handleFiles([createSampleIvFile()]);
  }
  ...
  private renderCanvas(): void {
    ...
    if (this.viewMode === 'assemblies') {
      renderIvAssemblies(canvas, this.filteredAssemblies, this.selectedAssemblyId || null);
      return;
    }
    if (this.viewMode === 'preview') {
