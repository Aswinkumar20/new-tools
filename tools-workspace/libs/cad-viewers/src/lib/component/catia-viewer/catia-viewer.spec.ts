...
import { CatiaViewerComponent } from './catia-viewer';
import { buildSampleCtBytes, createSampleCtFile, createCtFileRecord } from '../../utils/catia-viewer.utils';

@Component({ selector: 'lib-navigation', standalone: true, template: '' })
class StubNavigationComponent {}

describe('CatiaViewerComponent', () => {
  let component: CatiaViewerComponent;
  let fixture: ComponentFixture<CatiaViewerComponent>;
  ...
  beforeEach(async () => {
    ...
    await TestBed.configureTestingModule({
      ...
      .overrideComponent(CatiaViewerComponent, {
        remove: { imports: [Navigation] },
        add: { imports: [StubNavigationComponent] }
      })
      .compileComponents();

    fixture = TestBed.createComponent(CatiaViewerComponent);
  ...
  it('loads sample via handleFiles and enables export', async () => {
    ...
    expect(component.currentFile?.name).toBe('wing-rib.catpart');
  ...
  it('removeFile clears when last file removed and updates index otherwise', async () => {
    await component.loadSample();
    const second = createCtFileRecord(
      new File([buildSampleCtBytes()], 'second.catpart', { lastModified: 2 }),
      buildSampleCtBytes()
    );
  ...
  it('shows error suggestion after a hard load failure message', () => {
    component.errorMessage = 'bad.catpart: Invalid CATIA dump';
