...
} from '../../constants/catia-viewer.constants';
import type { CtAssembly, CtColumn, CtExportFormat, CtInstance, CtLoadedFile, CtPart, CtViewMode } from '../../types/catia-viewer.types';
...
import {
  ...
  toCad3dParts,
  toCad3dInstances
} from '../../utils/catia-viewer.utils';

@Component({
  selector: 'lib-catia-viewer',
  standalone: true,
  templateUrl: './catia-viewer.html',
  styleUrls: ['./catia-viewer.scss'],
  imports: [CommonModule, FormsModule, RouterLink, Navigation, TooltipDirective, StatValueTooltipHostDirective],
  changeDetection: ChangeDetectionStrategy.OnPush
})
export class CatiaViewerComponent implements AfterViewInit, OnDestroy {
  // ─── Dependencies ───────────────────────────────────────────────────────────
  ...
  readonly viewModes: Array<{ id: CtViewMode; label: string }> = [
  ...
  get filteredParts(): CtPart[] {
    return this.parsed ? filterCtParts(this.parsed.parts, this.query) : [];
  }

  get filteredAssemblies(): CtAssembly[] {
    return this.parsed ? filterCtAssemblies(this.parsed.assemblies, this.query) : [];
  }

  get filteredInstances(): CtInstance[] {
    return this.parsed ? filterCtInstances(this.parsed.instances, this.query) : [];
  }

  get filteredColumns(): CtColumn[] {
    return this.parsed?.columns ?? [];
  }

  get filteredRows(): Array<Record<string, string>> {
    return this.parsed ? filterCtRows(this.parsed.rows, this.query) : [];
  }

  get visibleParts(): CtPart[] {
    if (!this.hiddenPartIds.size) return this.filteredParts;
    return this.filteredParts.filter((g) => !this.hiddenPartIds.has(g.id));
  }

  get visibleInstances(): CtInstance[] {
  ...
  get selectedPart(): CtPart | null {
    return this.filteredParts.find((s) => s.id === this.selectedPartId) ?? null;
  }

  get selectedAssembly(): CtAssembly | null {
    return this.filteredAssemblies.find((e) => e.id === this.selectedAssemblyId) ?? null;
  }

  get selectedInstance(): CtInstance | null {
    return this.filteredInstances.find((inst) => inst.id === this.selectedInstanceId) ?? null;
  }

  get metadataRows() {
    return this.parsed ? buildCtMetadataRows(this.parsed) : [];
  }

  get partMetadataRows() {
    return this.selectedPart ? buildCtPartMetadata(this.selectedPart) : [];
  }

  get assemblyMetadataRows() {
    return this.selectedAssembly ? buildCtAssemblyMetadata(this.selectedAssembly) : [];
  }
  ...
  get primarySuggestion() {
    const s = resolveCtSuggestion({ hasFiles: this.files.length > 0, hasError: !!this.errorMessage });
  ...
  trackByFileId(_i: number, file: CtLoadedFile): string {
    return file.id;
  }
  ...
  trackByPart(_i: number, part: CtPart): string {
    return part.id;
  }

  trackByAssembly(_i: number, assembly: CtAssembly): string {
    return assembly.id;
  }

  trackByInstance(_i: number, instance: CtInstance): string {
    return instance.id;
  }

  trackByColumn(_i: number, column: CtColumn): string {
    return column.id;
  }
    ...
    try {
      for (const file of accepted) {
        try {
          ...
          const record = createCtFileRecord(file, bytes);
          ...
          this.errorMessage = `${file.name}: ${error instanceof Error ? error.message : 'Invalid CATIA dump'}`;
  ...
  setViewMode(mode: CtViewMode): void {
  ...
  exportAs(format: CtExportFormat, event: Event): void {
    ...
    try {
      ...
      else if (format === 'summary-json') downloadTextFile(exportCtSummaryJson(file), `${file.name}.summary.json`, 'application/json');
      else if (format === 'schema-csv') downloadTextFile(exportCtSchemaCsv(file.parsed), `${file.name}.schema.csv`, 'text/csv');
      else if (format === 'rows-csv') downloadTextFile(exportCtRowsCsv(file.parsed), `${file.name}.rows.csv`, 'text/csv');
      else if (format === 'png') {
  ...
  private selectPartFromInstance(inst: CtInstance): void {
    if (!inst.part) return;
  ...
  private renderCanvas(): void {
    ...
    if (this.viewMode === 'assemblies') {
      renderCtAssemblies(canvas, this.filteredAssemblies, this.selectedAssemblyId || null);
      return;
    }
    if (this.viewMode === 'preview') {
      renderCtInstances(canvas, this.visibleParts, this.visibleInstances, this.selectedInstanceId || null, this.view);
      return;
    }
    renderCtParts(canvas, this.visibleParts, this.selectedPartId || null, this.view);
  }
