...
import { GdsiiLayoutViewerComponent } from './gdsii-layout-viewer';
import { buildSampleGdBytes, createGdFileRecord, createSampleGdFile } from '../../utils/gdsii-layout-viewer.utils';

@Component({ selector: 'lib-navigation', standalone: true, template: '' })
class StubNavigationComponent {}

describe('GdsiiLayoutViewerComponent', () => {
  let component: GdsiiLayoutViewerComponent;
  let fixture: ComponentFixture<GdsiiLayoutViewerComponent>;
  ...
  beforeEach(async () => {
    ...
    await TestBed.configureTestingModule({
      imports: [GdsiiLayoutViewerComponent],
      providers: [
        provideRouter([]),
      ...
      .overrideComponent(GdsiiLayoutViewerComponent, {
        remove: { imports: [Navigation] },
        add: { imports: [StubNavigationComponent] }
      })
      .compileComponents();

    fixture = TestBed.createComponent(GdsiiLayoutViewerComponent);
  ...
  it('loads sample via handleFiles and enables export', async () => {
    ...
    expect(component.currentFile?.name).toBe('nand2-x1.gds');
  ...
  it('clears selection', () => {
    ...
    expect(component.selectedFeatId).toBe('');
    expect(component.selectedCellId).toBe('');
    expect(component.selectedLayerId).toBe('');
  });

  it('filters features and repairs selection when filter removes current item', () => {
    loadSampleRecord();
    const first = component.filteredFeatures[0];
    expect(first).toBeTruthy();
    component.selectedFeatId = first.id;
  ...
  it('removeFile clears when last file removed and updates index otherwise', async () => {
    await component.loadSample();
    const second = createGdFileRecord(
      new File([buildSampleGdBytes()], 'second.gds', { lastModified: 2 }),
      buildSampleGdBytes()
    );
  ...
  it('shows error suggestion after a hard load failure message', () => {
    component.errorMessage = 'bad.gds: Invalid GDSII dump';
  ...
  it('hides features for hidden layers in visibleFeatures', () => {
    loadSampleRecord();
    const layerId = component.parsed!.layers[0].id;
    const before = component.visibleFeatures.length;
    component.hiddenLayerIds = new Set([layerId]);
    expect(component.visibleFeatures.length).toBeLessThanOrEqual(before);
    expect(component.visibleFeatures.every((f) => f.layer !== layerId)).toBe(true);
  });
});
