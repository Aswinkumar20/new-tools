...
import { SolidworksViewerComponent } from './solidworks-viewer';
import { buildSampleSwBytes, createSampleSwFile, createSwFileRecord } from '../../utils/solidworks-viewer.utils';

@Component({ selector: 'lib-navigation', standalone: true, template: '' })
class StubNavigationComponent {}

describe('SolidworksViewerComponent', () => {
  let component: SolidworksViewerComponent;
  let fixture: ComponentFixture<SolidworksViewerComponent>;
  ...
  beforeEach(async () => {
    ...
    await TestBed.configureTestingModule({
      imports: [SolidworksViewerComponent],
      providers: [
        provideRouter([]),
      ...
      .overrideComponent(SolidworksViewerComponent, {
        remove: { imports: [Navigation] },
        add: { imports: [StubNavigationComponent] }
      })
      .compileComponents();

    fixture = TestBed.createComponent(SolidworksViewerComponent);
  ...
  it('loads sample via handleFiles and enables export', async () => {
    ...
    expect(component.currentFile?.name).toBe('valve-body.sldprt');
  ...
  it('hides parts and linked instances when part hidden', () => {
    loadSampleRecord();
  ...
  it('selectInstance links part and assembly when present', () => {
    ...
    if (inst.part) expect(component.selectedPartId).toBeTruthy();
  });
  ...
  it('removeFile clears when last file removed and updates index otherwise', async () => {
    await component.loadSample();
    const second = createSwFileRecord(
      new File([buildSampleSwBytes()], 'second.sldprt', { lastModified: 2 }),
      buildSampleSwBytes()
    );
  ...
  it('shows error suggestion after a hard load failure message', () => {
    component.errorMessage = 'bad.sldprt: Invalid SolidWorks dump';
  ...
  it('setViewMode is a no-op when mode unchanged', () => {
    loadSampleRecord();
    component.viewMode = 'parts';
    component.setViewMode('parts');
    expect(component.viewMode).toBe('parts');
  });

  it('soft-fail empty dump disables export', async () => {
    const payload = new TextEncoder().encode(JSON.stringify({ parts: [], assemblies: [], instances: [] }));
