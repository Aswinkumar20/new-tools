...
import { PltPlotViewerComponent } from './plt-plot-viewer';
import { buildSamplePlBytes, createPlFileRecord, createSamplePlFile } from '../../utils/plt-plot-viewer.utils';

@Component({ selector: 'lib-navigation', standalone: true, template: '' })
class StubNavigationComponent {}

describe('PltPlotViewerComponent', () => {
  let component: PltPlotViewerComponent;
  let fixture: ComponentFixture<PltPlotViewerComponent>;
  ...
  beforeEach(async () => {
    ...
    await TestBed.configureTestingModule({
      imports: [PltPlotViewerComponent],
      providers: [
        provideRouter([]),
      ...
      .overrideComponent(PltPlotViewerComponent, {
        remove: { imports: [Navigation] },
        add: { imports: [StubNavigationComponent] }
      })
      .compileComponents();

    fixture = TestBed.createComponent(PltPlotViewerComponent);
  ...
  it('loads sample via handleFiles and enables export', async () => {
    ...
    expect(component.currentFile?.name).toBe('title-block.plt');
    expect(component.canExport).toBe(true);
    expect(component.parsed?.commands.length).toBeGreaterThan(0);
  ...
  it('clears selection', () => {
    ...
    expect(component.selectedPenId).toBe('');
  });
  ...
  it('toggles pen visibility', () => {
    loadSampleRecord();
    const id = component.parsed!.pens[0].id;
    const event = { stopPropagation: jest.fn() } as unknown as Event;
    component.togglePenVisible(id, event);
    expect(event.stopPropagation).toHaveBeenCalled();
    expect(component.isPenHidden(id)).toBe(true);
    component.togglePenVisible(id, event);
    expect(component.isPenHidden(id)).toBe(false);
  });

  it('hides commands in visibleCommands when pen hidden', () => {
    loadSampleRecord();
    const pen = component.parsed!.pens[0];
    const before = component.visibleCommands.length;
    component.hiddenPenIds = new Set([pen.id]);
    expect(component.visibleCommands.length).toBeLessThanOrEqual(before);
    expect(component.visibleCommands.every((c) => c.pen !== pen.id && c.pen !== pen.name)).toBe(true);
  });
  ...
  it('switches view modes and toggles sidebar', () => {
    ...
    component.setViewMode('pens');
    expect(component.viewMode).toBe('pens');
    const before = component.sidebarCollapsed;
    component.toggleSidebar();
    expect(component.sidebarCollapsed).toBe(!before);
  });

  it('selectRow maps row name to pen/command ids', () => {
    loadSampleRecord();
    component.setViewMode('table');
  ...
  it('selectCommand links pen when present', () => {
    loadSampleRecord();
    const command = component.parsed!.commands[0];
    expect(command).toBeTruthy();
    component.selectCommand(command.id);
    expect(component.selectedCommandId).toBe(command.id);
    if (command.pen) expect(component.selectedPenId).toBeTruthy();
  });
  ...
  it('removeFile clears when last file removed and updates index otherwise', async () => {
    await component.loadSample();
    const second = createPlFileRecord(
      new File([buildSamplePlBytes()], 'second.plt', { lastModified: 2 }),
      buildSamplePlBytes()
    );
  ...
  it('shows error suggestion after a hard load failure message', () => {
    component.errorMessage = 'bad.plt: Invalid PLT dump';
  ...
  it('soft-fail empty dump disables export', async () => {
    const payload = new TextEncoder().encode(JSON.stringify({ pens: [], commands: [] }));
