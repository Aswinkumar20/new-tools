...
import { DgnViewerComponent } from './dgn-viewer';
import { buildSampleDgBytes, createDgFileRecord, createSampleDgFile } from '../../utils/dgn-viewer.utils';

@Component({ selector: 'lib-navigation', standalone: true, template: '' })
class StubNavigationComponent {}

describe('DgnViewerComponent', () => {
  let component: DgnViewerComponent;
  let fixture: ComponentFixture<DgnViewerComponent>;
  ...
  beforeEach(async () => {
    ...
    await TestBed.configureTestingModule({
      ...
      .overrideComponent(DgnViewerComponent, {
        remove: { imports: [Navigation] },
        add: { imports: [StubNavigationComponent] }
      })
      .compileComponents();

    fixture = TestBed.createComponent(DgnViewerComponent);
  ...
  it('loads sample via handleFiles and enables export', async () => {
    ...
    expect(component.currentFile?.name).toBe('site-corridor.dgn');
    expect(component.canExport).toBe(true);
    expect(component.parsed?.layers.length).toBeGreaterThan(0);
    expect(component.parsed?.civil.length).toBeGreaterThan(0);
  ...
  it('clears selection', () => {
    ...
    expect(component.selectedCivilId).toBe('');
  ...
  it('isolates selected level and showAllLayers restores visibility', () => {
    loadSampleRecord();
  ...
  it('switches view modes and toggles sidebar', () => {
    ...
    component.setViewMode('civil');
    expect(component.viewMode).toBe('civil');
  ...
  it('selectRow maps row name to an entity or civil id', () => {
    loadSampleRecord();
    component.setViewMode('table');
    expect(component.filteredRows.length).toBeGreaterThan(0);
    component.selectRow(0);
    expect(component.selectedEntityId || component.selectedCivilId || component.selectedLayerId).toBeTruthy();
  });

  it('selectEntity syncs the matching level', () => {
    loadSampleRecord();
  ...
  it('removeFile clears when last file removed and updates index otherwise', async () => {
    await component.loadSample();
    const second = createDgFileRecord(
      new File([buildSampleDgBytes()], 'second.dgn', { lastModified: 2 }),
      buildSampleDgBytes()
    );
  ...
  it('shows error suggestion after a hard load failure message', () => {
    component.errorMessage = 'bad.dgn: Invalid DGN dump';
  ...
  it('hides entities and civil for hidden levels', () => {
    loadSampleRecord();
    const layerId = component.parsed!.layers[0].id;
    const beforeEnt = component.visibleEntities.length;
    const beforeCiv = component.visibleCivil.length;
    component.hiddenLayerIds = new Set([layerId]);
    expect(component.visibleEntities.length).toBeLessThanOrEqual(beforeEnt);
    expect(component.visibleCivil.length).toBeLessThanOrEqual(beforeCiv);
    expect(component.visibleEntities.every((e) => e.level !== layerId)).toBe(true);
    expect(component.visibleCivil.every((c) => c.level !== layerId)).toBe(true);
  });
