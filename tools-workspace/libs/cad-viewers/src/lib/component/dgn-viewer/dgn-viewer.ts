...
import {
  ...
  DG_FORMATS_LABEL,
  DG_RELATED_TOOLS,
  DG_SUPPORTED_EXTENSIONS
} from '../../constants/dgn-viewer.constants';
import type {
  DgCivil,
  DgColumn,
  DgEntity,
...
import {
  buildDgEntityMetadata,
  buildDgLayerMetadata,
  buildDgCivilMetadata,
  buildDgMetadataRows,
  canExportDg,
  canvasToPngDataUrl,
  createDgFileRecord,
  ...
  resolveDgSuggestion,
  toCadGeom
} from '../../utils/dgn-viewer.utils';

@Component({
  selector: 'lib-dgn-viewer',
  standalone: true,
  templateUrl: './dgn-viewer.html',
  styleUrls: ['./dgn-viewer.scss'],
  imports: [CommonModule, FormsModule, RouterLink, Navigation, TooltipDirective, StatValueTooltipHostDirective],
  changeDetection: ChangeDetectionStrategy.OnPush
})
export class DgnViewerComponent implements AfterViewInit, OnDestroy {
  ...
  readonly acceptAttr = DG_ACCEPT_ATTR;
  readonly relatedTools = DG_RELATED_TOOLS;
  readonly supportedExtensions = DG_SUPPORTED_EXTENSIONS;
  readonly formatsLabel = DG_FORMATS_LABEL;
  readonly formatsHint = DG_FORMATS_HINT;
  readonly viewModes: Array<{ id: DgViewMode; label: string }> = [
  ...
  get canExport(): boolean {
    return canExportDg(this.currentFile);
  }
  ...
  get filteredLayers(): DgLayer[] {
    return this.parsed ? filterDgLayers(this.parsed.layers, this.query) : [];
  }

  get filteredCivil(): DgCivil[] {
    return this.parsed ? filterDgCivil(this.parsed.civil, this.query) : [];
  }

  get filteredEntities(): DgEntity[] {
    return this.parsed ? filterDgEntities(this.parsed.entities, this.query) : [];
  }

  get filteredColumns(): DgColumn[] {
    return this.parsed?.columns ?? [];
  }

  get filteredRows(): Array<Record<string, string>> {
    return this.parsed ? filterDgRows(this.parsed.rows, this.query) : [];
  }
  ...
  get selectedCivil(): DgCivil | null {
    return this.filteredCivil.find((c) => c.id === this.selectedCivilId) ?? null;
  }

  get selectedEntity(): DgEntity | null {
  ...
  get metadataRows() {
    return this.parsed ? buildDgMetadataRows(this.parsed) : [];
  }

  get layerMetadataRows() {
    return this.selectedLayer ? buildDgLayerMetadata(this.selectedLayer) : [];
  }

  get civilMetadataRows() {
    return this.selectedCivil ? buildDgCivilMetadata(this.selectedCivil) : [];
  }

  get entityMetadataRows() {
    return this.selectedEntity ? buildDgEntityMetadata(this.selectedEntity) : [];
  }

  get hasSelection(): boolean {
    return !!(this.selectedCivilId || this.selectedEntityId || this.selectedLayerId);
  }

  get primarySuggestion() {
    const s = resolveDgSuggestion({ hasFiles: this.files.length > 0, hasError: !!this.errorMessage });
    return !s || s.id === this.dismissedSuggestionId ? null : s;
  }

  tint(type: string, index: number): string {
    return dgTypeColor(type, index);
  }
  ...
  onKeydown(event: KeyboardEvent): void {
    ...
    if (event.key === 'Escape') {
      ...
      else if (this.viewMode === 'civil') this.shiftCivil(1);
  ...
  trackByLayer(_i: number, layer: DgLayer): string {
    return layer.id;
  }

  trackByCivil(_i: number, item: DgCivil): string {
    return item.id;
  }

  trackByEntity(_i: number, entity: DgEntity): string {
    return entity.id;
  }

  trackByColumn(_i: number, column: DgColumn): string {
    return column.id;
  }
  ...
  async handleFiles(files: File[]): Promise<void> {
    const { accepted, rejected } = filterValidDgFiles(files);
    ...
    try {
      for (const file of accepted) {
        try {
          const bytes = await readDgFileBytes(file);
          const record = createDgFileRecord(file, bytes);
          ...
          this.errorMessage = `${file.name}: ${error instanceof Error ? error.message : 'Invalid DGN dump'}`;
  ...
  async loadSample(): Promise<void> {
    await this.handleFiles([createSampleDgFile()]);
  }
  ...
  onFilterChange(): void {
    ...
    if (this.selectedCivilId && !this.filteredCivil.some((c) => c.id === this.selectedCivilId)) {
      this.selectedCivilId = this.filteredCivil[0]?.id ?? '';
    }
  ...
  onCanvasPointerUp(event?: PointerEvent): void {
    ...
    const id = pickCadEntityAtScreen(toCadGeom(this.visibleEntities, this.visibleCivil), this.view, canvas.height, sx, sy);
  ...
  private shiftCivil(delta: number): void {
    ...
    const next = list[Math.min(list.length - 1, Math.max(0, idx + delta))];
    if (next) this.selectCivil(next.id);
  }
  ...
  private resetViewForCurrent(): void {
    ...
    this.selectedCivilId = this.parsed?.civil[0]?.id ?? '';
  ...
  private renderCanvas(): void {
    ...
    renderDgDrawing(canvas, this.visibleEntities, this.visibleCivil, this.drawingSelectedId, this.view);
  }
