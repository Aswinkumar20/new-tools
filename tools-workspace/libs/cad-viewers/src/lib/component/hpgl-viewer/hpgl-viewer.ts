...
import {
  ...
  HG_FORMATS_LABEL,
  HG_RELATED_TOOLS,
  HG_SUPPORTED_EXTENSIONS
} from '../../constants/hpgl-viewer.constants';
import type { HgColumn, HgCommand, HgExportFormat, HgLayer, HgLoadedFile, HgViewMode } from '../../types/hpgl-viewer.types';
import {
...
import {
  ...
  resolveHgSuggestion,
  toCadGeom
} from '../../utils/hpgl-viewer.utils';

@Component({
  selector: 'lib-hpgl-viewer',
  standalone: true,
  templateUrl: './hpgl-viewer.html',
  styleUrls: ['./hpgl-viewer.scss'],
  imports: [CommonModule, FormsModule, RouterLink, Navigation, TooltipDirective, StatValueTooltipHostDirective],
  changeDetection: ChangeDetectionStrategy.OnPush
})
export class HpglViewerComponent implements AfterViewInit, OnDestroy {
  ...
  readonly acceptAttr = HG_ACCEPT_ATTR;
  readonly relatedTools = HG_RELATED_TOOLS;
  readonly supportedExtensions = HG_SUPPORTED_EXTENSIONS;
  readonly formatsLabel = HG_FORMATS_LABEL;
  readonly formatsHint = HG_FORMATS_HINT;
  readonly viewModes: Array<{ id: HgViewMode; label: string }> = [
    { id: 'plot', label: 'Plot' },
  ...
  get currentFile(): HgLoadedFile | null {
  ...
  get canExport(): boolean {
    return canExportHg(this.currentFile);
  }
  ...
  get filteredLayers(): HgLayer[] {
    return this.parsed ? filterHgLayers(this.parsed.layers, this.query) : [];
  }

  get filteredCommands(): HgCommand[] {
    return this.parsed ? filterHgCommands(this.parsed.commands, this.query) : [];
  }

  get filteredColumns(): HgColumn[] {
    return this.parsed?.columns ?? [];
  }

  get filteredRows(): Array<Record<string, string>> {
    return this.parsed ? filterHgRows(this.parsed.rows, this.query) : [];
  }

  get visibleCommands(): HgCommand[] {
    return this.filteredCommands.filter((e) => !this.hiddenLayerIds.has(e.layer));
  }

  get selectedLayer(): HgLayer | null {
    return this.filteredLayers.find((l) => l.id === this.selectedLayerId) ?? null;
  }

  get selectedCommand(): HgCommand | null {
  ...
  get plotSelectedId(): string | null {
    ...
    if (this.viewMode === 'layers' && this.selectedLayerId) {
      return this.visibleCommands.find((e) => e.layer === this.selectedLayerId)?.id ?? null;
    }
    return this.selectedCommandId || null;
  }

  get metadataRows() {
    return this.parsed ? buildHgMetadataRows(this.parsed) : [];
  }

  get layerMetadataRows() {
    return this.selectedLayer ? buildHgLayerMetadata(this.selectedLayer) : [];
  }

  get commandMetadataRows() {
    return this.selectedCommand ? buildHgCommandMetadata(this.selectedCommand) : [];
  }

  get hasSelection(): boolean {
    return !!(this.selectedCommandId || this.selectedLayerId);
  }

  get primarySuggestion() {
    const s = resolveHgSuggestion({ hasFiles: this.files.length > 0, hasError: !!this.errorMessage });
  ...
  trackByFileId(_i: number, file: HgLoadedFile): string {
    return file.id;
  }
  ...
  trackByLayer(_i: number, layer: HgLayer): string {
    return layer.id;
  }

  trackByCommand(_i: number, command: HgCommand): string {
    return command.id;
  }

  trackByColumn(_i: number, column: HgColumn): string {
    return column.id;
  }
  ...
  formatSize(bytes: number): string {
    return formatHgFileSize(bytes);
  }

  tint(type: string, index: number): string {
    return hgTypeColor(type, index);
  }
  ...
  async handleFiles(files: File[]): Promise<void> {
    const { accepted, rejected } = filterValidHgFiles(files);
    ...
    try {
      for (const file of accepted) {
        try {
          const bytes = await readHgFileBytes(file);
          const record = createHgFileRecord(file, bytes);
          ...
          this.errorMessage = `${file.name}: ${error instanceof Error ? error.message : 'Invalid HPGL dump'}`;
  ...
  async loadSample(): Promise<void> {
    await this.handleFiles([createSampleHgFile()]);
  }
  ...
  setViewMode(mode: HgViewMode): void {
  ...
  exportAs(format: HgExportFormat, event: Event): void {
    event.stopPropagation();
    this.showExportMenu = false;
    ...
    try {
      if (format === 'original') {
        ...
        downloadTextFile(exportHgSummaryJson(file), `${file.name}.summary.json`, 'application/json');
      } else if (format === 'schema-csv') {
        downloadTextFile(exportHgSchemaCsv(file.parsed), `${file.name}.schema.csv`, 'text/csv');
      } else if (format === 'rows-csv') {
        downloadTextFile(exportHgRowsCsv(file.parsed), `${file.name}.rows.csv`, 'text/csv');
      } else if (format === 'png') {
        const canvas = this.canvasHost?.nativeElement;
        if (!canvas || this.viewMode === 'table') {
          this.toast.info('Open Plot, Layers, or Commands to export a PNG snapshot');
          this.cdr.markForCheck();
          return;
        }
  ...
  selectCommand(id: string): void {
    ...
    const cmd = this.filteredCommands.find((e) => e.id === id);
    if (cmd?.layer) {
      const layer = this.filteredLayers.find((l) => l.id === cmd.layer || l.name === cmd.layer);
  ...
  selectRow(index: number): void {
    ...
    if (row?.name) {
      const cmd = this.filteredCommands.find((e) => e.id === row.name || e.name === row.name);
      if (cmd) {
  ...
  private renderCanvas(): void {
    ...
    renderHgPlot(canvas, this.visibleCommands, this.plotSelectedId, this.view);
  }
