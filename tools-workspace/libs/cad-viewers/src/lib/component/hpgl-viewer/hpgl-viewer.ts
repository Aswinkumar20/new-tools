...
export class HpglViewerComponent implements AfterViewInit, OnDestroy {
  ...
  exportAs(format: HgExportFormat, event: Event): void {
    ...
    try {
      ...
      else if (format === 'summary-json') downloadTextFile(exportHgSummaryJson(file), `${file.name}.summary.json`, 'application/json');
      else if (format === 'schema-csv') downloadTextFile(exportHgSchemaCsv(file.parsed), `${file.name}.schema.csv`, 'text/csv');
      else if (format === 'rows-csv') downloadTextFile(exportHgRowsCsv(file.parsed), `${file.name}.rows.csv`, 'text/csv');
      else if (format === 'png') {
