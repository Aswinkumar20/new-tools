...
import { HpglViewerComponent } from './hpgl-viewer';
import { buildSampleHgBytes, createHgFileRecord, createSampleHgFile } from '../../utils/hpgl-viewer.utils';

@Component({ selector: 'lib-navigation', standalone: true, template: '' })
class StubNavigationComponent {}

describe('HpglViewerComponent', () => {
  let component: HpglViewerComponent;
  let fixture: ComponentFixture<HpglViewerComponent>;
  ...
  beforeEach(async () => {
    ...
    await TestBed.configureTestingModule({
      ...
      .overrideComponent(HpglViewerComponent, {
        remove: { imports: [Navigation] },
        add: { imports: [StubNavigationComponent] }
      })
      .compileComponents();

    fixture = TestBed.createComponent(HpglViewerComponent);
  ...
  it('loads sample via handleFiles and enables export', async () => {
    ...
    expect(component.currentFile?.name).toBe('outline-plot.hpgl');
  ...
  it('clears selection', () => {
    ...
    expect(component.selectedCommandId).toBe('');
    expect(component.selectedLayerId).toBe('');
  });

  it('filters commands and repairs selection when filter removes current item', () => {
    loadSampleRecord();
    const first = component.filteredCommands[0];
    expect(first).toBeTruthy();
    component.selectedCommandId = first.id;
  ...
  it('switches view modes and toggles sidebar', () => {
    ...
    component.setViewMode('commands');
    expect(component.viewMode).toBe('commands');
    const before = component.sidebarCollapsed;
    component.toggleSidebar();
    expect(component.sidebarCollapsed).toBe(!before);
  });

  it('selectRow maps row name to a command id', () => {
    loadSampleRecord();
    component.setViewMode('table');
    expect(component.filteredRows.length).toBeGreaterThan(0);
    component.selectRow(0);
    expect(component.selectedCommandId || component.selectedLayerId).toBeTruthy();
  });
  ...
  it('removeFile clears when last file removed and updates index otherwise', async () => {
    await component.loadSample();
    const second = createHgFileRecord(
      new File([buildSampleHgBytes()], 'second.hpgl', { lastModified: 2 }),
      buildSampleHgBytes()
    );
  ...
  it('shows error suggestion after a hard load failure message', () => {
    component.errorMessage = 'bad.hpgl: Invalid HPGL dump';
  ...
  it('hides commands for hidden layers in visibleCommands', () => {
    loadSampleRecord();
    const layerId = component.parsed!.layers[0].id;
    const before = component.visibleCommands.length;
    component.hiddenLayerIds = new Set([layerId]);
    expect(component.visibleCommands.length).toBeLessThanOrEqual(before);
    expect(component.visibleCommands.every((c) => c.layer !== layerId)).toBe(true);
  });
});
