...
import { EaglePcbViewerComponent } from './eagle-pcb-viewer';
import { buildSampleEgBytes, createEgFileRecord, createSampleEgFile } from '../../utils/eagle-pcb-viewer.utils';

@Component({ selector: 'lib-navigation', standalone: true, template: '' })
class StubNavigationComponent {}

describe('EaglePcbViewerComponent', () => {
  let component: EaglePcbViewerComponent;
  let fixture: ComponentFixture<EaglePcbViewerComponent>;
  ...
  beforeEach(async () => {
    ...
    await TestBed.configureTestingModule({
      imports: [EaglePcbViewerComponent],
      providers: [
        provideRouter([]),
      ...
      .overrideComponent(EaglePcbViewerComponent, {
        remove: { imports: [Navigation] },
        add: { imports: [StubNavigationComponent] }
      })
      .compileComponents();

    fixture = TestBed.createComponent(EaglePcbViewerComponent);
  ...
  it('loads sample via handleFiles and enables export', async () => {
    ...
    expect(component.currentFile?.name).toBe('arduino-shield.brd');
  ...
  it('selectRow maps row name to a board or schematic item', () => {
    loadSampleRecord();
    component.setViewMode('table');
    expect(component.filteredRows.length).toBeGreaterThan(0);
    component.selectRow(0);
    expect(component.selectedBoardId || component.selectedSchId || component.selectedLayerId).toBeTruthy();
  });

  it('selectBoard syncs the matching layer', () => {
    loadSampleRecord();
  ...
  it('removeFile clears when last file removed and updates index otherwise', async () => {
    await component.loadSample();
    const second = createEgFileRecord(
      new File([buildSampleEgBytes()], 'second.brd', { lastModified: 2 }),
      buildSampleEgBytes()
    );
  ...
  it('shows error suggestion after a hard load failure message', () => {
    component.errorMessage = 'bad.brd: Invalid Eagle dump';
  ...
  it('hides board items for hidden layers in visibleBoardItems', () => {
    loadSampleRecord();
    ...
    expect(component.visibleBoardItems.every((e) => e.layer !== layerId)).toBe(true);
  });
