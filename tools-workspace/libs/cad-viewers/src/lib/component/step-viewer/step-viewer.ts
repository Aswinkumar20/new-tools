...
import {
  ...
  ST_FORMATS_LABEL,
  ST_RELATED_TOOLS,
  ST_SUPPORTED_EXTENSIONS
} from '../../constants/step-viewer.constants';
import type {
  StColumn,
  StExportFormat,
  StLoadedFile,
...
@Component({
  selector: 'lib-step-viewer',
  standalone: true,
  templateUrl: './step-viewer.html',
  styleUrls: ['./step-viewer.scss'],
...
export class StepViewerComponent implements AfterViewInit, OnDestroy {
  ...
  readonly formatsLabel = ST_FORMATS_LABEL;
  readonly formatsHint = ST_FORMATS_HINT;
  readonly viewModes: Array<{ id: StViewMode; label: string }> = [
  ...
  get filteredSolids(): StSolid[] {
    return this.parsed ? filterStSolids(this.parsed.solids, this.query) : [];
  }

  get filteredMeasurements(): StMeasurement[] {
    return this.parsed ? filterStMeasurements(this.parsed.measurements, this.query) : [];
  }

  get filteredProducts(): StProduct[] {
  ...
  get filteredRows(): Array<Record<string, string>> {
    return this.parsed ? filterStRows(this.parsed.rows, this.query) : [];
  }
  ...
  get selectedSolid(): StSolid | null {
    return this.filteredSolids.find((s) => s.id === this.selectedSolidId) ?? null;
  }

  get selectedMeas(): StMeasurement | null {
    return this.filteredMeasurements.find((m) => m.id === this.selectedMeasId) ?? null;
  }

  get metadataRows() {
    return this.parsed ? buildStMetadataRows(this.parsed) : [];
  }

  get solidMetadataRows() {
    return this.selectedSolid ? buildStSolidMetadata(this.selectedSolid) : [];
  }

  get measMetadataRows() {
    return this.selectedMeas ? buildStMeasMetadata(this.selectedMeas) : [];
  }
  ...
  get primarySuggestion() {
    const s = resolveStSuggestion({ hasFiles: this.files.length > 0, hasError: !!this.errorMessage });
  ...
  trackBySolid(_i: number, solid: StSolid): string {
    return solid.id;
  }

  trackByMeas(_i: number, meas: StMeasurement): string {
    return meas.id;
  }

  trackByProduct(_i: number, product: StProduct): string {
    return product.id;
  }

  trackByColumn(_i: number, column: StColumn): string {
    return column.id;
  }
  ...
  async handleFiles(files: File[]): Promise<void> {
    const { accepted, rejected } = filterValidStFiles(files);
    ...
    try {
      for (const file of accepted) {
        try {
          const bytes = await readStFileBytes(file);
          const record = createStFileRecord(file, bytes);
          ...
          this.errorMessage = `${file.name}: ${error instanceof Error ? error.message : 'Invalid STEP dump'}`;
  ...
  async loadSample(): Promise<void> {
    await this.handleFiles([createSampleStFile()]);
  }
  ...
  exportAs(format: StExportFormat, event: Event): void {
    event.stopPropagation();
    this.showExportMenu = false;
    ...
    try {
      ...
      else if (format === 'summary-json') downloadTextFile(exportStSummaryJson(file), `${file.name}.summary.json`, 'application/json');
      else if (format === 'schema-csv') downloadTextFile(exportStSchemaCsv(file.parsed), `${file.name}.schema.csv`, 'text/csv');
      else if (format === 'rows-csv') downloadTextFile(exportStRowsCsv(file.parsed), `${file.name}.rows.csv`, 'text/csv');
      else if (format === 'png') {
  ...
  private renderCanvas(): void {
    ...
    if (this.viewMode === 'measurements') {
      renderStMeasurements(canvas, this.filteredMeasurements, this.selectedMeasId || null);
      return;
    }
    renderStSolids(canvas, this.visibleSolids, this.selectedSolidId || null, this.view);
  }
