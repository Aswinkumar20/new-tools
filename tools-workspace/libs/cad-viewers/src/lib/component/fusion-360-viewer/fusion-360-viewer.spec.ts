...
import { Fusion360ViewerComponent } from './fusion-360-viewer';
import { buildSampleFuBytes, createFuFileRecord, createSampleFuFile } from '../../utils/fusion-360-viewer.utils';

@Component({ selector: 'lib-navigation', standalone: true, template: '' })
class StubNavigationComponent {}

describe('Fusion360ViewerComponent', () => {
  let component: Fusion360ViewerComponent;
  let fixture: ComponentFixture<Fusion360ViewerComponent>;
  ...
  beforeEach(async () => {
    ...
    await TestBed.configureTestingModule({
      imports: [Fusion360ViewerComponent],
      providers: [
        provideRouter([]),
      ...
      .overrideComponent(Fusion360ViewerComponent, {
        remove: { imports: [Navigation] },
        add: { imports: [StubNavigationComponent] }
      })
      .compileComponents();

    fixture = TestBed.createComponent(Fusion360ViewerComponent);
  ...
  it('loads sample via handleFiles and enables export', async () => {
    ...
    expect(component.currentFile?.name).toBe('enclosure-lid.f3d');
    expect(component.canExport).toBe(true);
    expect(component.parsed?.bodies.length).toBeGreaterThan(0);
  ...
  it('clears selection', () => {
    ...
    expect(component.selectedBodyId).toBe('');
    expect(component.selectedComponentId).toBe('');
    expect(component.selectedInstanceId).toBe('');
  });

  it('filters bodies and repairs selection when filter removes current item', () => {
    loadSampleRecord();
    const first = component.filteredBodies[0];
    expect(first).toBeTruthy();
    component.selectedBodyId = first.id;
    component.query = 'kind:__none__';
    component.onFilterChange();
    expect(component.filteredBodies.length).toBe(0);
    expect(component.selectedBodyId).toBe('');
  });

  it('toggles body visibility', () => {
    loadSampleRecord();
    const id = component.parsed!.bodies[0].id;
    const event = { stopPropagation: jest.fn() } as unknown as Event;
    component.toggleBodyVisible(id, event);
    expect(event.stopPropagation).toHaveBeenCalled();
    expect(component.isBodyHidden(id)).toBe(true);
    component.toggleBodyVisible(id, event);
    expect(component.isBodyHidden(id)).toBe(false);
  });

  it('hides bodies and linked instances when body hidden', () => {
    loadSampleRecord();
    const body = component.parsed!.bodies[0];
    const beforeBodies = component.visibleBodies.length;
    const beforeInstances = component.visibleInstances.length;
    component.hiddenBodyIds = new Set([body.id]);
    expect(component.visibleBodies.length).toBeLessThan(beforeBodies);
    expect(component.visibleInstances.length).toBeLessThanOrEqual(beforeInstances);
    expect(component.visibleBodies.every((b) => b.id !== body.id)).toBe(true);
  });
  ...
  it('selectRow maps row name to body/component/instance ids', () => {
    loadSampleRecord();
    component.setViewMode('table');
  ...
  it('selectInstance links body and component when present', () => {
    loadSampleRecord();
    ...
    if (inst.body) expect(component.selectedBodyId).toBeTruthy();
  });
  ...
  it('removeFile clears when last file removed and updates index otherwise', async () => {
    await component.loadSample();
    const second = createFuFileRecord(
      new File([buildSampleFuBytes()], 'second.f3d', { lastModified: 2 }),
      buildSampleFuBytes()
    );
  ...
  it('shows error suggestion after a hard load failure message', () => {
    component.errorMessage = 'bad.f3d: Invalid Fusion dump';
  ...
  it('setViewMode is a no-op when mode unchanged', () => {
    loadSampleRecord();
    component.viewMode = 'bodies';
    component.setViewMode('bodies');
    expect(component.viewMode).toBe('bodies');
  });

  it('soft-fail empty dump disables export', async () => {
    const payload = new TextEncoder().encode(JSON.stringify({ bodies: [], components: [], instances: [] }));
