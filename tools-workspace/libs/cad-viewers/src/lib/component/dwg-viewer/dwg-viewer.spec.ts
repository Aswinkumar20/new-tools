...
import { DwgViewerComponent } from './dwg-viewer';
import { buildSampleDwBytes, createDwFileRecord, createSampleDwFile } from '../../utils/dwg-viewer.utils';

@Component({ selector: 'lib-navigation', standalone: true, template: '' })
class StubNavigationComponent {}

describe('DwgViewerComponent', () => {
  let component: DwgViewerComponent;
  let fixture: ComponentFixture<DwgViewerComponent>;
  ...
  beforeEach(async () => {
    ...
    await TestBed.configureTestingModule({
      ...
      .overrideComponent(DwgViewerComponent, {
        remove: { imports: [Navigation] },
        add: { imports: [StubNavigationComponent] }
      })
      .compileComponents();

    fixture = TestBed.createComponent(DwgViewerComponent);
  ...
  it('loads sample via handleFiles and enables export', async () => {
    ...
    expect(component.currentFile?.name).toBe('office-l2.dwg');
  ...
  it('selectRow maps row name to an entity, measurement, or layer', () => {
    loadSampleRecord();
    component.setViewMode('table');
    expect(component.filteredRows.length).toBeGreaterThan(0);
    component.selectRow(0);
    expect(component.selectedEntityId || component.selectedMeasId || component.selectedLayerId).toBeTruthy();
  });

  it('selectEntity and selectMeas sync the matching layer', () => {
    loadSampleRecord();
  ...
  it('removeFile clears when last file removed and updates index otherwise', async () => {
    await component.loadSample();
    const second = createDwFileRecord(
      new File([buildSampleDwBytes()], 'second.dwg', { lastModified: 2 }),
      buildSampleDwBytes()
    );
  ...
  it('shows error suggestion after a hard load failure message', () => {
    component.errorMessage = 'bad.dwg: Invalid DWG dump';
  ...
  it('hasFilterHits is false when search matches nothing', () => {
    loadSampleRecord();
    component.setViewMode('preview');
    component.query = 'type:__none__';
    component.onFilterChange();
    expect(component.hasFilterHits).toBe(false);
  });
});
