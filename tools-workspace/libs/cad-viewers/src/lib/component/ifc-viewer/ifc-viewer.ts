...
import {
  ...
  IC_FORMATS_LABEL,
  IC_RELATED_TOOLS,
  IC_SUPPORTED_EXTENSIONS
} from '../../constants/ifc-viewer.constants';
import type { IcDiscipline, IcColumn, IcExportFormat, IcProperty, IcLoadedFile, IcElement, IcViewMode } from '../../types/ifc-viewer.types';
...
import {
  ...
  resolveIcSuggestion,
  toIcCad3d
} from '../../utils/ifc-viewer.utils';

@Component({
  selector: 'lib-ifc-viewer',
  standalone: true,
  templateUrl: './ifc-viewer.html',
  styleUrls: ['./ifc-viewer.scss'],
...
export class IfcViewerComponent implements AfterViewInit, OnDestroy {
  ...
  readonly acceptAttr = IC_ACCEPT_ATTR;
  readonly relatedTools = IC_RELATED_TOOLS;
  readonly supportedExtensions = IC_SUPPORTED_EXTENSIONS;
  readonly formatsLabel = IC_FORMATS_LABEL;
  readonly formatsHint = IC_FORMATS_HINT;
  readonly viewModes: Array<{ id: IcViewMode; label: string }> = [
  ...
  get canExport(): boolean {
    return canExportIc(this.currentFile);
  }
  ...
  get filteredElements(): IcElement[] {
    return this.parsed ? filterIcElements(this.parsed.elements, this.query) : [];
  }

  get filteredDisciplines(): IcDiscipline[] {
    return this.parsed ? filterIcDisciplines(this.parsed.disciplines, this.query) : [];
  }

  get filteredProperties(): IcProperty[] {
    return this.parsed ? filterIcProperties(this.parsed.properties, this.query) : [];
  }

  get filteredColumns(): IcColumn[] {
    return this.parsed?.columns ?? [];
  }

  get filteredRows(): Array<Record<string, string>> {
    return this.parsed ? filterIcRows(this.parsed.rows, this.query) : [];
  }

  get visibleElements(): IcElement[] {
    return this.filteredElements.filter((s) => !this.hiddenDisciplineIds.has(s.discipline));
  }

  get selectedElement(): IcElement | null {
    return this.filteredElements.find((s) => s.id === this.selectedElementId) ?? null;
  }

  get selectedDiscipline(): IcDiscipline | null {
    return this.filteredDisciplines.find((e) => e.id === this.selectedDiscId) ?? null;
  }

  get selectedProp(): IcProperty | null {
    return this.filteredProperties.find((inst) => inst.id === this.selectedPropId) ?? null;
  }

  get metadataRows() {
    return this.parsed ? buildIcMetadataRows(this.parsed) : [];
  }

  get elementMetadataRows() {
    return this.selectedElement ? buildIcElementMetadata(this.selectedElement) : [];
  }

  get discMetadataRows() {
    return this.selectedDiscipline ? buildIcDisciplineMetadata(this.selectedDiscipline) : [];
  }

  get propMetadataRows() {
    return this.selectedProp ? buildIcPropertyMetadata(this.selectedProp) : [];
  }

  get hasSelection(): boolean {
    return !!(this.selectedDiscId || this.selectedElementId || this.selectedPropId);
  }

  get primarySuggestion() {
    const s = resolveIcSuggestion({ hasFiles: this.files.length > 0, hasError: !!this.errorMessage });
  ...
  trackByElement(_i: number, part: IcElement): string {
    return part.id;
  }

  trackByDiscipline(_i: number, assembly: IcDiscipline): string {
    return assembly.id;
  }

  trackByProp(_i: number, instance: IcProperty): string {
    return instance.id;
  }

  trackByColumn(_i: number, column: IcColumn): string {
    return column.id;
  }
  ...
  async handleFiles(files: File[]): Promise<void> {
    const { accepted, rejected } = filterValidIcFiles(files);
    ...
    try {
      for (const file of accepted) {
        try {
          const bytes = await readIcFileBytes(file);
          const record = createIcFileRecord(file, bytes);
          ...
          this.errorMessage = `${file.name}: ${error instanceof Error ? error.message : 'Invalid IFC dump'}`;
  ...
  async loadSample(): Promise<void> {
    await this.handleFiles([createSampleIcFile()]);
  }
  ...
  onCanvasPointerUp(event?: PointerEvent): void {
    ...
    const id = pickCad3dSolidAtScreen(toIcCad3d(this.visibleElements), this.view, canvas.width, canvas.height, sx, sy);
  ...
  private shiftProp(delta: number): void {
    ...
    const next = list[Math.min(list.length - 1, Math.max(0, idx + delta))];
    if (next) this.selectProp(next.id);
  }
  ...
  private renderCanvas(): void {
    ...
    if (this.viewMode === 'disciplines') {
      renderIcDisciplines(canvas, this.filteredDisciplines, this.selectedDiscId || null);
      return;
    }
