...
import { buildSampleIcBytes, createIcFileRecord, createSampleIcFile } from '../../utils/ifc-viewer.utils';
...
describe('IfcViewerComponent', () => {
  ...
  beforeEach(async () => {
    ...
    await TestBed.configureTestingModule({
      ...
      .overrideComponent(IfcViewerComponent, {
  ...
  it('loads sample via handleFiles and enables export', async () => {
    ...
    expect(component.currentFile?.name).toBe('library-annex.ifc');
  ...
  it('isolates selected discipline and showAllDisciplines restores visibility', () => {
    ...
    expect(component.isDiscHidden(String(discs[1].name))).toBe(true);
  ...
  it('toggles discipline visibility without changing selection', () => {
    loadSampleRecord();
    const name = String(component.parsed!.disciplines[0].name);
    const event = { stopPropagation: jest.fn() } as unknown as Event;
    component.toggleDiscVisible(name, event);
    expect(event.stopPropagation).toHaveBeenCalled();
    expect(component.isDiscHidden(name)).toBe(true);
    component.toggleDiscVisible(name, event);
    expect(component.isDiscHidden(name)).toBe(false);
  });
  ...
  it('selectRow maps row name to element/discipline/property ids', () => {
    loadSampleRecord();
    component.setViewMode('table');
  ...
  it('selectProp links a matching element when present', () => {
    loadSampleRecord();
    const prop = component.filteredProperties[0];
    expect(prop).toBeTruthy();
    component.selectProp(prop.id);
  ...
  it('removeFile clears when last file removed and updates index otherwise', async () => {
    await component.loadSample();
    const second = createIcFileRecord(
      new File([buildSampleIcBytes()], 'second.ifc', { lastModified: 2 }),
      buildSampleIcBytes()
    );
  ...
  it('shows error suggestion after a hard load failure message', () => {
    component.errorMessage = 'bad.ifc: Invalid IFC dump';
  ...
  it('hides elements for hidden disciplines in visibleElements', () => {
    loadSampleRecord();
    const discName = String(component.parsed!.disciplines[0].name);
    const before = component.visibleElements.length;
    component.hiddenDisciplineIds = new Set([discName]);
    expect(component.visibleElements.length).toBeLessThanOrEqual(before);
    expect(component.visibleElements.every((e) => e.discipline !== discName)).toBe(true);
  });
});
