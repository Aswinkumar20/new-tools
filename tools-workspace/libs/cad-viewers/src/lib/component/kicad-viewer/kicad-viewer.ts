...
import {
  ...
  KC_FORMATS_LABEL,
  KC_RELATED_TOOLS,
  KC_SUPPORTED_EXTENSIONS
} from '../../constants/kicad-viewer.constants';
import type { KcBoardItem, KcColumn, KcExportFormat, KcLayer, KcLoadedFile, KcSchItem, KcViewMode } from '../../types/kicad-viewer.types';
import {
...
import {
  ...
  toKcBoardGeom,
  toKcSchGeom
} from '../../utils/kicad-viewer.utils';

@Component({
  selector: 'lib-kicad-viewer',
  standalone: true,
  templateUrl: './kicad-viewer.html',
  styleUrls: ['./kicad-viewer.scss'],
  imports: [CommonModule, FormsModule, RouterLink, Navigation, TooltipDirective, StatValueTooltipHostDirective],
  changeDetection: ChangeDetectionStrategy.OnPush
})
export class KiCadViewerComponent implements AfterViewInit, OnDestroy {
  ...
  readonly acceptAttr = KC_ACCEPT_ATTR;
  readonly relatedTools = KC_RELATED_TOOLS;
  readonly supportedExtensions = KC_SUPPORTED_EXTENSIONS;
  readonly formatsLabel = KC_FORMATS_LABEL;
  readonly formatsHint = KC_FORMATS_HINT;
  readonly viewModes: Array<{ id: KcViewMode; label: string }> = [
  ...
  get canExport(): boolean {
    return canExportKc(this.currentFile);
  }
  ...
  get filteredLayers(): KcLayer[] {
    return this.parsed ? filterKcLayers(this.parsed.layers, this.query) : [];
  }

  get filteredBoardItems(): KcBoardItem[] {
    return this.parsed ? filterKcBoardItems(this.parsed.boardItems, this.query) : [];
  }

  get filteredSchItems(): KcSchItem[] {
    return this.parsed ? filterKcSchItems(this.parsed.schItems, this.query) : [];
  }

  get filteredColumns(): KcColumn[] {
    return this.parsed?.columns ?? [];
  }

  get filteredRows(): Array<Record<string, string>> {
    return this.parsed ? filterKcRows(this.parsed.rows, this.query) : [];
  }
  ...
  get metadataRows() {
    return this.parsed ? buildKcMetadataRows(this.parsed) : [];
  }

  get layerMetadataRows() {
    return this.selectedLayer ? buildKcLayerMetadata(this.selectedLayer) : [];
  }

  get boardMetadataRows() {
    return this.selectedBoard ? buildKcBoardMetadata(this.selectedBoard) : [];
  }

  get schMetadataRows() {
    return this.selectedSch ? buildKcSchMetadata(this.selectedSch) : [];
  }
  ...
  get primarySuggestion() {
    const s = resolveKcSuggestion({ hasFiles: this.files.length > 0, hasError: !!this.errorMessage });
  ...
  trackByLayer(_i: number, layer: KcLayer): string {
    return layer.id;
  }

  trackByBoard(_i: number, item: KcBoardItem): string {
    return item.id;
  }

  trackBySch(_i: number, item: KcSchItem): string {
    return item.id;
  }

  trackByColumn(_i: number, column: KcColumn): string {
    return column.id;
  }
  ...
  async handleFiles(files: File[]): Promise<void> {
    const { accepted, rejected } = filterValidKcFiles(files);
    ...
    try {
      for (const file of accepted) {
        try {
          const bytes = await readKcFileBytes(file);
          const record = createKcFileRecord(file, bytes);
          ...
          this.errorMessage = `${file.name}: ${error instanceof Error ? error.message : 'Invalid KiCad dump'}`;
  ...
  async loadSample(): Promise<void> {
    await this.handleFiles([createSampleKcFile()]);
  }
  ...
  exportAs(format: KcExportFormat, event: Event): void {
    event.stopPropagation();
    this.showExportMenu = false;
  ...
  private renderCanvas(): void {
    ...
    if (this.viewMode === 'schematic') renderKcSch(canvas, this.filteredSchItems, this.plotSelectedId, this.view);
    else renderKcBoard(canvas, this.visibleBoardItems, this.plotSelectedId, this.view);
  }
