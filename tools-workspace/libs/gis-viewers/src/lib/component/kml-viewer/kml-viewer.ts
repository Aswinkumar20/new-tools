...
import {
  ...
  summarizeFeatures
} from '../../utils/kml-viewer.utils';

@Component({
  selector: 'lib-kml-viewer',
  standalone: true,
  templateUrl: './kml-viewer.html',
  styleUrls: ['./kml-viewer.scss'],
  imports: [CommonModule, RouterLink, Navigation, TooltipDirective],
  changeDetection: ChangeDetectionStrategy.OnPush
})
export class KmlViewerComponent implements AfterViewInit, OnDestroy {
  ...
  readonly acceptAttr = KML_ACCEPT_ATTR;
  readonly relatedTools = KML_RELATED_TOOLS;
  readonly supportedExtensions = KML_SUPPORTED_EXTENSIONS;
  readonly formatsLabel = KML_FORMATS_LABEL;
  readonly formatsHint = KML_FORMATS_HINT;
  readonly formatPropertyValue = formatPropertyValue;
  readonly featureFilters: ReadonlyArray<{ id: KmlFeatureFilter; label: string }> = [
    { id: 'all', label: 'All' },
  ...
  featureFilter: KmlFeatureFilter = 'all';
  featureSearch = '';
  selectedFeatureId: string | null = null;
  zoomPercent = 100;

  features: KmlFeatureSummary[] = [];
  filteredFeatures: KmlFeatureSummary[] = [];
  kindCounts: Record<KmlFeatureFilter, number> = {
    all: 0,
    point: 0,
    line: 0,
    polygon: 0,
  ...
  stats: KmlDiagramStats | null = null;
  ...
  get currentFile(): KmlLoadedFile | null {
    return this.currentFileIndex >= 0 ? this.kmlFiles[this.currentFileIndex] ?? null : null;
  }
  ...
  get selectedFeature(): KmlFeatureSummary | null {
  ...
  get primarySuggestion() {
    const suggestion = resolveKmlSuggestion({
      hasFiles: this.kmlFiles.length > 0,
  ...
  ngAfterViewInit(): void {
    ...
    ensureKmlStylesheet(this.assetService.getAssetPath('leaflet/leaflet.css'));
  ...
  trackByFileId(_index: number, file: KmlLoadedFile): string {
    return file.id;
  }
  ...
  trackByFeatureId(_index: number, feature: KmlFeatureSummary): string {
    return feature.id;
  }
  ...
  formatSize(bytes: number): string {
    return formatKmlFileSize(bytes);
  }
  ...
  async handleFiles(files: File[]): Promise<void> {
    const { accepted, rejected } = filterValidKmlFiles(files);
    ...
    try {
      for (const file of accepted) {
        const text = await readKmlFileText(file);
        let parsed;
        try {
          parsed = await parseKmlText(text);
        } catch (error) {
          const message = error instanceof Error ? error.message : 'Invalid KML';
        ...
        const existing = this.kmlFiles.findIndex((item) => item.id === record.id);
        if (existing >= 0) {
          this.kmlFiles[existing] = record;
          this.currentFileIndex = existing;
        } else {
          this.kmlFiles = [...this.kmlFiles, record];
          this.currentFileIndex = this.kmlFiles.length - 1;
        }
      }
      ...
      this.errorMessage = error instanceof Error ? error.message : 'Failed to load KML file';
      this.toast.error(this.errorMessage);
    } finally {
      this.loading = false;
      this.cdr.markForCheck();
  ...
  async loadSample(): Promise<void> {
    const sample = new File([KML_SAMPLE], 'sample-bay-area.kml', {
  ...
  async selectFile(index: number): Promise<void> {
    if (index < 0 || index >= this.kmlFiles.length || index === this.currentFileIndex) {
      return;
    }
  ...
  removeFile(index: number, event: Event): void {
    ...
    if (index < 0 || index >= this.kmlFiles.length) {
      return;
    }
    const next = this.kmlFiles.filter((_, i) => i !== index);
    this.kmlFiles = next;
    if (next.length === 0) {
      this.clearAll();
      return;
  ...
  setFeatureFilter(filter: KmlFeatureFilter): void {
  ...
  focusFeature(feature: KmlFeatureSummary): void {
  ...
  exportAs(format: KmlExportFormat, event?: Event): void {
    event?.stopPropagation();
    this.showExportMenu = false;
    ...
    const base = current.name.replace(/\.(kml|xml)$/i, '') || 'kml';
    try {
      if (format === 'kml') {
        ...
        downloadTextFile(current.text, `${base}.kml`, 'application/vnd.google-earth.kml+xml');
  ...
  private refreshFilteredFeatures(): void {
    this.filteredFeatures = filterKmlFeatures(
      this.features,
      this.featureFilter,
      this.featureSearch
    );
  ...
  private async renderCurrentFile(): Promise<void> {
    ...
    this.stats = buildKmlStats(current.data, this.features, current.documentTitle);
    this.selectedFeatureId = null;
    this.refreshFilteredFeatures();
    ...
    this.layer = L.geoJSON(features as never, {
      ...
      onEachFeature: (feature, layer) => {
        ...
        const description = summary?.description
          ? `<div class="kml-popup__desc">${escapeHtml(summary.description)}</div>`
          : '';
        const props = Object.entries(summary?.properties ?? {})
          ...
          .join('');
        layer.bindPopup(
          `<div class="kml-popup"><strong>${escapeHtml(title)}</strong><div class="kml-popup__type">${escapeHtml(summary?.geometryType ?? '')}</div>${description}${props}</div>`
        );
        layer.on('click', () => {
