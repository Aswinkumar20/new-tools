...
import { YamlViewerComponent } from './yaml-viewer';
import { buildSampleYamlBytes, createSampleYlFile, createYlFileRecord } from '../../utils/yaml-viewer.utils';
...
describe('YamlViewerComponent', () => {
  ...
  beforeEach(async () => {
    ...
    await TestBed.configureTestingModule({
      ...
      .overrideComponent(YamlViewerComponent, {
  ...
  it('loads sample via handleFiles and enables export', async () => {
    ...
    expect(component.currentFile?.name).toBe('k8s-deploy.yaml');
  ...
  it('selects node, issue, and row', () => {
    loadSampleRecord();
    const node = component.parsed!.nodes[0];
    component.selectNode(node.id);
    expect(component.selectedNodeId).toBe(node.id);
    const issue = component.parsed!.issues[0];
    if (issue) {
      component.selectIssue(issue.id);
      expect(component.selectedIssueId).toBe(issue.id);
    }
  ...
  it('switches view modes and toggles sidebar', () => {
    ...
    component.setViewMode('validate');
    expect(component.viewMode).toBe('validate');
  ...
  it('removeFile clears when last file removed and updates index otherwise', async () => {
    await component.loadSample();
    const second = createYlFileRecord(
      new File([buildSampleYamlBytes()], 'second.yaml', { lastModified: 2 }),
  ...
  it('shows error suggestion after a hard load failure message', () => {
    component.errorMessage = 'bad.yaml: Invalid YAML file';
  ...
  it('keeps scroll-owner structure for map wrap, cards, table, and sidebar', async () => {
    ...
    expect(root.querySelector('.yl-map-wrap')).toBeTruthy();
    expect(root.querySelector('.yl-workspace')).toBeTruthy();
    expect(root.querySelector('.yl-sidebar')).toBeTruthy();
    component.setViewMode('table');
    fixture.detectChanges();
    expect(root.querySelector('.yl-table')).toBeTruthy();
  });

  it('collapses sidebar via toggle and keeps toolbar present', () => {
    loadSampleRecord();
    expect(fixture.debugElement.query(By.css('.yl-toolbar'))).toBeTruthy();
    component.toggleSidebar();
    fixture.detectChanges();
    expect(component.sidebarCollapsed).toBe(true);
    expect((fixture.nativeElement as HTMLElement).querySelector('.yl-workspace--sidebar-collapsed')).toBeTruthy();
  });
});
