...
import { TomlViewerComponent } from './toml-viewer';
import { buildSampleTomlBytes, createSampleTmFile, createTmFileRecord } from '../../utils/toml-viewer.utils';

@Component({ selector: 'lib-navigation', standalone: true, template: '' })
class StubNavigationComponent {}

describe('TomlViewerComponent', () => {
  let component: TomlViewerComponent;
  let fixture: ComponentFixture<TomlViewerComponent>;
  ...
  beforeEach(async () => {
    ...
    await TestBed.configureTestingModule({
      ...
      .overrideComponent(TomlViewerComponent, {
        remove: { imports: [Navigation] },
        add: { imports: [StubNavigationComponent] }
      })
      .compileComponents();

    fixture = TestBed.createComponent(TomlViewerComponent);
  ...
  it('loads sample via handleFiles and enables export', async () => {
    ...
    expect(component.currentFile?.name).toBe('cargo-config.toml');
  ...
  it('removeFile clears when last file removed and updates index otherwise', async () => {
    await component.loadSample();
    const second = createTmFileRecord(
      new File([buildSampleTomlBytes()], 'second.toml', { lastModified: 2 }),
  ...
  it('shows error suggestion after a hard load failure message', () => {
    component.errorMessage = 'bad.toml: Invalid TOML file';
  ...
  it('keeps scroll-owner structure for map wrap, cards, grid, and sidebar', async () => {
    ...
    expect(root.querySelector('.tm-map-wrap')).toBeTruthy();
    expect(root.querySelector('.tm-workspace')).toBeTruthy();
    expect(root.querySelector('.tm-sidebar')).toBeTruthy();
    component.setViewMode('table');
    fixture.detectChanges();
    expect(root.querySelector('.tm-grid')).toBeTruthy();
  });

  it('collapses sidebar via toggle and keeps toolbar present', () => {
    loadSampleRecord();
    expect(fixture.debugElement.query(By.css('.tm-toolbar'))).toBeTruthy();
    component.toggleSidebar();
    fixture.detectChanges();
    expect(component.sidebarCollapsed).toBe(true);
    expect((fixture.nativeElement as HTMLElement).querySelector('.tm-workspace--sidebar-collapsed')).toBeTruthy();
  });
});
