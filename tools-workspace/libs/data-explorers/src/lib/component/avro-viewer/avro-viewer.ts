...
export class AvroViewerComponent implements AfterViewInit, OnDestroy {
  ...
  onFilterChange(): void {
    if (this.selectedFieldId && !this.filteredFields.some((f) => f.id === this.selectedFieldId)) {
      this.selectedFieldId = this.filteredFields[0]?.id ?? '';
    }
    if (this.selectedRecordId && !this.filteredRecords.some((r) => r.id === this.selectedRecordId)) {
