...
import { AvroViewerComponent } from './avro-viewer';
import { buildSampleAvroBytes, createAvFileRecord, createSampleAvFile } from '../../utils/avro-viewer.utils';
...
describe('AvroViewerComponent', () => {
  ...
  beforeEach(async () => {
    ...
    await TestBed.configureTestingModule({
      ...
      .overrideComponent(AvroViewerComponent, {
  ...
  it('loads sample via handleFiles and enables export', async () => {
    ...
    expect(component.currentFile?.name).toBe('clickstream.avro');
    expect(component.canExport).toBe(true);
    expect(component.parsed?.fields.length).toBeGreaterThan(0);
  ...
  it('filters fields and repairs selection when filter removes current item', () => {
    loadSampleRecord();
    const first = component.filteredFields[0];
    expect(first).toBeTruthy();
    component.selectedFieldId = first.id;
    component.query = 'field:__none__';
    component.onFilterChange();
    expect(component.filteredFields.length).toBe(0);
    expect(component.selectedFieldId).toBe('');
  });

  it('selects field and record', () => {
    loadSampleRecord();
    const field = component.parsed!.fields[0];
    const record = component.parsed!.records[0];
    component.selectField(field.id);
    expect(component.selectedFieldId).toBe(field.id);
    component.selectRecord(record.id);
    expect(component.selectedRecordId).toBe(record.id);
  });
  ...
  it('switches view modes and toggles sidebar', () => {
    ...
    component.setViewMode('sample');
    expect(component.viewMode).toBe('sample');
  ...
  it('removeFile clears when last file removed and updates index otherwise', async () => {
    await component.loadSample();
    const second = createAvFileRecord(
      new File([buildSampleAvroBytes()], 'second.avro', { lastModified: 2 }),
      buildSampleAvroBytes()
    );
  ...
  it('shows error suggestion after a hard load failure message', () => {
    component.errorMessage = 'bad.avro: Invalid Avro file';
  ...
  it('soft-fail empty dump disables export', async () => {
    const payload = new TextEncoder().encode(JSON.stringify({ type: 'record', name: 'Empty', fields: [] }));
  ...
  it('keeps scroll-owner structure for map wrap, cards, table, and sidebar', async () => {
    ...
    expect(root.querySelector('.av-map-wrap')).toBeTruthy();
    expect(root.querySelector('.av-workspace')).toBeTruthy();
    expect(root.querySelector('.av-sidebar')).toBeTruthy();
    component.setViewMode('table');
    fixture.detectChanges();
    expect(root.querySelector('.av-table')).toBeTruthy();
  });

  it('collapses sidebar via toggle and keeps toolbar present', () => {
    loadSampleRecord();
    expect(fixture.debugElement.query(By.css('.av-toolbar'))).toBeTruthy();
    component.toggleSidebar();
    fixture.detectChanges();
    expect(component.sidebarCollapsed).toBe(true);
    expect((fixture.nativeElement as HTMLElement).querySelector('.av-workspace--sidebar-collapsed')).toBeTruthy();
  });
});
