...
import { buildSampleJsonBytes, createJnFileRecord, createSampleJnFile } from '../../utils/json-viewer.utils';
...
describe('JsonViewerComponent', () => {
  ...
  beforeEach(async () => {
    ...
    await TestBed.configureTestingModule({
      ...
      .overrideComponent(JsonViewerComponent, {
  ...
  it('loads sample via handleFiles and enables export', async () => {
    ...
    expect(component.currentFile?.name).toBe('api-users.json');
  ...
  it('selects node, schema, and row', () => {
    loadSampleRecord();
    const node = component.parsed!.nodes[0];
    component.selectNode(node.id);
    expect(component.selectedNodeId).toBe(node.id);
    const schema = component.parsed!.schema[0];
    component.selectSchema(schema.id);
    expect(component.selectedSchemaId).toBe(schema.id);
  ...
  it('removeFile clears when last file removed and updates index otherwise', async () => {
    await component.loadSample();
    const second = createJnFileRecord(
      new File([buildSampleJsonBytes()], 'second.json', { lastModified: 2 }),
  ...
  it('shows error suggestion after a hard load failure message', () => {
    component.errorMessage = 'bad.json: Invalid JSON file';
  ...
  it('keeps scroll-owner structure for map wrap, cards, table, and sidebar', async () => {
    ...
    expect(root.querySelector('.jn-map-wrap')).toBeTruthy();
    expect(root.querySelector('.jn-workspace')).toBeTruthy();
    expect(root.querySelector('.jn-sidebar')).toBeTruthy();
    component.setViewMode('table');
    fixture.detectChanges();
    expect(root.querySelector('.jn-table')).toBeTruthy();
  });

  it('collapses sidebar via toggle and keeps toolbar present', () => {
    loadSampleRecord();
    expect(fixture.debugElement.query(By.css('.jn-toolbar'))).toBeTruthy();
    component.toggleSidebar();
    fixture.detectChanges();
    expect(component.sidebarCollapsed).toBe(true);
    expect((fixture.nativeElement as HTMLElement).querySelector('.jn-workspace--sidebar-collapsed')).toBeTruthy();
  });
});
