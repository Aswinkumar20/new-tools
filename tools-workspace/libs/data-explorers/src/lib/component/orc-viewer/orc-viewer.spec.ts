...
import { OrcViewerComponent } from './orc-viewer';
import { buildSampleOrcBytes, createOrcFileRecord, createSampleOrcFile } from '../../utils/orc-viewer.utils';

@Component({ selector: 'lib-navigation', standalone: true, template: '' })
class StubNavigationComponent {}

describe('OrcViewerComponent', () => {
  let component: OrcViewerComponent;
  let fixture: ComponentFixture<OrcViewerComponent>;
  ...
  beforeEach(async () => {
    ...
    await TestBed.configureTestingModule({
      ...
      .overrideComponent(OrcViewerComponent, {
        remove: { imports: [Navigation] },
        add: { imports: [StubNavigationComponent] }
      })
      .compileComponents();

    fixture = TestBed.createComponent(OrcViewerComponent);
  ...
  it('loads sample via handleFiles and enables export', async () => {
    ...
    expect(component.currentFile?.name).toBe('hive-facts.orc');
  ...
  it('selects column, row, and stripe', () => {
    loadSampleRecord();
    ...
    component.selectStripe(0);
    expect(component.selectedStripeIndex).toBe(0);
  });
  ...
  it('switches view modes and toggles sidebar', () => {
    ...
    component.setViewMode('stripes');
    expect(component.viewMode).toBe('stripes');
  ...
  it('removeFile clears when last file removed and updates index otherwise', async () => {
    await component.loadSample();
    const second = createOrcFileRecord(
      new File([buildSampleOrcBytes()], 'second.orc', { lastModified: 2 }),
      buildSampleOrcBytes()
    );
  ...
  it('shows error suggestion after a hard load failure message', () => {
    component.errorMessage = 'bad.orc: Invalid ORC file';
  ...
  it('keeps scroll-owner structure for map wrap, cards, table, and sidebar', async () => {
    ...
    expect(root.querySelector('.orc-map-wrap')).toBeTruthy();
    expect(root.querySelector('.orc-workspace')).toBeTruthy();
    expect(root.querySelector('.orc-sidebar')).toBeTruthy();
    component.setViewMode('table');
    fixture.detectChanges();
    expect(root.querySelector('.orc-table')).toBeTruthy();
  });

  it('collapses sidebar via toggle and keeps toolbar present', () => {
    loadSampleRecord();
    expect(fixture.debugElement.query(By.css('.orc-toolbar'))).toBeTruthy();
    component.toggleSidebar();
    fixture.detectChanges();
    expect(component.sidebarCollapsed).toBe(true);
    expect((fixture.nativeElement as HTMLElement).querySelector('.orc-workspace--sidebar-collapsed')).toBeTruthy();
  });
});
