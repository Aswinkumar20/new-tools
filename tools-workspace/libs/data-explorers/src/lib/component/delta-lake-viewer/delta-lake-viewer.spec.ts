...
import { DeltaLakeViewerComponent } from './delta-lake-viewer';
import { buildSampleDeltaBytes, createDlFileRecord, createSampleDlFile } from '../../utils/delta-lake-viewer.utils';

@Component({ selector: 'lib-navigation', standalone: true, template: '' })
class StubNavigationComponent {}

describe('DeltaLakeViewerComponent', () => {
  let component: DeltaLakeViewerComponent;
  let fixture: ComponentFixture<DeltaLakeViewerComponent>;
  ...
  beforeEach(async () => {
    ...
    await TestBed.configureTestingModule({
      imports: [DeltaLakeViewerComponent],
      providers: [
        provideRouter([]),
      ...
      .overrideComponent(DeltaLakeViewerComponent, {
        remove: { imports: [Navigation] },
        add: { imports: [StubNavigationComponent] }
      })
      .compileComponents();

    fixture = TestBed.createComponent(DeltaLakeViewerComponent);
  ...
  it('loads sample via handleFiles and enables export', async () => {
    ...
    expect(component.currentFile?.name).toBe('event-log.delta');
    expect(component.canExport).toBe(true);
    expect(component.parsed?.columns.length).toBeGreaterThan(0);
    expect(component.parsed?.versions.length).toBeGreaterThan(0);
  ...
  it('selects column, row, and version', () => {
    loadSampleRecord();
    ...
    component.selectVersion(1);
    expect(component.selectedVersionIndex).toBe(1);
  });
  ...
  it('setViewMode is a no-op when mode unchanged', () => {
    loadSampleRecord();
    component.viewMode = 'versions';
    component.setViewMode('versions');
    expect(component.viewMode).toBe('versions');
  });
  ...
  it('removeFile clears when last file removed and updates index otherwise', async () => {
    await component.loadSample();
    const second = createDlFileRecord(
      new File([buildSampleDeltaBytes()], 'second.delta', { lastModified: 2 }),
  ...
  it('shows error suggestion after a hard load failure message', () => {
    component.errorMessage = 'bad.delta: Invalid Delta dump';
  ...
  it('soft-fail empty dump disables export', async () => {
    const payload = new TextEncoder().encode(JSON.stringify({ columns: [], rows: [], versions: [] }));
  ...
  it('keeps scroll-owner structure for map wrap, cards, table, and sidebar', async () => {
    ...
    expect(root.querySelector('.dl-map-wrap')).toBeTruthy();
    expect(root.querySelector('.dl-workspace')).toBeTruthy();
    expect(root.querySelector('.dl-sidebar')).toBeTruthy();
    component.setViewMode('table');
    fixture.detectChanges();
    expect(root.querySelector('.dl-table')).toBeTruthy();
  });

  it('collapses sidebar via toggle and keeps toolbar present', () => {
    loadSampleRecord();
    expect(fixture.debugElement.query(By.css('.dl-toolbar'))).toBeTruthy();
    component.toggleSidebar();
    fixture.detectChanges();
    expect(component.sidebarCollapsed).toBe(true);
    expect((fixture.nativeElement as HTMLElement).querySelector('.dl-workspace--sidebar-collapsed')).toBeTruthy();
  });
});
