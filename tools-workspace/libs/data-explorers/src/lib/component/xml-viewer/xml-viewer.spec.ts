...
import { buildSampleXmlBytes, createSampleXmFile, createXmFileRecord } from '../../utils/xml-viewer.utils';
...
describe('XmlViewerComponent', () => {
  ...
  beforeEach(async () => {
    ...
    await TestBed.configureTestingModule({
      ...
      .overrideComponent(XmlViewerComponent, {
  ...
  it('loads sample via handleFiles and enables export', async () => {
    ...
    expect(component.currentFile?.name).toBe('catalog-feed.xml');
  ...
  it('selects node, attribute, and row', () => {
    loadSampleRecord();
    ...
    if (attr) {
      component.selectAttr(attr.id);
      expect(component.selectedAttrId).toBe(attr.id);
    }
  ...
  it('removeFile clears when last file removed and updates index otherwise', async () => {
    await component.loadSample();
    const second = createXmFileRecord(
      new File([buildSampleXmlBytes()], 'second.xml', { lastModified: 2 }),
      buildSampleXmlBytes()
    );
  ...
  it('shows error suggestion after a hard load failure message', () => {
    component.errorMessage = 'bad.xml: Invalid XML file';
  ...
  it('keeps scroll-owner structure for map wrap, cards, table, and sidebar', async () => {
    ...
    expect(root.querySelector('.xm-map-wrap')).toBeTruthy();
    expect(root.querySelector('.xm-workspace')).toBeTruthy();
    expect(root.querySelector('.xm-sidebar')).toBeTruthy();
    component.setViewMode('table');
    fixture.detectChanges();
    expect(root.querySelector('.xm-table')).toBeTruthy();
  });

  it('collapses sidebar via toggle and keeps toolbar present', () => {
    loadSampleRecord();
    expect(fixture.debugElement.query(By.css('.xm-toolbar'))).toBeTruthy();
    component.toggleSidebar();
    fixture.detectChanges();
    expect(component.sidebarCollapsed).toBe(true);
    expect((fixture.nativeElement as HTMLElement).querySelector('.xm-workspace--sidebar-collapsed')).toBeTruthy();
  });
});
