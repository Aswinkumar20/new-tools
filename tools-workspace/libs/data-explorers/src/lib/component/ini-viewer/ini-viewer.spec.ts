...
import { IniViewerComponent } from './ini-viewer';
import { buildSampleIniBytes, createInFileRecord, createSampleInFile } from '../../utils/ini-viewer.utils';

@Component({ selector: 'lib-navigation', standalone: true, template: '' })
class StubNavigationComponent {}

describe('IniViewerComponent', () => {
  let component: IniViewerComponent;
  let fixture: ComponentFixture<IniViewerComponent>;
  ...
  beforeEach(async () => {
    ...
    await TestBed.configureTestingModule({
      ...
      .overrideComponent(IniViewerComponent, {
        remove: { imports: [Navigation] },
        add: { imports: [StubNavigationComponent] }
      })
      .compileComponents();

    fixture = TestBed.createComponent(IniViewerComponent);
  ...
  it('loads sample via handleFiles and enables export', async () => {
    ...
    expect(component.currentFile?.name).toBe('app-config.ini');
    expect(component.canExport).toBe(true);
    expect(component.parsed?.sections.length).toBeGreaterThan(0);
  ...
  it('filters keys and repairs selection when filter removes current item', () => {
    loadSampleRecord();
    const first = component.filteredKeys[0];
    expect(first).toBeTruthy();
    component.selectedKeyId = first.id;
    component.query = 'name:__none__';
    component.onFilterChange();
    expect(component.filteredKeys.length).toBe(0);
    expect(component.selectedKeyId).toBe('');
  });

  it('selects section, key, and row', () => {
    loadSampleRecord();
    component.selectSection(0);
  ...
  it('setViewMode is a no-op when mode unchanged', () => {
    loadSampleRecord();
    component.viewMode = 'sections';
    component.setViewMode('sections');
    expect(component.viewMode).toBe('sections');
  });
  ...
  it('removeFile clears when last file removed and updates index otherwise', async () => {
    await component.loadSample();
    const second = createInFileRecord(
      new File([buildSampleIniBytes()], 'second.ini', { lastModified: 2 }),
      buildSampleIniBytes()
    );
  ...
  it('shows error suggestion after a hard load failure message', () => {
    component.errorMessage = 'bad.ini: Invalid INI file';
  ...
  it('keeps scroll-owner structure for map wrap, cards, table, and sidebar', async () => {
    ...
    expect(root.querySelector('.in-map-wrap')).toBeTruthy();
    expect(root.querySelector('.in-workspace')).toBeTruthy();
    expect(root.querySelector('.in-sidebar')).toBeTruthy();
    component.setViewMode('table');
    fixture.detectChanges();
    expect(root.querySelector('.in-grid')).toBeTruthy();
  });

  it('collapses sidebar via toggle and keeps toolbar present', () => {
    loadSampleRecord();
    expect(fixture.debugElement.query(By.css('.in-toolbar'))).toBeTruthy();
    component.toggleSidebar();
    fixture.detectChanges();
    expect(component.sidebarCollapsed).toBe(true);
    expect((fixture.nativeElement as HTMLElement).querySelector('.in-workspace--sidebar-collapsed')).toBeTruthy();
  });
});
