...
import { CsvViewerComponent } from './csv-viewer';
import { buildSampleCsvBytes, createCvFileRecord, createSampleCvFile } from '../../utils/csv-viewer.utils';
...
describe('CsvViewerComponent', () => {
  ...
  beforeEach(async () => {
    ...
    await TestBed.configureTestingModule({
      ...
      .overrideComponent(CsvViewerComponent, {
  ...
  it('loads sample via handleFiles and enables export', async () => {
    ...
    expect(component.currentFile?.name).toBe('bakery-invoices.csv');
  ...
  it('removeFile clears when last file removed and updates index otherwise', async () => {
    await component.loadSample();
    const second = createCvFileRecord(
      new File([buildSampleCsvBytes()], 'second.csv', { lastModified: 2 }),
      buildSampleCsvBytes()
    );
  ...
  it('shows error suggestion after a hard load failure message', () => {
    component.errorMessage = 'bad.csv: Invalid CSV file';
  ...
  it('keeps scroll-owner structure for map wrap, cards, table, and sidebar', async () => {
    ...
    expect(root.querySelector('.cv-map-wrap')).toBeTruthy();
    expect(root.querySelector('.cv-workspace')).toBeTruthy();
    expect(root.querySelector('.cv-sidebar')).toBeTruthy();
    component.setViewMode('table');
    fixture.detectChanges();
    expect(root.querySelector('.cv-table')).toBeTruthy();
  });

  it('collapses sidebar via toggle and keeps toolbar present', () => {
    loadSampleRecord();
    expect(fixture.debugElement.query(By.css('.cv-toolbar'))).toBeTruthy();
    component.toggleSidebar();
    fixture.detectChanges();
    expect(component.sidebarCollapsed).toBe(true);
    expect((fixture.nativeElement as HTMLElement).querySelector('.cv-workspace--sidebar-collapsed')).toBeTruthy();
  });
});
