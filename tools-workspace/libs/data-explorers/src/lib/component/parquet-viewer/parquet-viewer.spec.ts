...
import { ParquetViewerComponent } from './parquet-viewer';
import { buildSampleParquetBytes, createPqFileRecord, createSamplePqFile } from '../../utils/parquet-viewer.utils';

@Component({ selector: 'lib-navigation', standalone: true, template: '' })
class StubNavigationComponent {}

describe('ParquetViewerComponent', () => {
  let component: ParquetViewerComponent;
  let fixture: ComponentFixture<ParquetViewerComponent>;
  ...
  beforeEach(async () => {
    ...
    await TestBed.configureTestingModule({
      imports: [ParquetViewerComponent],
      providers: [
        provideRouter([]),
      ...
      .overrideComponent(ParquetViewerComponent, {
        remove: { imports: [Navigation] },
        add: { imports: [StubNavigationComponent] }
      })
      .compileComponents();

    fixture = TestBed.createComponent(ParquetViewerComponent);
  ...
  it('loads sample via handleFiles and enables export', async () => {
    ...
    expect(component.currentFile?.name).toBe('nyc-taxi.parquet');
    expect(component.canExport).toBe(true);
    expect(component.parsed?.columns.length).toBeGreaterThan(0);
  ...
  it('selects column and row', () => {
    loadSampleRecord();
    const column = component.parsed!.columns[0];
  ...
  it('switches view modes and toggles sidebar', () => {
    ...
    component.setViewMode('profiling');
    expect(component.viewMode).toBe('profiling');
  ...
  it('removeFile clears when last file removed and updates index otherwise', async () => {
    await component.loadSample();
    const second = createPqFileRecord(
      new File([buildSampleParquetBytes()], 'second.parquet', { lastModified: 2 }),
      buildSampleParquetBytes()
    );
  ...
  it('shows error suggestion after a hard load failure message', () => {
    component.errorMessage = 'bad.parquet: Invalid Parquet file';
  ...
  it('keeps scroll-owner structure for map wrap, cards, table, and sidebar', async () => {
    ...
    expect(root.querySelector('.pq-map-wrap')).toBeTruthy();
    expect(root.querySelector('.pq-workspace')).toBeTruthy();
    expect(root.querySelector('.pq-sidebar')).toBeTruthy();
    component.setViewMode('table');
    fixture.detectChanges();
    expect(root.querySelector('.pq-table')).toBeTruthy();
  });

  it('collapses sidebar via toggle and keeps toolbar present', () => {
    loadSampleRecord();
    expect(fixture.debugElement.query(By.css('.pq-toolbar'))).toBeTruthy();
    component.toggleSidebar();
    fixture.detectChanges();
    expect(component.sidebarCollapsed).toBe(true);
    expect((fixture.nativeElement as HTMLElement).querySelector('.pq-workspace--sidebar-collapsed')).toBeTruthy();
  });
});
