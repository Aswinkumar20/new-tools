...
import { DuckdbViewerComponent } from './duckdb-viewer';
import { buildSampleDuckdbBytes, createDkFileRecord, createSampleDkFile } from '../../utils/duckdb-viewer.utils';

@Component({ selector: 'lib-navigation', standalone: true, template: '' })
class StubNavigationComponent {}

describe('DuckdbViewerComponent', () => {
  let component: DuckdbViewerComponent;
  let fixture: ComponentFixture<DuckdbViewerComponent>;
  ...
  beforeEach(async () => {
    ...
    await TestBed.configureTestingModule({
      ...
      .overrideComponent(DuckdbViewerComponent, {
        remove: { imports: [Navigation] },
        add: { imports: [StubNavigationComponent] }
      })
      .compileComponents();

    fixture = TestBed.createComponent(DuckdbViewerComponent);
  ...
  it('loads sample via handleFiles and enables export', async () => {
    ...
    expect(component.currentFile?.name).toBe('analytics.duckdb');
  ...
  it('selects table, column, and row', () => {
    loadSampleRecord();
    expect(component.filteredTables.length).toBeGreaterThan(1);
    component.selectDbTable(1);
    expect(component.selectedTableIndex).toBe(1);
  ...
  it('removeFile clears when last file removed and updates index otherwise', async () => {
    await component.loadSample();
    const second = createDkFileRecord(
      new File([buildSampleDuckdbBytes()], 'second.duckdb', { lastModified: 2 }),
  ...
  it('shows error suggestion after a hard load failure message', () => {
    component.errorMessage = 'bad.duckdb: Invalid DuckDB file';
  ...
  it('keeps scroll-owner structure for map wrap, cards, grid, and sidebar', async () => {
    ...
    expect(root.querySelector('.dk-map-wrap')).toBeTruthy();
    expect(root.querySelector('.dk-workspace')).toBeTruthy();
    expect(root.querySelector('.dk-sidebar')).toBeTruthy();
    component.setViewMode('table');
    fixture.detectChanges();
    expect(root.querySelector('.dk-grid')).toBeTruthy();
  });

  it('collapses sidebar via toggle and keeps toolbar present', () => {
    loadSampleRecord();
    expect(fixture.debugElement.query(By.css('.dk-toolbar'))).toBeTruthy();
    component.toggleSidebar();
    fixture.detectChanges();
    expect(component.sidebarCollapsed).toBe(true);
    expect((fixture.nativeElement as HTMLElement).querySelector('.dk-workspace--sidebar-collapsed')).toBeTruthy();
  });
});
