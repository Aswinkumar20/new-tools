...
import { TsvViewerComponent } from './tsv-viewer';
import { buildSampleTsvBytes, createSampleTvFile, createTvFileRecord } from '../../utils/tsv-viewer.utils';

@Component({ selector: 'lib-navigation', standalone: true, template: '' })
class StubNavigationComponent {}

describe('TsvViewerComponent', () => {
  let component: TsvViewerComponent;
  let fixture: ComponentFixture<TsvViewerComponent>;
  ...
  beforeEach(async () => {
    ...
    await TestBed.configureTestingModule({
      ...
      .overrideComponent(TsvViewerComponent, {
        remove: { imports: [Navigation] },
        add: { imports: [StubNavigationComponent] }
      })
      .compileComponents();

    fixture = TestBed.createComponent(TsvViewerComponent);
  ...
  it('loads sample via handleFiles and enables export', async () => {
    ...
    expect(component.currentFile?.name).toBe('blast-hits.tsv');
  ...
  it('selects column and row', () => {
    loadSampleRecord();
    const col = component.parsed!.columns[0];
    component.selectColumn(col.id);
    expect(component.selectedColumnId).toBe(col.id);
  ...
  it('switches view modes and toggles sidebar', () => {
    ...
    component.setViewMode('columns');
    expect(component.viewMode).toBe('columns');
  ...
  it('removeFile clears when last file removed and updates index otherwise', async () => {
    await component.loadSample();
    const second = createTvFileRecord(
      new File([buildSampleTsvBytes()], 'second.tsv', { lastModified: 2 }),
      buildSampleTsvBytes()
    );
  ...
  it('shows error suggestion after a hard load failure message', () => {
    component.errorMessage = 'bad.tsv: Invalid TSV file';
  ...
  it('keeps scroll-owner structure for map wrap, cards, table, and sidebar', async () => {
    ...
    expect(root.querySelector('.tv-map-wrap')).toBeTruthy();
    expect(root.querySelector('.tv-workspace')).toBeTruthy();
    expect(root.querySelector('.tv-sidebar')).toBeTruthy();
    component.setViewMode('table');
    fixture.detectChanges();
    expect(root.querySelector('.tv-table')).toBeTruthy();
  });

  it('collapses sidebar via toggle and keeps toolbar present', () => {
    loadSampleRecord();
    expect(fixture.debugElement.query(By.css('.tv-toolbar'))).toBeTruthy();
    component.toggleSidebar();
    fixture.detectChanges();
    expect(component.sidebarCollapsed).toBe(true);
    expect((fixture.nativeElement as HTMLElement).querySelector('.tv-workspace--sidebar-collapsed')).toBeTruthy();
  });
});
