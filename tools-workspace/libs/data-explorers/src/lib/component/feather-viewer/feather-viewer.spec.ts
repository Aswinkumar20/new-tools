...
import { FeatherViewerComponent } from './feather-viewer';
import { buildSampleFeatherBytes, createFtFileRecord, createSampleFtFile } from '../../utils/feather-viewer.utils';

@Component({ selector: 'lib-navigation', standalone: true, template: '' })
class StubNavigationComponent {}

describe('FeatherViewerComponent', () => {
  let component: FeatherViewerComponent;
  let fixture: ComponentFixture<FeatherViewerComponent>;
  ...
  beforeEach(async () => {
    ...
    await TestBed.configureTestingModule({
      imports: [FeatherViewerComponent],
      providers: [
        provideRouter([]),
      ...
      .overrideComponent(FeatherViewerComponent, {
        remove: { imports: [Navigation] },
        add: { imports: [StubNavigationComponent] }
      })
      .compileComponents();

    fixture = TestBed.createComponent(FeatherViewerComponent);
  ...
  it('loads sample via handleFiles and enables export', async () => {
    ...
    expect(component.currentFile?.name).toBe('pandas-metrics.feather');
  ...
  it('removeFile clears when last file removed and updates index otherwise', async () => {
    await component.loadSample();
    const second = createFtFileRecord(
      new File([buildSampleFeatherBytes()], 'second.feather', { lastModified: 2 }),
      buildSampleFeatherBytes()
    );
  ...
  it('shows error suggestion after a hard load failure message', () => {
    component.errorMessage = 'bad.feather: Invalid Feather file';
  ...
  it('keeps scroll-owner structure for map wrap, cards, table, and sidebar', async () => {
    ...
    expect(root.querySelector('.ft-map-wrap')).toBeTruthy();
    expect(root.querySelector('.ft-workspace')).toBeTruthy();
    expect(root.querySelector('.ft-sidebar')).toBeTruthy();
    component.setViewMode('table');
    fixture.detectChanges();
    expect(root.querySelector('.ft-table')).toBeTruthy();
  });

  it('collapses sidebar via toggle and keeps toolbar present', () => {
    loadSampleRecord();
    expect(fixture.debugElement.query(By.css('.ft-toolbar'))).toBeTruthy();
    component.toggleSidebar();
    fixture.detectChanges();
    expect(component.sidebarCollapsed).toBe(true);
    expect((fixture.nativeElement as HTMLElement).querySelector('.ft-workspace--sidebar-collapsed')).toBeTruthy();
  });
});
