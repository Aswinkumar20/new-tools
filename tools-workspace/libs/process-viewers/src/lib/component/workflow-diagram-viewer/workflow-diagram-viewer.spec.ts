...
import { WorkflowDiagramViewerComponent } from './workflow-diagram-viewer';
import { createSampleWorkflowFile, createWorkflowFileRecord } from '../../utils/workflow-diagram-viewer.utils';

@Component({ selector: 'lib-navigation', standalone: true, template: '' })
class StubNavigationComponent {}

describe('WorkflowDiagramViewerComponent', () => {
  let component: WorkflowDiagramViewerComponent;
  let fixture: ComponentFixture<WorkflowDiagramViewerComponent>;
  ...
  beforeEach(async () => {
    ...
    await TestBed.configureTestingModule({
      imports: [WorkflowDiagramViewerComponent],
      providers: [
        provideRouter([]),
      ...
      .overrideComponent(WorkflowDiagramViewerComponent, {
        remove: { imports: [Navigation] },
        add: { imports: [StubNavigationComponent] }
      })
      .compileComponents();

    fixture = TestBed.createComponent(WorkflowDiagramViewerComponent);
  ...
  function sampleBytes(): Uint8Array {
    return new TextEncoder().encode(WORKFLOW_XML_SAMPLE);
  }

  function loadSampleRecord(): void {
    const file = createSampleWorkflowFile();
    const record = createWorkflowFileRecord(file, sampleBytes());
  ...
  it('loads sample via handleFiles and enables export', async () => {
    ...
    expect(component.currentFile?.name).toBe('sample-support-ticket.xml');
    expect(component.canExport).toBe(true);
    expect(component.parsed?.nodes.length).toBe(8);
    expect(component.parsed?.edges.length).toBe(8);
  ...
  it('filters edges and repairs selection in edges mode', () => {
    loadSampleRecord();
    ...
    component.onFilterChange();
    expect(component.filteredEdges).toHaveLength(0);
  ...
  it('removeFile clears when last file removed and updates index otherwise', async () => {
    await component.loadSample();
    const second = createWorkflowFileRecord(new File([sampleBytes()], 'second.xml', { lastModified: 2 }), sampleBytes());
  ...
  it('keeps scroll-owner structure for map wrap, cards, table, and sidebar', async () => {
    ...
    expect(root.querySelector('.wf-map-wrap')).toBeTruthy();
    expect(root.querySelector('.wf-workspace')).toBeTruthy();
    expect(root.querySelector('.wf-sidebar')).toBeTruthy();
    component.setViewMode('table');
    fixture.detectChanges();
    expect(root.querySelector('.wf-table')).toBeTruthy();
  });

  it('collapses sidebar via toggle and keeps toolbar present', () => {
    loadSampleRecord();
    expect(fixture.debugElement.query(By.css('.wf-toolbar'))).toBeTruthy();
    component.toggleSidebar();
    fixture.detectChanges();
    expect(component.sidebarCollapsed).toBe(true);
    expect((fixture.nativeElement as HTMLElement).querySelector('.wf-workspace--sidebar-collapsed')).toBeTruthy();
    expect((fixture.nativeElement as HTMLElement).querySelector('.wf-sidebar')).toBeFalsy();
  });
});
