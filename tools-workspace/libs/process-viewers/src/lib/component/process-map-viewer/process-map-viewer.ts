...
export class ProcessMapViewerComponent implements AfterViewInit, OnDestroy {
  ...
  get selectedFlow(): ProcessMapFlow | null {
    return this.filteredFlows.find((f) => f.id === this.selectedFlowId) ?? null;
  }
    ...
    try {
      ...
      if (this.currentFile) {
        ...
        if (this.currentFile.softFail) {
          this.toast.warning('Parsed with little or no variants/activities — metadata may still be available');
  ...
  onFilterChange(): void {
    ...
    if (this.selectedFlowId && !this.filteredFlows.some((f) => f.id === this.selectedFlowId)) {
      this.selectedFlowId = this.filteredFlows[0]?.id ?? '';
    }
    this.renderCanvas();
    this.cdr.markForCheck();
  }
  ...
  private renderCanvas(): void {
    ...
    if (this.viewMode === 'variants') {
      renderProcessMapVariants(canvas, this.filteredVariants, this.selectedVariantId || null);
    } else if (this.viewMode === 'frequencies') {
      renderProcessMapFrequencies(canvas, this.filteredActivities, this.selectedActivityId || null);
    } else {
      renderProcessMapFlows(canvas, this.filteredFlows, this.selectedFlowId || null);
    }
  }
