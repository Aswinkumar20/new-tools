...
import { BpelViewerComponent } from './bpel-viewer';
import { createBpelFileRecord, createSampleBpelFile } from '../../utils/bpel-viewer.utils';

@Component({ selector: 'lib-navigation', standalone: true, template: '' })
class StubNavigationComponent {}

describe('BpelViewerComponent', () => {
  let component: BpelViewerComponent;
  let fixture: ComponentFixture<BpelViewerComponent>;
  ...
  beforeEach(async () => {
    ...
    await TestBed.configureTestingModule({
      ...
      .overrideComponent(BpelViewerComponent, {
        remove: { imports: [Navigation] },
        add: { imports: [StubNavigationComponent] }
      })
      .compileComponents();

    fixture = TestBed.createComponent(BpelViewerComponent);
  ...
  function sampleBytes(): Uint8Array {
    return new TextEncoder().encode(BPEL_XML_SAMPLE);
  }

  function loadSampleRecord(): void {
    const file = createSampleBpelFile();
    const record = createBpelFileRecord(file, sampleBytes());
  ...
  it('loads sample via handleFiles and enables export', async () => {
    ...
    expect(component.currentFile?.name).toBe('sample-loan-approval.bpel');
    expect(component.canExport).toBe(true);
    expect(component.parsed?.name).toBe('LoanApproval');
    expect(component.parsed?.partners.length).toBe(3);
    expect(component.parsed?.activities.length).toBeGreaterThan(0);
  ...
  it('filters partners and repairs selection in partners mode', () => {
    loadSampleRecord();
    component.setViewMode('partners');
    const first = component.filteredPartners[0];
    component.selectedPartnerId = first.id;
    component.query = 'partner:__none__';
    component.onFilterChange();
    expect(component.filteredPartners.length).toBe(0);
    expect(component.selectedPartnerId).toBe('');
  });

  it('selects activity and partner', () => {
    loadSampleRecord();
    const invoke = component.parsed!.activities.find((a) => a.kind === 'invoke')!;
    component.selectActivity(invoke.id);
    expect(component.selectedActivityId).toBe(invoke.id);
    const partner = component.parsed!.partners.find((p) => p.name === 'customer')!;
    component.selectPartner(partner.id);
    expect(component.selectedPartnerId).toBe(partner.id);
  });
  ...
  it('switches view modes and toggles sidebar', () => {
    ...
    expect(component.viewMode).toBe('activities');
  ...
  it('setViewMode is a no-op when mode unchanged', () => {
    loadSampleRecord();
    component.viewMode = 'orchestration';
    component.setViewMode('orchestration');
    expect(component.viewMode).toBe('orchestration');
  });
  ...
  it('removeFile clears when last file removed and updates index otherwise', async () => {
    await component.loadSample();
    const second = createBpelFileRecord(new File([sampleBytes()], 'second.bpel', { lastModified: 2 }), sampleBytes());
  ...
  it('shows error suggestion after a hard load failure message', () => {
    component.errorMessage = 'bad.bpel: Invalid BPEL process';
  ...
  it('keeps scroll-owner structure for map wrap, cards, table, and sidebar', async () => {
    ...
    expect(root.querySelector('.bpel-map-wrap')).toBeTruthy();
    expect(root.querySelector('.bpel-workspace')).toBeTruthy();
    expect(root.querySelector('.bpel-sidebar')).toBeTruthy();
    component.setViewMode('table');
    fixture.detectChanges();
    expect(root.querySelector('.bpel-table')).toBeTruthy();
  });

  it('collapses sidebar via toggle and keeps toolbar present', () => {
    loadSampleRecord();
    expect(fixture.debugElement.query(By.css('.bpel-toolbar'))).toBeTruthy();
    component.toggleSidebar();
    fixture.detectChanges();
    expect(component.sidebarCollapsed).toBe(true);
    expect((fixture.nativeElement as HTMLElement).querySelector('.bpel-workspace--sidebar-collapsed')).toBeTruthy();
    expect((fixture.nativeElement as HTMLElement).querySelector('.bpel-sidebar')).toBeFalsy();
  });
});
