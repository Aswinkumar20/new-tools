...
import { EpcDiagramViewerComponent } from './epc-diagram-viewer';
import { createEpcFileRecord, createSampleEpcFile } from '../../utils/epc-diagram-viewer.utils';

@Component({ selector: 'lib-navigation', standalone: true, template: '' })
class StubNavigationComponent {}

describe('EpcDiagramViewerComponent', () => {
  let component: EpcDiagramViewerComponent;
  let fixture: ComponentFixture<EpcDiagramViewerComponent>;
  ...
  beforeEach(async () => {
    ...
    await TestBed.configureTestingModule({
      imports: [EpcDiagramViewerComponent],
      providers: [
        provideRouter([]),
      ...
      .overrideComponent(EpcDiagramViewerComponent, {
        remove: { imports: [Navigation] },
        add: { imports: [StubNavigationComponent] }
      })
      .compileComponents();

    fixture = TestBed.createComponent(EpcDiagramViewerComponent);
  ...
  function sampleBytes(): Uint8Array {
    return new TextEncoder().encode(EPC_XML_SAMPLE);
  }

  function loadSampleRecord(): void {
    const file = createSampleEpcFile();
    const record = createEpcFileRecord(file, sampleBytes());
  ...
  it('loads sample via handleFiles and enables export', async () => {
    ...
    expect(component.currentFile?.name).toBe('sample-order-fulfillment.epc');
    expect(component.canExport).toBe(true);
    expect(component.parsed?.events.length).toBe(7);
    expect(component.parsed?.functions.length).toBe(6);
    expect(component.parsed?.flows.length).toBe(16);
  ...
  it('filters events in events mode and repairs selection', () => {
    loadSampleRecord();
    component.setViewMode('events');
    const first = component.filteredNodes[0];
  ...
  it('filters functions in functions mode', () => {
    loadSampleRecord();
    component.setViewMode('functions');
    expect(component.filteredNodes.every((n) => n.kind === 'function')).toBe(true);
    const first = component.filteredNodes[0];
    component.selectedNodeId = first.id;
  ...
  it('selects node', () => {
    loadSampleRecord();
    const node = component.parsed!.nodes.find((n) => /credit/i.test(n.name))!;
    component.selectNode(node.id);
    expect(component.selectedNodeId).toBe(node.id);
    expect(component.selectedNode?.name).toContain('credit');
  });
  ...
  it('removeFile clears when last file removed and updates index otherwise', async () => {
    await component.loadSample();
    const second = createEpcFileRecord(new File([sampleBytes()], 'second.epc', { lastModified: 2 }), sampleBytes());
  ...
  it('keeps scroll-owner structure for map wrap, cards, table, and sidebar', async () => {
    ...
    expect(root.querySelector('.epc-map-wrap')).toBeTruthy();
    expect(root.querySelector('.epc-workspace')).toBeTruthy();
    expect(root.querySelector('.epc-sidebar')).toBeTruthy();
    component.setViewMode('table');
    fixture.detectChanges();
    expect(root.querySelector('.epc-table')).toBeTruthy();
  });

  it('collapses sidebar via toggle and keeps toolbar present', () => {
    loadSampleRecord();
    expect(fixture.debugElement.query(By.css('.epc-toolbar'))).toBeTruthy();
    component.toggleSidebar();
    fixture.detectChanges();
    expect(component.sidebarCollapsed).toBe(true);
    expect((fixture.nativeElement as HTMLElement).querySelector('.epc-workspace--sidebar-collapsed')).toBeTruthy();
    expect((fixture.nativeElement as HTMLElement).querySelector('.epc-sidebar')).toBeFalsy();
  });
});
