...
import { BpmnAnalyticsViewerComponent } from './bpmn-analytics-viewer';
import { createBpmnAnalyticsFileRecord, createSampleBpmnAnalyticsFile } from '../../utils/bpmn-analytics-viewer.utils';

@Component({ selector: 'lib-navigation', standalone: true, template: '' })
class StubNavigationComponent {}

describe('BpmnAnalyticsViewerComponent', () => {
  let component: BpmnAnalyticsViewerComponent;
  let fixture: ComponentFixture<BpmnAnalyticsViewerComponent>;
  ...
  beforeEach(async () => {
    ...
    await TestBed.configureTestingModule({
      imports: [BpmnAnalyticsViewerComponent],
      providers: [
        provideRouter([]),
      ...
      .overrideComponent(BpmnAnalyticsViewerComponent, {
        remove: { imports: [Navigation] },
        add: { imports: [StubNavigationComponent] }
      })
      .compileComponents();

    fixture = TestBed.createComponent(BpmnAnalyticsViewerComponent);
  ...
  function sampleBytes(): Uint8Array {
    return new TextEncoder().encode(BPMN_ANALYTICS_JSON_SAMPLE);
  }

  function loadSampleRecord(): void {
    const file = createSampleBpmnAnalyticsFile();
    const record = createBpmnAnalyticsFileRecord(file, sampleBytes());
  ...
  it('loads sample via handleFiles and enables export', async () => {
    ...
    expect(component.currentFile?.name).toBe('sample-order-analytics.json');
    expect(component.canExport).toBe(true);
    expect(component.parsed?.processName).toBe('Order Fulfillment');
    expect(component.parsed?.activities.length).toBe(6);
    expect(component.bottleneckActivities.length).toBeGreaterThan(0);
  ...
  it('filters activities and repairs selection when filter removes current item', () => {
    loadSampleRecord();
    const first = component.filteredActivities[0];
    expect(first).toBeTruthy();
    component.selectedId = first.id;
  ...
  it('filters bottlenecks by severity token', () => {
    loadSampleRecord();
    component.query = 'critical';
    component.onFilterChange();
    expect(component.bottleneckActivities.every((a) => a.severity === 'critical')).toBe(true);
  });

  it('selects activity and updates selectedId', () => {
    loadSampleRecord();
    const backorder = component.parsed!.activities.find((a) => a.id === 'Task_Backorder')!;
    component.selectActivity(backorder.id);
    expect(component.selectedId).toBe(backorder.id);
    expect(component.selectedActivity?.name).toBe(backorder.name);
  });
  ...
  it('switches view modes and toggles sidebar', () => {
    ...
    component.setViewMode('overlays');
    expect(component.viewMode).toBe('overlays');
  ...
  it('setViewMode is a no-op when mode unchanged', () => {
    loadSampleRecord();
    component.viewMode = 'bottlenecks';
    component.setViewMode('bottlenecks');
    expect(component.viewMode).toBe('bottlenecks');
  });
  ...
  it('removeFile clears when last file removed and updates index otherwise', async () => {
    await component.loadSample();
    const second = createBpmnAnalyticsFileRecord(
      new File([sampleBytes()], 'second.json', { lastModified: 2 }),
      sampleBytes()
    );
  ...
  it('shows error suggestion after a hard load failure message', () => {
    component.errorMessage = 'bad.json: Invalid BPMN analytics file';
  ...
  it('keeps scroll-owner structure for map wrap, cards, table, and sidebar', async () => {
    ...
    expect(root.querySelector('.ban-map-wrap')).toBeTruthy();
    expect(root.querySelector('.ban-workspace')).toBeTruthy();
    expect(root.querySelector('.ban-sidebar')).toBeTruthy();
    component.setViewMode('table');
    fixture.detectChanges();
    expect(root.querySelector('.ban-table')).toBeTruthy();
  });

  it('collapses sidebar via toggle and keeps toolbar present', () => {
    loadSampleRecord();
    expect(fixture.debugElement.query(By.css('.ban-toolbar'))).toBeTruthy();
    component.toggleSidebar();
    fixture.detectChanges();
    expect(component.sidebarCollapsed).toBe(true);
    expect((fixture.nativeElement as HTMLElement).querySelector('.ban-workspace--sidebar-collapsed')).toBeTruthy();
    expect((fixture.nativeElement as HTMLElement).querySelector('.ban-sidebar')).toBeFalsy();
  });
});
