...
export function buDownloadBlob(blob: Blob, filename: string): void {
  if (!blob.size) {
...
export function buDownloadJson(data: unknown, filename: string): void {
  const payload = JSON.stringify(data, null, 2);
  if (!payload) {
    throw new Error('Cannot download empty file');
  }
  buDownloadBlob(new Blob([payload], { type: 'application/json' }), filename);
}

/** Build a filesystem-safe timestamp for download filenames. */
export function buDownloadTimestamp(date = new Date()): string {
