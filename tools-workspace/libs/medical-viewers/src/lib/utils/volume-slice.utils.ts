/** Shared 3D volume slice extraction (x fastest, y, z slowest — NIfTI/NRRD/MINC layout). */
...
/** Compute histogram bins for volume intensity distribution. */
export function computeVolumeHistogram(data: Float32Array, binCount = 32): VolumeHistogram {
  if (!data.length) {
...
export function minMaxVolume(data: Float32Array): { min: number; max: number } {
  let min = Infinity;
  let max = -Infinity;
  for (let i = 0; i < data.length; i++) {
    const v = data[i];
