...
import {
  ...
  downloadDataUrl,
  downloadTextFile,
  exportMriMetadataJson,
  exportMriSummaryJson,
  filterValidMriFiles,
  formatMriFileSize,
  probeMriPixel,
  readMriFileBytes,
  resolveMriSuggestion,
  sortSlices,
  groupBySeries,
  enrichMriFileRecord
} from '../../utils/mri-viewer.utils';
...
@Component({
  selector: 'lib-mri-viewer',
  standalone: true,
  templateUrl: './mri-viewer.html',
  styleUrls: ['./mri-viewer.scss'],
  imports: [CommonModule, RouterLink, Navigation, TooltipDirective],
  changeDetection: ChangeDetectionStrategy.OnPush
})
export class MriViewerComponent implements AfterViewInit, OnDestroy {
  ...
  readonly acceptAttr = MRI_ACCEPT_ATTR;
  readonly relatedTools = MRI_RELATED_TOOLS;
  readonly supportedExtensions = MRI_SUPPORTED_EXTENSIONS;
  readonly formatsLabel = MRI_FORMATS_LABEL;
  readonly formatsHint = MRI_FORMATS_HINT;
  readonly windowPresets = MRI_WINDOW_PRESETS;

  mriFiles: MriLoadedFile[] = [];
  currentFileIndex = -1;
  seriesGroups: MriSeriesGroup[] = [];
  activeSeriesIndex = 0;
  loading = false;
  errorMessage = '';
  showDropZone = false;
  ...
  get primarySuggestion() {
    const suggestion = resolveMriSuggestion({
      hasFiles: this.mriFiles.length > 0,
  ...
  get seriesFiles(): MriLoadedFile[] {
    return this.seriesGroups[this.activeSeriesIndex]?.files ?? this.mriFiles;
  }

  get activeSeries(): MriSeriesGroup | null {
  ...
  trackByFileId(_index: number, file: MriLoadedFile): string {
    return file.id;
  }
  ...
  formatSize(bytes: number): string {
    return formatMriFileSize(bytes);
  }
  ...
  async handleFiles(files: File[]): Promise<void> {
    const { accepted, rejected } = filterValidMriFiles(files);
    ...
    try {
      const loaded: MriLoadedFile[] = [];
      for (const file of accepted) {
        try {
          const bytes = await readMriFileBytes(file);
          loaded.push(enrichMriFileRecord(createMriFileRecord(file, bytes)));
        } catch (error) {
          const message = error instanceof Error ? error.message : 'Invalid MRI DICOM';
      ...
      if (loaded.length) {
        const merged = sortSlices([...this.mriFiles, ...loaded]);
        // de-dupe by id
        const byId = new Map<string, MriLoadedFile>();
        for (const item of merged) {
          byId.set(item.id, item);
        }
        this.mriFiles = sortSlices(Array.from(byId.values()));
        this.rebuildSeries(Math.max(0, this.mriFiles.length - loaded.length));
      ...
      this.errorMessage = error instanceof Error ? error.message : 'Failed to load MRI DICOM';
      this.toast.error(this.errorMessage);
    } finally {
      this.loading = false;
      this.cdr.markForCheck();
  ...
  async loadSample(): Promise<void> {
    await this.handleFiles([createSampleMriFile()]);
  }
  ...
  removeFile(index: number, event: Event): void {
    ...
    const next = this.mriFiles.filter((f) => f.id !== targetId);
    this.mriFiles = next;
    if (next.length === 0) {
      this.clearAll();
      return;
  ...
  private rebuildSeries(preferredFileIndex = 0): void {
    this.seriesGroups = groupBySeries(this.mriFiles);
  ...
  exportAs(format: MriExportFormat, event?: Event): void {
    event?.stopPropagation();
    this.showExportMenu = false;
    const current = this.currentFile;
    if (!current) {
    ...
    try {
      if (format === 'original') {
        ...
        downloadTextFile(exportMriMetadataJson(current), `${base}-metadata.json`, 'application/json');
        this.toast.success('Exported metadata JSON');
      } else if (format === 'summary-json') {
        downloadTextFile(exportMriSummaryJson(current), `${base}-summary.json`, 'application/json');
  ...
  onCanvasClick(event: MouseEvent): void {
    ...
    const hit = probeMriPixel(parsed, x, y);
    if (!hit) {
      this.probe = null;
    } else {
  ...
  private syncWindowFromCurrent(): void {
    const parsed = this.parsed;
    if (!parsed) return;
    const win = defaultWindowForMri(parsed);
