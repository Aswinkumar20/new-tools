...
import {
  ...
  downloadDataUrl,
  downloadTextFile,
  exportEegCalipersJson,
  exportEegSummaryJson,
  exportEegVisibleChannelsJson,
  filterValidEegFiles,
  formatWaveformFileSize,
  readWaveformFileBytes,
  resolveEegSuggestion,
  sortEegChannelIds
} from '../../utils/eeg-viewer.utils';
import { applyMontage, computeCaliper } from '../../utils/waveform-parse.utils';
import {
  clipboardFiles,
  clipboardText,
  fileFromPastedText,
...
@Component({
  selector: 'lib-eeg-viewer',
  standalone: true,
  templateUrl: './eeg-viewer.html',
  styleUrls: ['./eeg-viewer.scss'],
  imports: [CommonModule, RouterLink, Navigation, TooltipDirective],
  changeDetection: ChangeDetectionStrategy.OnPush
})
export class EegViewerComponent implements AfterViewInit, OnDestroy {
  ...
  readonly acceptAttr = EEG_ACCEPT_ATTR;
  readonly relatedTools = EEG_RELATED_TOOLS;
  readonly supportedExtensions = EEG_SUPPORTED_EXTENSIONS;
  readonly formatsLabel = EEG_FORMATS_LABEL;
  readonly formatsHint = EEG_FORMATS_HINT;
  readonly montagePresets = EEG_MONTAGE_PRESETS;
  readonly interactionModes: ReadonlyArray<{ id: EegInteractionMode; label: string }> = [
    { id: 'navigate', label: 'Navigate' },
    { id: 'caliper', label: 'Calipers' }
  ];

  recordings: EegLoadedRecording[] = [];
  currentIndex = -1;
  loading = false;
  errorMessage = '';
  showDropZone = false;
  ...
  interactionMode: EegInteractionMode = 'navigate';
  visibleChannelIds = new Set<string>();
  montageMode: WaveformMontageMode = 'referential';
  gain = 32;
  pixelsPerSecond = 100;
  startSample = 0;
  windowSamples = 768;
  ...
  private activeCaliper: WaveformCaliper | null = null;

  private dragDepth = 0;
  private resizeObserver: ResizeObserver | null = null;

  get currentRecording(): EegLoadedRecording | null {
    return this.currentIndex >= 0 ? this.recordings[this.currentIndex] ?? null : null;
  }

  get canExport(): boolean {
    return canExportEeg(this.currentRecording);
  }
  ...
  get primarySuggestion() {
    const suggestion = resolveEegSuggestion({
  ...
  get visibleChannelIdList(): string[] {
    const rec = this.currentRecording;
    if (!rec) return [];
    return sortEegChannelIds(rec.waveform.channels.map((c) => c.id)).filter((id) =>
      this.visibleChannelIds.has(id)
    );
  }

  get displayChannels(): WaveformChannel[] {
    const rec = this.currentRecording;
    if (!rec) return [];
    return applyMontage(rec.waveform, this.montageMode, this.visibleChannelIdList);
  }
  ...
  get durationLabel(): string {
    ...
    return `${startSec.toFixed(2)}–${endSec.toFixed(2)} s · ${rec.waveform.sampleRateHz} Hz · ${this.montageMode}`;
  }
  ...
  onKeydown(event: KeyboardEvent): void {
    ...
    if (event.key === '+' || event.key === '=') {
      ...
      event.preventDefault();
      if (this.caliperDraft || this.activeCaliper) {
  ...
  async onWindowPaste(event: ClipboardEvent): Promise<void> {
    ...
    const file = fileFromPastedText(
      text,
      text.trim().startsWith('{') ? 'pasted-eeg.json' : 'pasted-eeg.csv',
  ...
  trackByRecordingId(_index: number, rec: EegLoadedRecording): string {
    return rec.id;
  }
  ...
  trackByCaliperId(_index: number, cal: EegCaliperMark): string {
    return cal.id;
  }
  ...
  async handleFiles(files: File[]): Promise<void> {
    const { accepted, rejected } = filterValidEegFiles(files);
    ...
    try {
      const loaded: EegLoadedRecording[] = [];
      for (const file of accepted) {
        try {
          const bytes = await readWaveformFileBytes(file);
          loaded.push(createEegRecording(file, bytes));
        } catch (error) {
          const message = error instanceof Error ? error.message : 'Invalid EEG recording';
      ...
      if (loaded.length) {
        ...
        const byId = new Map<string, EegLoadedRecording>();
      ...
      this.errorMessage = error instanceof Error ? error.message : 'Failed to load EEG';
      this.toast.error(this.errorMessage);
    } finally {
      this.loading = false;
      this.cdr.markForCheck();
  ...
  async loadSample(): Promise<void> {
    await this.handleFiles([createSampleEegFile()]);
  }
  ...
  setMontage(mode: WaveformMontageMode): void {
    this.montageMode = mode;
    if (mode === 'bipolar' && this.visibleChannelIdList.length < 2) {
      this.toast.info('Bipolar montage needs at least 2 visible channels');
    }
    this.renderCanvas();
    this.cdr.markForCheck();
  }
  ...
  toggleChannel(id: string): void {
    if (this.visibleChannelIds.has(id)) {
      if (this.visibleChannelIds.size <= 1) {
        this.toast.info('At least one channel must remain visible');
        return;
      }
      this.visibleChannelIds.delete(id);
    } else {
      this.visibleChannelIds.add(id);
    }
    this.renderCanvas();
    this.cdr.markForCheck();
  }
  ...
  isChannelVisible(id: string): boolean {
    return this.visibleChannelIds.has(id);
  }

  showAllChannels(): void {
    const rec = this.currentRecording;
    if (!rec) return;
    this.visibleChannelIds = new Set(rec.waveform.channels.map((c) => c.id));
    this.renderCanvas();
    this.cdr.markForCheck();
  }
  ...
  exportAs(format: EegExportFormat, event?: Event): void {
    event?.stopPropagation();
    this.showExportMenu = false;
    const current = this.currentRecording;
    if (!current) {
    ...
    const base = current.name.replace(/\.[^.]+$/, '') || 'eeg-recording';
    try {
      if (format === 'original') {
        ...
        downloadTextFile(
          exportEegSummaryJson(current, this.visibleChannelIdList, this.montageMode),
          `${base}-summary.json`,
          'application/json'
        );
        this.toast.success('Exported summary JSON');
      } else if (format === 'visible-json') {
        downloadTextFile(
          exportEegVisibleChannelsJson(
            current,
            this.visibleChannelIdList,
            this.montageMode,
            this.startSample,
            this.windowSamples
          ),
        ...
        this.toast.success('Exported visible window JSON');
      } else if (format === 'calipers-json') {
        downloadTextFile(exportEegCalipersJson(current, this.calipers), `${base}-calipers.json`, 'application/json');
  ...
  adjustGain(delta: number): void {
    this.gain = Math.max(8, Math.min(96, this.gain + delta));
    this.renderCanvas();
    this.cdr.markForCheck();
  }
  ...
  setInteractionMode(mode: EegInteractionMode): void {
  ...
  private renderCanvas(): void {
    ...
    const channelHeight = 52;
    const height = Math.max(240, channels.length * channelHeight + 16);
    canvas.width = width;
    canvas.height = height;

    const caliperOverlay =
      this.activeCaliper ?? (this.calipers.length ? this.calipers[this.calipers.length - 1] : null);
